...
### 2026-06-21 — [taking] REQ-0015 (UX ввода: pre-submit чеклист пробелов + 8×5)

Беру REQ-0015. Задача 3 (Excel-заливка по дням) уже закрыта в U5 (fill-handle на ячейке). Делаю: (1) pre-submit чеклист пробелов недели — инлайн над approval-bar; (2) кнопка «Заполнить неделю 8×5» в toolbar (mode=week). Чистые хелперы + тесты, DOM-free. dry-run, без коммита.
