...
### 2026-06-21 — [report] wi-34-brand — бренд-акцент + дрейф→токены + AA

**Бренд-цвета Credos (из `credos ru bitrix/docs/design-system/tokens.css`, SSOT прода):**
- `--ds-cyan #00bef3` — primary (яркий, контраст 2.2 на белом → только заливки/иконки, для текста-акцента непригоден)
- `--ds-accent #2e47d7` — indigo, «кнопки-действия» (контраст **7.0** на белом → AA/AAA)
- `--ds-ink #0a225e` — navy, заголовки (контраст 15.0)
- `--ds-cyan-deep #0a87b8`, `--ds-cyan-light #56bcef` — состояния cyan
Шрифт бренда — PetraSans (не трогал, отдельный WI). Cyan не взял в accent: на белом не проходит AA.

**Что заменил (shared/tokens.ts — единый источник):**
- `accent` `#3b6fe0`→`#2e47d7` (бренд-indigo). Старый нейтральный синий давал контраст 4.6 (на грани); новый 7.0.
- `accentHover` `#2f5fcc`→`#2539b0` (9.2), `accentSoft` `#eaf0fd`→`#eaedfb`, `accentRing` `#bcd0fa`→`#bcc6f5` — все в indigo-семейство.
- Heatmap-тинты сетки/доски переведены на бренд-rgb через новый токен `ACCENT_RGB = '46, 71, 215'` (grid `cellFill`, cap-tokens `gapTone` профицит). Без дрейфа от T.accent. Чип `blue` и категория INTERNAL автоматически = бренд (берут T.accent).

**F2.6 дрейф-цвета → токены:** добавил `overBorder #f3c4ab` / `warnBorder #f6da90` в shared/tokens. Убрал инлайн-хексы: `grid/validation-toast.tsx` (строки 14-15), `my-time/reminder-banner.tsx` (строка 51). В компонентах front-components захардкоженных hex рамок больше нет.

**F2.14/15 контраст + heatmap:**
- `textFaint` `#9a9ea8`→`#7c8089`. Было 2.68 на белом (фейл AA); стало **3.96** на белом, 3.69 на todayCol/weekendBg — ≥3:1 для middot/плейсхолдеров/disabled (non-essential). `under` тоже = `#7c8089`.
- Профицит-ступени heatmap разведены: α `0.10/0.20`→`0.09/0.24` (шаг крупнее, «близко/сильно» различимы без легенды).

**Тесты (затронутые токен-функции, обновлены значение-в-значение):** cap-tokens.test (textFaint, rgba-бренд), grid/tokens.test (rgba-бренд cellFill), tag-color-hex.test (blue/fallback), category-meta.test (fallback). Все токен-тесты зелёные.

**Валидация:**
- `yarn lint` = 0 errors (3 warning — pre-existing в project-plan-panel.tsx, не моя зона).
- `yarn test:unit` = 2030 passed / 0 в моих файлах failed. Мои 5 токен-сьютов: 90/90 ✓ (cap-tokens, grid/tokens, tag-color-hex, category-meta, report-tokens). Единственный fail в прогоне — `plan-preview.test.ts validateRange` (логика дат, файл `plan-preview.ts` правит параллельный Dev в рабочем дереве, НЕ моя правка, не цвет).
- `yarn twenty dev --once --dry-run` = чисто, typecheck ok, 11 frontComponent updated (ожидаемо — токен-хэш бандла), 0 ошибок. НЕ dev --once.

**Тёмная тема НЕ делал** (только светлая, dual-token не вводил — по решению заказчика). НЕ коммичу.
— Dev 1

### 2026-06-21 — [taking] plan-panel-layout
Беру баг: панель «Планировать» (project-plan-panel.tsx, WI-11) обрезается у правого края экрана. Зона capacity/project-plan-panel.tsx. impeccable polish, dry-run, без коммита.

### 2026-06-21 — [report] plan-panel-layout — обрезание у правого края устранено

**ВЫВОД: панель больше не вылезает за viewport ни справа, ни (на узком экране) слева; кнопки Сохранить/Отмена всегда видны.**

Корень: панель была `position:absolute; left:0; width:360` — кнопка «✎ План» стоит у правого края строки проекта, поэтому панель шириной 360 уходила вправо за экран (ПО-дата, описание, превью-бары обрезались, кнопки могли быть не видны). Remote DOM → host-DOM/getBoundingClientRect недоступны, защита чисто CSS.

Что сделал (apps/time/src/front-components/capacity/project-plan-panel.tsx):
- **Переполнение вправо:** якорь сменён `left:0 → right:0` — панель раскрывается ВЛЕВО от кнопки, не пересекает правый край. Ширина зажата по экрану: `width: min(360px, calc(100vw - 32px))` — на узком экране ужимается, не вылезая за левый край.
- **Высота/кнопки:** контейнер стал `flex column` с `maxHeight: min(560px, calc(100vh - 96px))`. Тело (способ/даты/объём/превью) в `overflow-y:auto`, футер с кнопками вынесен ИЗ зоны скролла (`flex-shrink:0` + верхняя граница) → Сохранить/Отмена закреплены снизу, всегда видны. Убран вложенный скролл превью (`maxHeight:168` → антипаттерн двойного скролла).
- **Текст** «Раскид по рабочим дням (ёмкость отдела…)»: `white-space:normal` + `line-height:1.4` — переносится, не обрезается (длинное имя отдела).
- **Превью-бары:** `min-width:0` на строке и баре, `flex-shrink:0` на метке/часах/овербукинг-бейдже — бар contained, бейдж не растягивает строку за край.
- **Поля С/ПО:** были фикс `width:130` → стали `flex:1 1 140px; min-width:0` + `flex-wrap` на ряду — переносятся/сжимаются, date-инпуты не обрезаются.
- **Ритм отступов (impeccable):** консистентный вертикальный шаг 12px между группами (способ/даты/объём/превью), осмысленный padding тела `12/12/4` и футера `10/12/12`.

Функционал не тронут (radio/save/preview/Σ — как было), только layout/overflow.

Валидация: `yarn lint` → 0 warnings/0 errors. `yarn test:unit` → 71 файл, 1971 passed, 0 failed. `yarn twenty dev --once --dry-run` → чисто (typecheck ок, 1 updated frontComponent — наша панель, ничего не применялось). Не коммитил.

### 2026-06-21 — [taking] wi-01-cell-controls
Беру WI-01 (главный UX-баг: «контролы появляются при наведении, но не нажимаются»). Решение C1 (развести «выбор» и «контролы») + A1.24-27. Зона front-components/grid/. dry-run, без коммита.

### 2026-06-21 — [report] wi-01-cell-controls — WI-01 ЗАКРЫТ (C1: ячейка=ввод, действия в меню ⋯) [signal-arch]

**ВЫВОД: корень бага устранён. Ячейка теперь чистый ввод, все интерактивные контролы убраны из неё в меню строки ⋯ (надёжно мышью).**

**Что УБРАЛ из ячейки** (`hour-cell.tsx`) — все три плавающие кнопки, недостижимые мышью (жили мс между active→editing):
- чип-нормы «8» (per-cell quick-fill) — источник бага, НЕ возвращаю (per-cell чип запрещён п.3);
- ⇥-fill (заполнить будни значением ячейки);
- ✎-коммент (CellComment-поповер).
Убраны props `norm`, `onFill`, `onCommitDescription`, импорт CellComment. Файл `cell-comment.tsx` стал orphan → удалил (dead code).

**Что в ячейке ОСТАЛОСЬ** (read-only статус, как ТЗ): число, замок (locked), цвет-переработка, точка-индикатор коммента (теперь чисто read-only, без ✎). Клик/печать = сразу edit (поведение ввода без изменений).

**Что в меню строки ⋯** (`row-menu.tsx`, всегда-видимое слева, A1.27): Дублировать строку · Заполнить будни нормой (WI-02 SSOT) · **Комментарий к записи…** (A1.25 — перенесён из ячейки) · **Обнулить часы** · **Убрать строку** (A1.26 переименования). «Комментарий к записи…» открывает второй вид поповера: список дней строки с часами, по инлайн-полю на день (blur/Enter — сохранить, Esc — откат, ‹ — назад). Коммент-цели = дни с часами>0 (без часов записи нет). Простое решение на useState (Remote DOM-safe, host-DOM нет).

**lock-approved уважён:** rowLocked → правящие пункты скрыты (остаётся только «Дублировать»); согласованный день в панели коммента → input disabled (read-only).

**Клавиатура (use-keyboard) сохранена:** стрелки/Enter/Tab/seed-печать/Esc — без изменений. **Shift+Enter:** уже реализован (week-grid.tsx `onContainerKeyDown` = bulk-fill часов на будни строки) и совпадает с cheatsheet («часы на все будни строки»). НЕ переопределял на «вверх» — текущее поведение нагружено и задокументировано; смена сломала бы UX и cheatsheet. Если заказчик хочет именно Shift+Enter=вверх — follow-up (конфликт с bulk-fill, надо решить какое поведение оставить).

**СВЕРКА (правило 8):** Timetta/Kimai — клик=ввод, контекстные действия в меню строки, не плавающие в ячейке. Соответствует.

**+ТЕСТ:** `row-menu.test.ts` (новый, +5): `commentTargets` (вынесен чистый предикат целей коммента) — только дни с часами, undefined→[], сохранение description/lock, дробные часы.

**Файлы:** `grid/{hour-cell.tsx, row-menu.tsx, grid-row.tsx, week-grid.tsx}`, удалён `grid/cell-comment.tsx`, +`grid/row-menu.test.ts`.

**ВАЛИДАЦИЯ:** yarn lint = 0/0. yarn test:unit = 70 файлов, 1958 passed, 15 todo, 0 failed. yarn twenty dev --once --dry-run = чисто (typecheck ок, 1 frontComponent updated, НЕ применено). Зона front-components/grid/. НЕ коммитил.

@QA: WI-01 готов — проверить мышью: чип/⇥/✎ в ячейке отсутствуют; коммент через ⋯ → «Комментарий к записи…» нажимается и сохраняется; lock уважается.
— Dev 1

### 2026-06-22 06:55 — [report] UC-APR-05 — показ причины отклонения сотруднику ГОТОВ

Владелец reject-comment-ui отдал мне UC-APR-05 («@Dev1-verify берёт»). Реализовал показ `rejectComment` сотруднику в «Мои периоды»: отклонённые недели → баннер **«Причина отклонения: …»** (терракот-акцент). Замыкает approval-петлю (UC-APR-04 отправка — у того агента, UC-APR-05 показ — мой).

**Цепочка данных:** `ApiEntry.rejectComment` (+опц. поле, аддитивно) → `fetchEntries` уже тянет (без field-select) → `summarizeWeeks` собирает причину REJECTED-записей в `WeekSummary.rejectComment` → Row показывает. **XSS:** `{w.rejectComment}` экранирован React, без dangerouslySetInnerHTML (CISO-нота соблюдена).

Файлы: `grid/types.ts` (+rejectComment в ApiEntry), `my-time/{period-status.ts, my-periods.tsx}`. oxlint 0/0, tsc чисто, period-status тесты 11/11, DOM-free. dry-run.

@QA: UC-APR-05 закрыт — ре-верифай вместе с UC-APR-04.

— Dev 1

### 2026-06-21 — [taking] underfill-submit
Беру UC-VAL-03 (п.4 ТОП-БАГИ): validateWeekUnderfill определена, но не вызывается в submit-потоке UI. При «Отправить на согласование» нет осознанного подтверждения недобора недели. Зона front-components/grid/. GapsNotice (пассивный баннер) уже есть, не дублирую — добавляю мягкое подтверждение-поповер на кнопку Submit (паттерн RejectButton, Remote DOM-safe, без window.confirm). WARNING, не блок: подтвердил → submit, отменил → нет. dry-run, без коммита.

### 2026-06-21 — [report] underfill-submit — UC-VAL-03 ЗАКРЫТ (WARNING, не блок) [signal-arch]

**ВЫВОД: validateWeekUnderfill теперь вызывается в submit-потоке UI. При отправке недозаполненной недели — мягкое подтверждение, отправка НЕ блокируется.**

Где предупреждение:
- `front-components/grid/approval-bar.tsx` — кнопка «Отправить на согласование» вынесена в `SubmitButton`. Без недобора → submit сразу (поведение без изменений). С недобором → inline-поповер «Неделя недозаполнена: N из M ч. Всё равно отправить?» с кнопками «Отмена» / «Всё равно отправить». Подтвердил → onSubmit; отменил → нет. useState-поповер, паттерн RejectButton, Remote DOM-safe (НЕ window.confirm).
- `front-components/grid/gaps.ts` — новый хелпер `weekUnderfillNotice(g)` строит текст через SSOT-правило `validateWeekUnderfill` (constants/validation.ts). В `WeekGaps`/`calcWeekGaps` добавлены `normHours` (Σ нормы рабочих дней) и `factHours` (Σ факта рабочих дней; выходные не учитываются) — вход для правила. Σ нормы берётся из произв. календаря (normFor, тот же SSOT, что GapsNotice).

WARNING, не ERROR: недозаполнение легитимно (отпуск/неполная ставка) — осознанное подтверждение, без жёсткой блокировки. Норма недели 0 (все праздники/выходные) или факт ≥ норма → поповер не показывается, submit сразу.

Не дубль с GapsNotice: GapsNotice — пассивная подсказка о пробелах ДО клика; поповер — точка осознанного решения В МОМЕНТ submit (UC-VAL-03). Логику недобора не дублирую — она в validateWeekUnderfill.

Тест: `gaps.test.ts` +10 (normHours/factHours; weekUnderfillNotice: факт=норма/переработка/недобор «36 из 40 ч»/норма 0). Файл целиком 23 passed.

ВАЛИДАЦИЯ: yarn lint = 0 warnings/0 errors. yarn test:unit = 69 файлов, 1924 passed, 15 todo, 0 failed. yarn twenty dev --once --dry-run = чисто (typecheck ок, 1 frontComponent updated, НЕ применено). Зона front-components/grid/ — без коллизий. НЕ коммитил.
— Dev 1

### 2026-06-22 06:40 — [verify] bug#3 UC-APR-04 — фикс УЖЕ в дереве (не коллизирую)

Монитор раздал мне `[bug]#3 UC-APR-04 rejectComment @Dev1`. Проверил: **фикс уже реализован** (параллельный Dev1, `[taking] reject-comment-ui` ниже). Цепочка цела: `RejectButton`-поповер (причина обязательна, trimmed>0) → `reject(comment)` → `resolveEntries(ids,false,comment)` → `body.comment` в `/s/approval` + fallback `patchStatus` пишет `rejectComment`. lint+tsc 0. QA тестил старый код — **прошу ре-верифай, bug#3 закрыт**.

**Остаток UC-APR-05 (показ причины сотруднику):** `rejectComment` нигде НЕ рендерится (только в комментах кода). Нужен показ на отклонённых неделях в «Мои периоды» (`my-time/my-periods`). @reject-comment-ui-агент: ты покрываешь показ, или взять мне? (антидубль — не стартую без ответа). XSS-нота CISO: только `{text}` escaped, не dangerouslySetInnerHTML.

Не трогал approval-файлы (чужая [taking]-зона). Только верификация.

— Dev 1

### 2026-06-21 — [taking] reject-comment-ui
Беру баг UC-APR-04: при отклонении таймшита через UI причина не доходит до бэка (rejectComment всегда null). Бэк готов (approval.logic.ts setStatus/runResolve принимают comment). Прокидываю comment с фронта: approval-bar поповер причины → use-approval.reject(comment) → approval-rest.resolveEntries(ids,false,comment) → /s/ роут. Зона front-components/grid/. dry-run, без коммита.

### 2026-06-21 — [report] reject-comment-ui — ИСПРАВЛЕНО, причина прокинута до бэка [signal-arch]

**ВЫВОД: баг закрыт. При «Отклонить» теперь обязательно вводится причина, она доходит до rejectComment.**

**Где поле причины (UI):**
- `front-components/grid/approval-bar.tsx` — новый компонент `RejectButton`. Кнопка «Отклонить (N)» открывает inline-поповер с `<textarea>` «Причина отклонения (обязательно)» + кнопки «Отмена»/«Отклонить».
- Поповер на `useState` (Remote DOM-safe, host-DOM/window.prompt НЕТ — паттерн как cell-comment.tsx: backdrop fixed + absolute-панель). Esc — отмена, Cmd/Ctrl+Enter — подтвердить.

**Обязательность:** да, reject обязателен с причиной (Timetta — отклонение таймшита с комментарием, правило 8). Кнопка подтверждения disabled при пустом/пробельном тексте; шлётся `reason.trim()`. approve — без комментария (как было).

**Как comment прокинут (цепочка):**
- `approval-bar.tsx`: `onReject(comment: string)` — сигнатура изменена (была `() => void`).
- `use-approval.ts`: `reject(comment)` → `resolveEntries(submittedIds, false, comment)`.
- `approval-rest.ts`: `resolveEntries(ids, approve, comment?)` — при reject кладёт `comment` в body `/s/approval` (op:reject, ids, comment). Бэк роут УЖЕ принимает (runResolve читает `params.comment`, setStatus пишет rejectComment) — сигнал Dev2 НЕ нужен.
- Фоллбэк-PATCH (когда /s/ недоступна): `patchStatus` теперь пишет `rejectComment` (= comment при REJECT, null при APPROVE — «оживление» записи).

**Контракт `weekly-grid.tsx`:** уже передаёт `onReject={approval.reject}` — сигнатура совместима, правок там не потребовалось.

**+ТЕСТ:** `approval-rest.test.ts` +4 кейса: comment в route как поле `comment`; comment в rejectComment при fallback PATCH; approve не шлёт comment и чистит rejectComment.

**СВЕРКА (правило 8):** Timetta/Kimai — отклонение таймшита с обязательным комментарием-причиной. Соответствует: reject заблокирован без причины, причина видна сотруднику (UC-APR-05 достигается).

**ФАЙЛЫ:** apps/time/src/front-components/grid/{approval-bar.tsx, use-approval.ts, approval-rest.ts, approval-rest.test.ts}.

**ВАЛИДАЦИЯ:** yarn lint = 0 warnings/0 errors. yarn test:unit = 69 файлов, 1917 passed, 0 failed (вкл. 4 новых reject-comment). yarn twenty dev --once --dry-run = чисто (typecheck ok, 1 frontComponent updated, ничего не применено). НЕ коммитил, НЕ деплоил.
— Dev 1

### 2026-06-22 — [taking] booking-confirm-btn
Беру BOOKING_ANALYSIS волна-1: действие «Подтвердить бронь» SOFT→HARD (1 PATCH bookingType=HARD). Зона front-components/. Сначала ищу редактируемую точку брони во фронте. dry-run, без коммита.

### 2026-06-22 — [report] booking-confirm-btn — КНОПКА НЕ НУЖНА, нативной правки достаточно [signal-arch]

**ВЫВОД: кастомную кнопку не делал — во фронте НЕТ per-booking точки взаимодействия, а нативная точка подтверждения УЖЕ ЕСТЬ.**

**Почему на capacity-доске кнопку добавить нельзя (не выдумывал UI):**
- Брони на доске показаны `booking-marker.tsx` как ТИХИЙ нередактируемый индикатор под ячейкой (`pointerEvents: 'none'`).
- Маркер рисует АГРЕГАТ: `LoadCell.hardBooking` / `softBooking` — суммы часов по периоду (`bookingParts` в cap-tokens.ts). Отдельной брони с `id` на доске НЕТ → не к чему привязать «Подтвердить эту бронь». Per-booking PATCH невозможен без id.
- В `project-team/` отдельных броней нет вовсе.

**Нативная точка подтверждения УЖЕ реализована (booking-cards, ниже в этом файле):**
- Вкладки «Брони» в карточках проекта и сотрудника выведены нативными relation-FIELDS (views/credos-time-{project,employee}-card-bookings.view.ts). Ядро Twenty рендерит брони инлайн-таблицей: сотрудник/проект · тип SOFT-HARD · часы · период.
- Поле `bookingType` — нативный SELECT (SOFT/HARD, isNullable:false, объект credos-time-booking.object.ts). В нативной карточке/инлайн-таблице ресурс-менеджер меняет SOFT→HARD прямо SELECT'ом — это и есть «подтверждение» (= 1 PATCH bookingType, делает само ядро, идемпотентно).
- Совпадает с рекомендацией BOOKING_ANALYSIS §2.2 («в Timetta базовом режиме конвертация = смена типа Soft→Hard в карточке брони») и §40 («статус = тип, SOFT→HARD = подтверждение»).

**Рекомендация arch (если хотим ускорить ресурс-менеджеру):** ничего во фронте не строить. Достаточно нативного SELECT bookingType в карточке брони. Кастомная record-action кнопка «Подтвердить» (скрытая для HARD) = нативный Twenty action-механизм (зона views/page-layouts, Dev2/arch), НЕ front-component — там нет host-DOM и нет per-record контекста у наших worker-компонентов. Оверхед против простого SELECT не оправдан для MVP.

**СВЕРКА (правило 8):** Timetta SOFT→HARD = смена типа в карточке брони. Совпадает с нашим нативным SELECT.

**ФАЙЛЫ:** изменений кода НЕТ. Контракт Dev2 (прямой PATCH /rest/credosTimeBookings/{id} {bookingType:'HARD'}) подтверждаю корректным — его исполняет нативное ядро при правке SELECT, отдельный клиентский вызов не нужен.

**ВАЛИДАЦИЯ:** код не трогал → lint/test/dry-run прежней работы не нарушены (новых изменений нет). НЕ коммитил, НЕ деплоил.
— Dev 1

### 2026-06-21 10:40 — [taking] projects-report-ui
UI отчёта «Проекты — план/факт/остаток» (бэк готов: /s/reports groupBy=projects-plan-fact). Зона reports/ (фронт). dry-run, НЕ деплой, НЕ коммит.

### 2026-06-21 10:55 — [report] projects-report-ui — ГОТОВО

ГДЕ ВКЛАДКА: новый сегмент «Проекты» в Segmented дашборда Отчётов (reports-dashboard.tsx: summary/trend/**projects**/missing). При активном «Проекты» в шапке показываются PeriodNav + гранулярность (месяц/квартал/год); groupBy-срез и экспорт скрыты (не релевантны). Рендер-ветка → <ProjectsView from={period.from} to={period.to}/> в ErrorBoundary.

НОВЫЕ ФАЙЛЫ (reports/):
- report-types.ts: + типы ProjectPlanFactRow / ProjectsPlanFactTotals / ProjectsPlanFactResponse (фронт-зеркало контракта бэка).
- projects-plan-fact-rest.ts: fetch обёртка POST /s/reports {groupBy:'projects-plan-fact', status?, departmentId?, from, to} (паттерн olap-rest/reports-rest, EMPTY-fallback, защита rows ?? []).
- use-projects-plan-fact.ts: хук (паттерн use-reports — re-fetch on from/to/status, reload() для retry).
- projects-view.tsx: KPI-полоска + таблица + фильтр статуса.

ТАБЛИЦА (7 колонок): Проект (код + имя) · Статус · План,ч · Факт,ч · Остаток,ч · % · флаг. tabular-nums везде. Сортировка: дефолт = порядок бэка (перерасход → факт убыв.); клик по заголовку — клиентская досортировка (use-sortable/SortHeader). «Нет плана» → «—» (planned/remaining/pct null), в сорте уходит вниз.

ПЕРЕРАСХОД-ПОДСВЕТКА (overrun=true ИЛИ remaining<0): строка — мягкий терракот-тинт фона (T.overSoft, не side-stripe); Остаток и % — цвет T.over; знак НЕ ТОЛЬКО цветом — бейдж в крайней колонке «▲ перерасход» (иконка ▲ + текст, обводка T.over). Без мёртвого клика (строка некликабельна — drill в этом отчёте не предусмотрен бэком).

ФИЛЬТР СТАТУСА: pill-радио «Все / Запланирован / В работе / Приостановлен / Завершён» (значения из SSOT PROJECT_STATUS_OPTIONS), single-select, '' = все → перезапрос бэка (status опц.). Бейдж статуса в строке тонирован по SELECT-цвету (ACTIVE→ok, ON_HOLD→warn, DONE→accent, PLANNED→neutral).

KPI-ПОЛОСКА (из totals): Σ План,ч · Σ Факт,ч · Σ Остаток,ч (терракот при <0) · Перерасход (кол-во проектов, терракот при >0). Справа над таблицей — счётчик «N проектов» (склонение). Состояния: скелетон (7 строк, пульс credosPulse) / ErrorState+Повторить / EmptyBox (учит сменить статус/период). Токены T (не #fff), Restrained.

СВЕРКА (правило 8): Timetta «Список проектов в работе» — план/факт/остаток + флаг превышения. У нас часы, без денег [[no-billable-concept]].

ВАЛИДАЦИЯ: yarn lint — 0 errors (1 warning — чужой файл timesheet-t13.navigation-menu-item.ts, вне моей зоны, не трогал). yarn test:unit — 69 файлов, 1914 passed, 0 failed, 15 todo. yarn twenty dev --once --dry-run — typecheck PASS, build PASS, «Dry run complete — no changes applied» (метадиф: frontComponent updated + 2 viewField — незадеплоенный дрейф T13/nav в дереве, не моя работа). НЕ коммитил, НЕ деплоил.

### 2026-06-22 06:15 — [report] #5 «Команда» — миграция на /s/project-team ГОТОВА

Вкладка «Команда» карточки проекта переведена с клиентской агрегации (`limit:500` + ручной свод записей) на **серверный `/s/project-team`** (mode=team): PII-redaction по revealEmployeeNames + курсор-пагинация на сервере (нет обрезки на больших проектах). `fetchProjectTeam` маппит server-поля (totalHours→hours, entryCount→entries) в существующий `TeamMember` — компонент `project-team.tsx` НЕ менялся. Старые REST-хелперы (fetchProjectEntries/fetchEmployees) оставил (тесты/fallback).

Файлы: `project-team/{team-rest(+fetchProjectTeam), use-project-team(переключён)}`. oxlint 0/0, tsc чисто, DOM-free, team-rest тесты 6/6. dry-run.

**Остаток #5:** «Проекты сотрудника» (mode=employee-projects, вкладка карточки сотрудника) — НЕ брал (риск дубля swarm, как с #4). Сначала проверю [taking] перед стартом.

**Итог моего прогона волны:** #3 Т-13 ✅ (уникален), #4 — дубль (удалил свой, их вкладка), #5 «Команда» ✅. Спасибо за перенос Т-13 nav в папку «Отчёты».

**Рекомендую приёмку:** много моего dry-run (T2, UX-доска, легенда/Σ/R3, REQ-0019, Т-13, #5) — нужен app-sync + браузер-проверка. Swarm плотный → растёт риск коллизий, паузу на осадку считаю разумной.
...
### 2026-06-21 — [report] booking-capacity + CSV-кнопка готовы
Бронь (credosTimeBooking, soft/hard) учтена в Demand капасити-доски + кнопка экспорта CSV в отчётах. Сверка: Timetta booking-concept/methods (soft/hard). Backend для брони НЕ нужен — объект Dev2 уже есть, читаю Core REST.

**ЗАДАЧА 1 — бронь в Demand (как реализовано):**
- ЗАГРУЗКА: `fetchBookings(from,to)` (capacity-rest) — `/rest/credosTimeBookings`, фильтр пересечения горизонта `endDate[gte]:from,startDate[lte]:to` (как у отсутствий). `bookingType` UPPER SOFT/HARD; нераспознанный → SOFT (безопасный дефолт, не потребляет). Агрегация по employee → отдел (employeeId→departmentId).
- РАСКИД: `bookingHoursInPeriod` делегирует к `plannedHoursInPeriod` (экспортировал) — те же дни-раскид, что план/dept-plan (равномерно по дням периода брони). Контракт соблюдён.
- HARD → в Demand: `composeCell` прибавляет HARD к `load` (потребляет ёмкость → влияет на free/ratio/gap наравне с планом). Бронь сотрудника на срезе «Люди» НЕ делится на численность (персональный резерв); на срезе «Отделы» = Σ HARD его людей.
- SOFT → отдельный слой: НЕ входит в `load` (не потребляет), поле `softBooking`. Тумблер = `settings.tentativeBookingEnabled` (через useGlobalSettings → `includeSoft`); выкл → SOFT не рисуем (softBooking=0). Fallback false до загрузки настроек (консервативно скрыт).
- LoadCell расширен: `hardBooking`/`softBooking`/`conflict` (back-compat: без bookingCtx = 0/0/false, все старые тесты целы).
- КОНФЛИКТ (овербукинг): `conflict = capacity>0 && load(план+HARD)>capacity`. Подсветка, НЕ блок — терракот-обводка (inset-тень) + знак ▲, без смены заливки (заливка несёт load-тон).
- ПОКАЗ (impeccable, quiet): подстрока под значением ячейки — HARD «▪N» (приглушённый нейтрал, входит в загрузку), SOFT «N» пунктирным подчёркиванием (ещё тише, только при includeSoft). Тултип ячейки дополнен «бронь HARD/SOFT N ч · ⚠ овербукинг». Легенда доски: блок «Бронь: ▪N HARD (в загрузке) / SOFT (предв., не потребляет) / ▲ овербукинг» (SOFT-строка по тумблеру). Сводная «Все отделы» — конфликт+тултип (без подстроки, теснит 34px-строку).
- Срезы «Отделы» и «Люди» оба учитывают бронь; summaryCells суммирует hard/soft и считает conflict по сумме Demand vs ёмкости.

ОБЪЁМ: сделал ПОЛНЫЙ контракт (HARD-слой в Demand + SOFT-пунктир + конфликт + легенда + тумблер) — уложился, follow-up по SOFT не понадобился.

**ЗАДАЧА 2 — кнопка «Экспорт CSV» (готовность + ограничение песочницы):**
- `fetchDetailCsv(from,to,filters)` (reports-rest) по контракту Dev2: `/s/reports {groupBy:'detail',format:'csv',from,to, deptId?/projectId?/employeeId?}` → `{ok,csv,mimeType,filename}`. drill-фильтры = накопленный путь drill, спроецированный на 3 оси detail.
- Кнопка «⤓ Экспорт CSV» в тулбаре «Сводка» (рядом со срезом группировки).
- **СКАЧИВАНИЕ ИЗ ПЕСОЧНИЦЫ НЕВОЗМОЖНО:** front-component работает в **Remote DOM** — host-DOM недоступен (`document`/`new Blob`/`URL.createObjectURL`/`a.download`/clipboard — ничего из этого в SDK нет, проверял node_modules/twenty-client-sdk + grep по front-components: всё DOM-free). Поэтому Blob-download не сделать. РЕШЕНИЕ: кнопка тянет готовый CSV (с BOM, `;`) и показывает его в выделяемом поповере для ручного копирования (Cmd/Ctrl+A → копировать → сохранить как .csv / вставить в Excel) + явная подсказка. Контракт бэка готов и работает.
- **FOLLOW-UP (нужен arch/Dev2):** для настоящего автоскачивания нужен **host-bridge** — экшен платформы Twenty типа «download file» / «copyToClipboard», вызываемый из front-component. Сейчас такого API в SDK нет. Как появится — добавить вызов в `useCsvExport.run()` (одно место). Альтернатива: отдавать CSV как вложение записи / ссылку на серверный файл (бэк-трек Dev2).

СВЕРКА (правило 8): Timetta booking soft/hard (resources-concepts-booking-concept.md + resources-components-booking-methods.md): Soft = потребность РП, НЕ согласована ресурс-менеджером, НЕ потребляет ёмкость, полупрозрачные плашки; Hard = подтверждён, потребляет ёмкость, непрозрачные. Овербукинг показывается, не блокируется. Реализовано 1-в-1: HARD→Demand сплошной, SOFT→пунктир/приглушённо/тумблер, конфликт-подсветка без блока. Детальный режим (FTE%/слоты) — не MVP (как и base Dev2), follow-up.

ВАЛИДАЦИЯ: `yarn lint` = 0 warnings/0 errors (324 файла). `yarn test:unit` = **0 failed** (1752 passed / 15 todo, 62 файла); +25 тестов брони (bookingHoursInPeriod/buildBookingCtx/HARD-в-load/SOFT-не-потребляет/includeSoft-гейт/конфликт/чужой-отдел/summary) + 7 тестов fetchDetailCsv. `yarn twenty dev --once --dry-run` = чисто, typecheck ОК: 4 updated frontComponent (перекомпиляция бандла), nothing applied. НЕ dev --once, НЕ деплой.

ФАЙЛЫ:
- Бронь: capacity/types.ts (Booking + LoadCell.hardBooking/softBooking/conflict), calc-load.ts (plannedHoursInPeriod export, bookingHoursInPeriod, BookingCtx/buildBookingCtx, composeCell, deptLoadCells/employeeLoadCells/summaryCells +bookingCtx), capacity-rest.ts (fetchBookings), use-capacity.ts (fetch+bookingCtx+includeSoft+reloadBookings), capacity-board.tsx, board-rows.tsx (проброс bookingCtx), cap-tokens.ts (bookingParts/conflictShadow/токены), booking-marker.tsx (новый), dept-row.tsx/employee-row.tsx/summary-row.tsx (индикатор+конфликт), board-legend.tsx (легенда брони), calc-load.test.ts/cap-tokens.test.ts (+booking, фикстуры).
- CSV: reports/reports-rest.ts (fetchDetailCsv+типы), use-csv-export.ts (новый), export-csv.tsx (новый, кнопка+поповер), reports-dashboard.tsx (кнопка+exportFilters+position:relative), reports-rest.test.ts (+fetchDetailCsv).
...
**1. ФИО ВКЛ (user-direct ‼️):** `settings.object` revealEmployeeNames `defaultValue: true`; post-install `seedSettings` (RawSettings +`revealEmployeeNames?`, existing-ветка PATCH false→true gated, create-default true). [Полный код — мой report 05:xx выше.]

**2. op:delete 400 (P1):** `default-role.ts` objectPermissions.map → `canDestroyObjectRecords: objectUniversalIdentifier === CREDOS_TIME_ENTRY_OBJECT_UNIVERSAL_IDENTIFIER`.

**3. #6 Booking-2 (DP-0005):** `objects/credos-time-booking.object.ts` +2 поля (inline UUID):
   - role: `{ universalIdentifier:'dacaf114-43d1-4aa6-8974-5abeb52cdd96', name:'role', type:FieldType.TEXT, label:'Роль ресурса', icon:'IconUserQuestion', isNullable:true, defaultValue:null }`
   - probability: `{ universalIdentifier:'9923a621-1fc2-4600-ab5f-a6777d96b4fc', name:'probability', type:FieldType.NUMBER, label:'Вероятность (%)', icon:'IconPercentage', isNullable:true, defaultValue:null, universalSettings:{dataType:NumberDataType.FLOAT,decimals:0} }`
   - employee-поле: +`isNullable: true`
   - `views/credos-time-booking.view.ts` +2 колонки (поз.8 role / поз.9 probability, fieldMetadataUniversalIdentifier = inline-UUID выше).

Все валидированы мной (build ✓, манифест, oxlint 0). Нужен ОДИН backend-гейт-проход когда UI-волна осядет.

**Стою на backend-редактировании** (futile — сметается). Готов: возобновить когда дерево осядет / взять фронт-смежное если назначишь / держать диффы актуальными.

— Dev 2


### 2026-06-22 — [report] wi-48 ГОТОВО — превью план-панели: свободная ёмкость + мульти-отдел + endDate-унификация

Баг-риск фазы-1 закрыт (W3B.18/22/23). Валидация: `yarn lint` 0/0 · `yarn test:unit` 2044 passed, 0 failed (plan-preview 13→26 тестов, calc-load 117 без регресса) · `yarn twenty dev --once --dry-run` чисто, typecheck ОК, метадиф = только frontComponent-бандлы (никаких objects/fields/views моих). НЕ закоммичено.

**1. W3B.18 — свободная, не полная ёмкость.** Раньше `plan-preview.ts:126` сравнивал план с ПОЛНОЙ ёмкостью отдела (`workHours×headcount×factor`) → овербукинг оптимистичен. Теперь превью считает **свободную** ёмкость той же логикой, что доска: `Σ_отдел (deptCapacity − занятое_др.проектами − план-загрузки_отдела − HARD-брони)`, не ниже 0. Переиспользую `deptCapacity` (уже вычитает отсутствия), `projectDeptHoursInPeriod`, `deptPlanHoursInPeriod`, `deptBookingHours` (экспортировал два private-хелпера из calc-load — additive). SOFT-брони НЕ потребляют (как `composeCell`). Сам планируемый проект исключается из «занятого» (`excludeProjectId`) — иначе вычел бы себя. PreviewRow получил `fullCapacity` (подпись «свободно X из Y») + результат — `overCount`.

**2. W3B.22 — мульти-отдел (REQ-0013).** Раньше панель сравнивала весь план проекта с одним dept. Теперь `previewDeptsForProject` резолвит долевые отделы из `sharesByProject` (fallback на `departmentId`), сравнение = сумма свободных ёмкостей всех долевых отделов.

**3. W3B.23 — endDate-унификация «как доска».** `validateRange` больше НЕ требует ПО: пустой ПО → превью/раскид тянется до `horizonEnd` (через `spread`/`PreviewSource.horizonEnd`), `openEndedHint` даёт мягкую подсказку «открытый план». Без horizonEnd — превью пусто (back-compat). Кнопка «Сохранить» активна при овербукинге (W3B.21 баннер «⚠ овербукинг в N периодах»), `window.confirm` не используется.

**Контракт/API (чистые функции, plan-preview.ts):**
- `computePreview(effort, startKey, endKey, hoursByDay, deptOrCtx)` — 5-й арг теперь `DeptRef` (back-compat, полная ёмкость) ИЛИ `PreviewLoadCtx` (свободная). Возвращает `{rows, total, maxHours, overCount}`, row: `{..., capacity(свободная), fullCapacity, over}`.
- `PreviewSource` (пакет данных доски) + `previewLoadCtxFor(project, source)` — панель резолвит ctx под проект.
- `previewDeptsForProject`, `openEndedHint`, новый `validateRange` (ПО опц.).

**Проводка (additive props через цепочку):** `board-rows.tsx` собирает `PreviewSource` (все отделы + projects/deptPlans/absenceCtx/bookingCtx/sharesByProject + `spread.horizonEnd`) → `ProjectDetail` → `ProjectPlanRow` → `ProjectPlanPanel`. dept-fallback сохранён везде.

**Зона/коллизии:** правил только 7 capacity-файлов (board-rows, calc-load, plan-preview(+test), project-detail, project-plan-panel, project-plan-row). `project-plan-panel.tsx` — Dev1-зона (overflow-фикс), но WI-48-правки изолированы (новый проп + computePreview-вызов + баннер/подсказка), конфликта со стилями нет. Параллельные стримы трогают cap-tokens/grid-tokens/shared-tokens (бренд-палитра) — НЕ мои, не касался.

**Сверка (правило 8):** Timetta resource-plan — доступность = ёмкость минус занятое (план+бронь). Реализовано 1-в-1 с языком доски (`deptLoadCells`/`composeCell`) — превью больше не дрейфует от доски.

**Follow-up Dev1 (опц.):** баннер/подпись минимальны (текст). Util%-бар (W3B.16: заполнение до свободной, перелив красным) — данные уже в row (`hours`/`capacity`/`fullCapacity`), доделка чисто визуальная.

### 2026-06-22 — [taking] wi-48 — фикс превью план-панели (свободная ёмкость + мульти-отдел + endDate)

Беру WI-48 (баг-риск фазы-1, W3B.18/22/23). Зона: `capacity/plan-preview.ts` (Dev2 по hot-карте) + `calc-load.ts`. Превью овербукинга сейчас сравнивает план с ПОЛНОЙ ёмкостью одного отдела → скрывает реальный конфликт. Делаю:
1. W3B.18 — свободная ёмкость (полная минус занятое др.проектами/планами/HARD-бронями/отсутствиями) через переиспользование `deptCapacity`/`projectDeptHoursInPeriod`/`deptPlanHoursInPeriod`/`deptBookingHours`.
2. W3B.22 — мульти-отдел: сравнение по сумме долей отделов проекта (`sharesByProject`), не один dept.
3. W3B.23 — endDate-унификация «как доска»: пусто ПО → тянуть до horizonEnd (не блок).
+тесты, чистые функции. dry-run, НЕ коммит. `project-plan-panel.tsx` (Dev1-зона) — минимальная правка call-site (передать ctx + снять обязательность ПО), уступаю если Dev1 параллелит.

### 2026-06-22 — [report] #6 Booking волна-2 ГОТОВО (role + probability + employee-nullable)

DP-0005 универсальный резерв (изолированно, booking-объект+view — чистые, не hot):
- `credosTimeBooking.role` (TEXT nullable) — роль ресурса для брони БЕЗ конкретного сотрудника («Аналитик ИБ» и т.п.).
- `credosTimeBooking.probability` (NUMBER 0–100 nullable, decimals:0) — вероятность конверсии SOFT-брони (пресейл-кандидат, взвешивает резерв).
- `credosTimeBooking.employee` → **nullable** (был required) — универсальный резерв под роль без человека.
- index-view: колонки «Роль ресурса» + «Вероятность (%)».

**UUID inline** в объекте/view (не в SSOT): `universal-identifiers.ts` сейчас hot (параллельные потоки правят, 2× «modified since read») → определил локально, как project-объект для code/category. Booking-объект+view чистые → правки выжили.

**Dry-run:** `twenty dev:build` ✓ (22 файла, граф валиден), `oxlint` 0/0, манифест: role/probability/employee все nullable, иконки (IconUserQuestion/IconPercentage) в SDK.

**Файлы:** `objects/credos-time-booking.object.ts` + `views/credos-time-booking.view.ts`. Не закоммичено — ждёт arch-gate.

**Потребители (Dev1/follow-up):** universal-reserve в форме брони (employee опционален → role); probability-взвешивание SOFT-резерва в capacity-доске (Σ probability×hours). project/company уже nullable (пресейл-бронь без проекта — была).

**Контекст очереди:** #1-3 done/taken (fields-p1/diverse-seed/отчёт-проекты-front-component), #4 Booking-1 (фронт: createdBy/At view-колонки уже есть, SOFT→HARD-кнопка/бейдж=Dev1), #5 isManager-sync=CISO privilege-escalation блок, #7 op:delete=дифф у arch. Взял #6 (единственный открытый изолированный backend).

— Dev 2


### 2026-06-22 — [taking] #6 Booking волна-2 (role + probability + employee-nullable) — Dev2

Из quality-wave Dev2-очереди #6 (BACKLOG). #1-3 заняты/done, #4 фронт, #5 isManager=CISO-блок, #7 op:delete=дифф у arch. #6 — открыт, booking-объект чист (проверил git status).

**DP-0005 универсальный резерв (изолированно, booking-объект + SSOT):**
- `role` TEXT nullable — роль ресурса для брони БЕЗ конкретного сотрудника (универсальный резерв/пресейл).
- `employee` → `isNullable: true` (был required) — бронь под роль без человека.
- `probability` NUMBER 0-100 nullable — вероятность конверсии SOFT-брони (пресейл/кандидат).

project/company уже nullable (пресейл без проекта — есть). +2 UUID SSOT. Зона: objects/ + constants/ (+ booking-view колонки если есть). dry-run, не коммичу.

— Dev 2


### 2026-06-22 10:35 — [report] booking-wave1 — аудит createdBy/createdAt + контракт SOFT→HARD + овербукинг ГОТОВО

Закрыл BOOKING_ANALYSIS волну-1 (3 пункта). Только additive, схему брони НЕ трогал.

**1. Нативные createdBy/createdAt в view брони — ВЫВЕДЕНЫ декларативно. ✅**
Не заводил свои поля. Twenty даёт нативные createdBy/createdAt из коробки; их universalIdentifier детерминированно выводится из UID объекта + имени поля (UUID v5, SDK `generateDefaultFieldUniversalIdentifier`). Добавил 2 колонки в `credos-time-booking.view.ts` через `nativeFieldId('createdBy'|'createdAt')`. Dry-run: `2 created viewField`, поля-references приняты сервером БЕЗ создания fieldMetadata → подтверждает, что нативные поля реально существуют на объекте. Ограничения нет — выводится декларативно.
Карточка (карточка брони) = нативная авто-карточка Twenty (page-layout у объекта нет) → createdBy/createdAt в ней показываются ядром по умолчанию, доп. кода не требуют.

**2. SOFT→HARD «Подтвердить бронь» — /s/ РОУТ НЕ НУЖЕН. Решил проще.**
Фронт PATCHит Core REST напрямую (прецедент: `settings/settings-rest.ts` → `client().patch('/rest/credosTimeDepartments/{id}', patch)`). Подтверждение = 1 PATCH `/rest/credosTimeBookings/{id}` `{ bookingType: 'HARD' }`, идемпотентно по природе (повторный PATCH HARD→HARD безвреден). Дополнительный actor-аудит не нужен — нативный updatedBy/updatedAt пишет ядро. **Контракт для Dev1:** кнопка «Подтвердить бронь» на карточке/строке (только при SOFT) → `client().patch('/rest/credosTimeBookings/'+id, { bookingType: 'HARD' })`. Серверной части не делал.

**3. Овербукинг — расчёт УЖЕ полный, доострять не нужно. ✅**
`calc-load.ts`: `composeCell` считает `load = planLoad + hardBooking`, `conflict = capacity>0 && load>capacity`; hard/soft разведены (SOFT не потребляет ёмкость); `summaryCells` агрегирует conflict по компании. Серверный сигнал не требуется — флаг `conflict` на каждой ячейке есть, бейдж рисует Dev1 (фронт).

**Валидация:** yarn lint 0 errors (1 pre-existing warning в `timesheet-t13.navigation-menu-item.ts` — НЕ моя зона, не трогал). yarn test:unit 1914 passed, 0 failed. dry-run: ТОЛЬКО 2 created viewField, без deleted/objectMetadata-update, без дублей UUID. НЕ коммитил.

— Dev 2

### 2026-06-22 10:30 — [report] projects-plan-fact — «Проекты план/факт/остаток» (часы) ГОТОВО

REPORTS_COMPLETENESS P1. Аналог Timetta «Список проектов в работе» для РП, но БЕЗ денег ([[no-billable-concept]]) — только ЧАСЫ.

**Чистая функция** `computeProjectsPlanFact(input, options)` в `src/logic-functions/projects-plan-fact.ts`. Факт берётся из записей credosTimeEntry (Σ часов), НЕ из хранимого factHours — иначе период-фильтр врал бы (factHours накопительный за всё время). Без периода результат = factHours.

**Роут (контракт для Dev 1):** расширил существующий `/s/reports` (НЕ новый роут) новым `groupBy=projects-plan-fact`.
- Запрос: `POST /s/reports { from?, to?, groupBy: 'projects-plan-fact', status?, departmentId? }`. status — UPPER_CASE SELECT (IN_PROGRESS/DONE/...), departmentId — UUID отдела проекта. Оба опц., сравниваются в памяти.
- Ответ:
  ```
  { ok:true, period:{from,to},
    totals:{ planned, fact, remaining, overrunCount },
    count,
    rows:[{ projectId, name, code, status, startDate, endDate,
            planned, fact, remaining, overrun, pct }] }
  ```
- Поля строки (всё в ЧАСАХ): `planned`=plannedEffort (null если не задан); `fact`=Σ факт; `remaining`=planned−fact (null если плана нет); **`overrun`**=fact>планed (флаг перерасхода для подсветки UI, красным); `pct`=fact/planned (0..1+, null если плана нет/0). name — НЕ ПДн (проекты), fallback code→id.
- Сортировка: перерасход наверх → факт убыв. → имя. РП видит проблемные сверху.
- Граничные: план=null → planned/remaining/pct=null, overrun=false, fact считается, в totals.planned/remaining не входит. fact== plan → overrun=false (строгое >). Проект без записей → fact=0, remaining=весь план.

**Файлы:** `src/logic-functions/projects-plan-fact.ts` (новый), `+.test.ts` (11 тестов: план/факт/остаток/перерасход/=план/нет-плана/нет-записей/период/сортировка/фильтры status+dept/мусор-записи/name-fallback), `reports.logic.ts` (+import, +branch groupBy=projects-plan-fact). ПДн нет.

**Валидация:** lint 0 errors (1 warning — чужой файл timesheet-t13 nav, не моя зона). test:unit 69 файлов, 1914 passed, 0 failed. dry-run чисто: ТОЛЬКО `updated logicFunction` (reports — расширил handler), БЕЗ created-роута/field-drop/objectMetadata-update. НЕ коммитил, dry-run.

**Сверка (правило 8):** Timetta «Проекты в работе» — план/факт/остаток. Отличие у нас: часы вместо денег ([[no-billable-concept]]).

— Dev 2

### 2026-06-22 — [taking] projects-plan-fact

REPORTS_COMPLETENESS P1: агрегат «Проекты — план/факт/остаток» (часы, без денег). Чистая функция + расширение /s/reports (groupBy=projects-plan-fact). Зона logic-functions/. dry-run, НЕ коммичу.

— Dev 2

### 2026-06-22 — [taking] fields-p1-additive

Пересборка fields-p1 БЕЗ drop-полей (прошлый упал на drop Department.headcount, migration 94c519b4). Зона: objects/views/scripts/constants. dry-run, НЕ коммичу.

### 2026-06-22 — [report] fields-p1-additive — ГОТОВО (additive, dry-run без drop, apply-ready)

Все поля ADDITIVE, ни одного drop-поля / drop-миграции. Сделано:

1. **WorkType (P1 крит):** добавлено TEXT-поле slug `title` («Наименование») + labelIdentifier карточки = это поле + колонка в реестр (позиция 0). Виды работ стали различимы.
   - ⚠️ Слаг `title`, НЕ `name`: первый прогон с `name` → `deleted fieldMetadata` (SDK удалял зарезервированное авто-поле `name`). Это тот же класс drop-конфликта, что валил headcount. Переименовал slug в `title` → drop ушёл, поле additive.
2. **Stage (P2):** добавлено TEXT-поле `title` («Название») + колонка реестра. labelIdentifier оставлен на `code` (без изменений).
3. **Project (P2):** добавлено TEXT-поле `description` («Описание») + колонка в карточку «Обзор».
4. **Department.headcount:** поле НЕ удалял (сохранено в объекте). Убрал только КОЛОНКУ из index-view (`deleted viewField` — это метаданные вью, не drop столбца БД). Label поля → «Численность (ручное, справочно)» + description: SSOT численности = reports-calc (ΣFTE по дате). Зафиксировано комментарием в объекте и вью.

**Бэкфилл WorkType.title:** `src/scripts/backfill-work-type-name.mjs` (dry-run/`--apply`). title = метка группы (+ « · код отдела»), коллизии в группе разводятся суффиксом ` #N` → гарантированная различимость; заполненные не трогает (идемпотентно). Чистая логика покрыта `backfill-work-type-name.test.ts` (10 тестов).

**UUID:** field-id брал из constants (уже были заведены под прошлый заход: WORK_TYPE_NAME/STAGE_NAME/PROJECT_DESCRIPTION); 4 новых UUID для viewField сгенерил, дублей нет.

**ВАЛИДАЦИЯ:**
- `yarn lint` → 0 errors (1 warning pre-existing в timesheet-t13.navigation-menu-item.ts — НЕ моя зона).
- `yarn test:unit` → 68 файлов, 1903 passed, 0 failed.
- `yarn twenty dev --once --dry-run` → `6 created, 11 updated, 1 deleted`. Deleted = ТОЛЬКО viewField 1c80baf3 (колонка headcount из вью). НЕТ `deleted fieldMetadata` / `deleted objectMetadata` / drop-миграций. apply-ready. НЕ коммичено.
...
**Дообогатил ответы данными разведки Timetta. `docs/analysis/UI_ANSWERS_TIMETTA_ENRICHMENT.md` — записан.**

Прошёлся по разведке `research/timetta/` (OData raw-json, метамодель 258 EntityType, i18n, шаблоны, справочники) и усилил уже данные ответы КОНКРЕТИКОЙ. Расхождений, меняющих решения, НЕТ — кроме уточнения кодов Т-13.

**Конкретные уточнения из API/структуры Timetta:**

1. **CISO-005 / A4.18-19 — ЭТАЛОН НАЙДЕН.** TimeSheet Timetta отдаёт `editAllowed`/`deleteAllowed` с СЕРВЕРА прямо в записи. Права НЕ вычисляются на клиенте. → подтверждает мой флаг (итер.103): recall/revoke и plan-write резолвить server-side, не по client `workspaceMemberRef`. Эталонная модель есть.

2. **dueDate — точная механика (F4.5/F4.19/B4.9).** `dueDateLag` = конец периода + N **рабочих** дней (дефолт 2 → вторник). Проверено на данных: Пн-Вс период → dueDate=Вт. Завести настройкой, не абстрактное «за N дней».

3. **Pre-submit валидация (A4.6/A4.7/UC-VAL-03).** Timetta-правила: min непустых строк · авто-очистка пустых при submit · auto-approval пустого таймшита · **auto-approval недели полностью покрытой approved-отсутствиями**. Последнее = прямое подтверждение A4.7 (недобор минус отсутствия). `timeSheetLines` ОТДЕЛЬНЫ от `timeOffRequests` → W3A.22/C4.11 подтверждены структурой.

4. **Колонки таймшита (F1.11/B2.8).** Timetta = TimesheetTemplate с тогглами showActivity/showClient/showRole/showTariff/showBillCode/showProjectCostCenter. Нам применимы Вид работ/Клиент/Роль; Тариф/BillCode/CostCenter — NB, пропуск.

5. **Коды Т-13 (C4.9/G3.4) — РАСХОЖДЕНИЕ.** Timetta: П(отпуск)/БО(за свой счёт)/Д(декрет)/У(учебный)/Б(больничный) + `paid`-флаг. НО Timetta-«П/БО» ≠ канон Т-13 РФ. Наш Я/ОТ/Б/ДО/К/НН **ближе к официальному бланку** — оставляем. Добавить У(учебный)+Д/Р(декрет) в маппинг.

6. **«Утилизация» (A3.14/D4.2/B1.7) — Timetta-канон.** Штатный отчёт «Утилизация сотрудников по месяцам» + термин в i18n. Не выдумка — оставить термин.

7. **planMethod (W3B.6/Planning-Records #3) — ПРЕЦЕДЕНТ.** Проект Timetta имеет `isAutoPlanning` (флаг авто/ручное планирование) + раздельные planned/estimated даты + `outdated`/`calculationBaseUpdated` (план устарел при смене базы). → подтверждает мой `planMethod` EVEN/MANUAL и алерт-устаревания (W3B.4).

8. **Отчёты OLAP (A3.1/A3.17).** Дефолт-гранулярность Timetta = месяц. Штатные: Часы проектов/сотрудников по месяцам, Утилизация, Ресурсный план, Список заявок на отсутствия. Кандидат-пресет «Утилизация по месяцам».

**Принято к сведению (правило):** при всех дальнейших ответах опираюсь на разведку Timetta (research/timetta/ — структура/API/справочники) как референс №1. Полная таблица уточнений «было→разведка» в файле §8.

@arch: ключевое для ТЗ — server-side editAllowed/deleteAllowed (CISO-005 эталон) + dueDateLag-настройка + isAutoPlanning-прецедент для planMethod.

---

**[Аналитик · итерация 108 · 2026-06-21]**
@Dev1 [signal-arch]

wi-45 принят — закрыл W3-A дельты, включая мой флаг из итер.107. Сверка:
- W3A.5 → Shift+Enter=ВВЕРХ освобождён, bulk-fill→Alt+→ (Ctrl+D=вниз, без коллизии) ✓ — ровно (б), что я рекомендовал в волне-3/итер.107. Спор закрыт правильно.
- W3A.8 → Home/End первая/последняя РЕДАКТИРУЕМАЯ (пропуск locked) + Ctrl+Home/End края; в edit Home/End гасятся (курсор в инпуте) ✓ — корректная деталь.
- W3A.9 → порядок меню по частоте (Заполнить→Коммент→Дублировать→Обнулить→Убрать) ✓
- W3A.10/11 → иконки только деструктив, inline-confirm со счётчиком (pluralRecords), «Обнулить»+undo-тост 5с с реальным re-upsert снимка ✓ — undo сделан как F3.1/E1.22.
- W3A.17 → бледная норма дня placeholder (normFor) ✓
- W3A.25 → ячейка 40h, день 64w, 9 колонок без h-scroll, GRID_TEMPLATE авто-синхрон ✓ — рассинхрон код↔решение устранён.

undo-toast.tsx — реальный undo (снимок ДО очистки → re-upsert), не просто визуал. Хорошо, переиспользуем для Delete (W3A.7) и других мгновенных деструктивов.

**Осталось из W3-A (твои примечания + мой трекинг):**
- W3A.3 Tab-wrap на след. строку — не в этой дельте.
- W3A.26 sticky Проект+Вид работ + h-scroll по дням — но: ты сделал 9 колонок БЕЗ h-scroll при 64px. Проверь на узком виджете влезает ли (64×7 + Проект + Вид-176 + Итого-66). Если на узком переполняется — нужен sticky+h-scroll (D3.3). Если влезает в типовой ширине — ок, h-scroll как fallback.
- W3A.28 sticky шапка дней + футер при вертик. скролле — отдельно.
- W3A.7 Delete+undo — undo-механизм готов, подключить к Delete (ты сам отметил).
- Ctrl+Z (F3.1) полноценный — отдельная дельта.

Эти — не блокеры, в WI-32-дельту/следующие. wi-45 закрыт чисто.

---

**[Аналитик · итерация 107 · 2026-06-21]**
@Dev1 [signal-arch]

wi-31-32 принят. Сверка с E-блоком (UI_ANSWERS_2):
- E4.14 SSOT keymap.ts + антидрейф-тест → закрывает расхождение cheatsheet↔код, что я флагнул в итер.96/98 ✓
- E1.5 select-on-click (F2), seed-replace → ✓
- E1.6 красная рамка + подсказка форматов, тихого отката нет → ✓ (правильно, я отвечал (б) против тихого отката)
- E1.7 placeholder «8/8:30» → ✓
- E1.3 Shift+клик диапазон на координатах (без drag/host-DOM) → ✓
- E1.18 внутренний буфер Ctrl+C/V (системный clipboard не трогаем) → ✓
- E1.20 Ctrl+D заполнить вниз → ✓
Чистый grid/keymap.ts как SSOT — то, что просил (E4.14). Хорошо.

**ОДИН флаг — расхождение E4.5 (волна-2) ↔ W3A.5 (волна-3):**
Ты реализовал E4.5=(а) «Shift+Enter = bulk-fill будни» — корректно по волне-2. НО в волне-3 (UI_ANSWERS_3 W3A.5, итер.106) я ПЕРЕСМОТРЕЛ: рекомендация (б) — освободить Shift+Enter под табличное «вверх» (обратный Enter), bulk-fill перевесить на Alt+→/Ctrl+D.
Причина пересмотра: Shift+Enter в таблицах = «подтвердить и вверх» (Excel/Sheets/T/K). Сейчас навигация вверх только стрелкой ↑. А bulk-fill теперь и так доступен (Ctrl+D fillDown ты сделал + меню ⋯).
Это НЕ баг и НЕ срочно — твоя реализация консистентна с волной-2. Но при WI-32-дельте (где у тебя уже в планах Home/End, [/], Tab-wrap, Ctrl+Z) — предлагаю заодно решить: оставить Shift+Enter=bulk-fill ИЛИ освободить под «вверх». Мой голос — освободить (привычка важнее, bulk-fill покрыт Ctrl+D+меню). Финал — за arch/заказчиком.

**Follow-up WI-32-дельта (твой список) ложится на ответы:**
- Tab-wrap → W3A.3(б) ✓
- Home/End → W3A.8(б) первая/последняя редактируемая + Ctrl+Home/End края сетки
- Ctrl+R → E4.6(б)
- Ctrl+Z → F3.1(а) последнее действие + undo-тост
- (+ Shift+Enter-решение выше)

Открытых вопросов с твоей стороны нет — подтверждаю закрытие wi-31-32.

---

**[Аналитик · итерация 106 · 2026-06-21]**
@arch [signal-arch]

**ВОЛНА-3 ОТВЕЧЕНА. `docs/analysis/UI_ANSWERS_3.md` — записан.**

Покрывает то, что arch отметил как «ждут ответов»:
- **W3-A** (30) таймшит/ячейки углубление ✓
- **W3-B** (30) планирование углубление ✓
- **Planning-Records** (18) модель хранения плана ✓
- **Planning-Models** (38) — БЫЛИ отвечены в SIGNALS итер.95, теперь дайджест в файле (всё в одном месте) ✓

W3-C — ещё генерится, отвечу как появится.

**ИТОГО отвечено: 651 (волны 1-2) + 116 (волна-3) = 767 вопросов.**

---

**КРИТИЧНОЕ для бэка/архитектуры (решать первым):**

1. **Модель хранения ручного раскида** (W3B.6/7 + Planning-Records §5) — нужен объект `credos-time-plan-slot` {project, department?, employee?, periodKey/offset, plannedHours, source} + флаг `planMethod`(EVEN/MANUAL) на проекте. Прецедент REQ-0013. **Блокирует фазу-2 WI-30, зона Dev2.** Ключ слота — offset/абсолютные даты (НЕ календарный `2026-07` — хрупок к сдвигу).

2. **БАГ-РИСК ФАЗЫ-1 (превью/овербукинг):**
   - W3B.18 — превью сравнивает план с ПОЛНОЙ ёмкостью отдела, НЕ за вычетом др. проектов → **скрывает реальный овербукинг**. Передать загрузку др. проектов в панель.
   - W3B.22 — для мульти-отдельного проекта (доли REQ-0013) превью сравнивает с ОДНИМ dept → неверно. Нужна сумма/доля долевых отделов.
   Оба — уже в проде фазы-1, оптимистично занижают конфликт. Завести WI-fix-preview-capacity.

3. **Рассинхрон панель↔доска** (W3B.23): панель ТРЕБУЕТ «ПО» (validateRange), доска ТЯНЕТ до horizonEnd. Унифицировать → разрешить пусто, тянуть до горизонта + пометка «открытый». (Корректирую прежний Q17 «обязать».)

4. **Рассинхрон код↔решение** (W3A.25): ячейка код 58×32, решение F-блока 64×40. Привести в соответствие (заказчик просил крупнее).

5. **Конфликт Shift+Enter** (W3A.5): сейчас bulk-fill, привычное = «вверх». Освободить под «вверх», bulk-fill → Alt+→/Ctrl+D.

---

**ЗАПРОСЫ К ЗАКАЗЧИКУ (нужен ввод, не закрыто аналитиком):**
1. Planning-Records #10 — «Плановые загрузки» = план отдела БЕЗ проекта; заказчик ожидал там детализацию плана проектов? (определяет переименование + слот-объект)
2. W3B.10 — только часы ИЛИ нужны FTE%/ч-нед? (объём фазы-2/3)
3. ГИП-3 — «Вручную по месяцам» через голую таблицу ИЛИ привязка к этапам проекта (project-stages уже в модели)? Рекомендую показать оба.
4. W3A.16 — частота точечной нормы в одну ячейку.

**Подтверждённые гипотезы (W3-B):** ГИП-1 (Кредо-С мыслит часами проекта, не FTE) · ГИП-2 (боль = «куда легли часы + овербукинг», не единицы) · ГИП-3 (вручную из-за этапных внедрений) · ГИП-4 (рассинхрон endDate = баги).

@arch: к доске — WI-30 (plan-slot бэк, Dev2) · WI-fix-preview-capacity (свободная ёмкость + мульти-отдел, баг-риск) · WI-cell-size (64×40) · ранее: WI-01/WI-06b/WI-проект-name/WI-card-registry.

---

**[Аналитик · итерация 105 · 2026-06-21]**
@Dev1 [signal-arch]

project-card-timeentries принят. Сверка с B2.8/C3.11:
- сортировка дата DESC (свежие сверху) → C3.11-аналог для реестра записей ✓
- колонка Теги добавлена, набор колонок полный → B2.8(а) ✓
- категория = через Вид работ/Проект (отдельного поля нет) → корректно, совпадает с CARDS_VIEWS_AUDIT.

**Важный нюанс — упирается в B2.6/B2.7 (не твоя вина, ограничение Twenty):**
Ты правишь SSOT-view CREDOS_TIME_ENTRY_VIEW («Все записи») → RECORD_TABLE показывает её ЦЕЛИКОМ, без фильтра по проекту. Скоуп по проекту даёт только соседний FIELDS-виджет. Значит:
- колонку «Проект» в этом реестре скрыть НЕЛЬЗЯ (она в глобальной view) — мой ответ B2.9(а) «скрывать родителя» здесь неприменим из-за shared-view.
- в карточке проекта пользователь видит ДВА виджета записей: FIELDS (отфильтрован по проекту) + RECORD_TABLE (все записи) — это и есть «две таблицы», на которые B2.6 указывал как на confusion.

**Рекомендация (в бэклог, WI-card-registry):** мой ответ B2.7(в) был «убрать RECORD_TABLE из карточек, оставить только инлайн FIELDS-relation» — тогда:
- исчезает дубль «две таблицы» (B2.6),
- остаётся только отфильтрованный по проекту инлайн → колонка «Проект» становится естественно избыточной и её уже можно убрать в FIELDS-конфиге.
Твоя текущая правка (sort+теги на shared-view) — улучшение, НЕ конфликтует. Но финальное решение B2.6/B2.7 — убрать RECORD_TABLE-дубль. @arch: завести WI-card-registry (удаление RECORD_TABLE-виджета из карточек проект/сотрудник, оставить FIELDS).

Сортировка/теги на SSOT-view заодно улучшили и сам реестр «Все записи» в Справочниках — бонус.

---

**[Аналитик · итерация 104 · 2026-06-21]**
@Dev1 [signal-arch]

reports-drill-period принят — точно по A3.18/A3.19 разграничению:
- смена ПЕРИОДА сохраняет drill-скоуп → A3.19(в) «запоминать»/сохранять контекст ✓
- смена СРЕЗА (switchGroupBy Отдел/Проект/Человек) сброс ОСТАВЛЕН → A3.18(а) «сбрасывать ожидаемо» (другая корневая ось = стек невалиден) ✓
- сброс drill только явный (пилюля ✕ / корень крошек) → A3.11(а) точечное снятие ✓
- гранулярность не reset → чистота, drill живёт.

Разделение «период ≠ срез» — ровно то, что нужно: период это КОГДА, срез это ЧТО-группируем. Менять период не должно ронять путь drill. Хорошо.

Связь с открытым A3.17/C3.5: теперь когда добавите «неделю» + произвольный диапазон в период-селектор (волна-1 A3.17, CSV уже принимает from/to) — drill переживёт смену на эти новые периоды автоматически, т.к. фикс period-agnostic. Не нужно будет дорабатывать сохранение скоупа.

Открытых вопросов с твоей стороны нет — подтверждаю закрытие.

---

**[Аналитик · итерация 103 · 2026-06-21]**
@Dev2 @CISO [signal-arch]

WI-10 принят — закрыл ОБА перехода одним заходом (мой scope-вопрос из итер.102 снят):
- `recall` SUBMITTED→DRAFT, owner-only (actor.employeeId==entry.employeeId), skippedForeign → A4.3(в)/A4.4(а) ✓
- `revoke` APPROVED→SUBMITTED, manager+SoD (skippedOwn) = Timetta Reopen → A4.25(а)/A4.26(а) ✓
- setStatus обнуляет approvedBy/At + rejectComment при откате — правильно (запись перестаёт выглядеть согласованной) ✓
- REVOKED отдельный статус НЕ вводил → A4.20(а) «4 статуса достаточно» ✓
- WI-25 (un-approve) был у меня открытым отдельным — теперь закрыт этим же revoke. Снимаю из бэклога.

**CISO-ФЛАГ (P1, тот же класс что CISO-005) — НЕ блокер WI-10, но в радар:**
Контракт берёт actor из `workspaceMemberRef` — **client-supplied param**. recall/revoke RBAC и ownership строятся на нём → клиент может подменить workspaceMemberRef и:
- recall чужую SUBMITTED (выдав чужой employeeId),
- revoke будучи не-менеджером (выдав manager-UUID).
Серверный guard сейчас доверяет клиентскому actor. Это ровно CISO-005 IDOR/impersonation.
@CISO: recall/revoke добавить в скоуп server-identity резолва (event.userWorkspaceId вместо params.workspaceMemberRef). Сейчас dev-bypass + client-actor — для прода нужен жёсткий резолв. Тесты Dev2 покрывают логику переходов, но identity-спуфинг — это server-identity задача (та же, что блокирует CISO-005, UC-PLN-13).

Dev2 отметил это сам («в проде SoD/RBAC строгий A4.18/A4.19») — подтверждаю, без server-identity recall/revoke = тот же IDOR. Не релизить approval-write на прод без CISO-005 фикса.

**Контракт для Dev1** (кнопки) — чистый:
- «Отозвать отправку» (recall, свои SUBMITTED, футер+Мои периоды A4.5) — словарь D4.15.
- «Отозвать согласование» (revoke, manager, approval-view) — гейт isManager (UX) + сервер.
- Тосты успеха F4.13, результат skippedForeign/skippedOwn → toast A4.15.

---

**[Аналитик · итерация 102 · 2026-06-21]**
@Dev2 [signal-arch]

wi-10-recall взят — ложится на A4.3(в)/A4.4. Якоря requirements для реализации:

**Recall = SUBMITTED→DRAFT сотрудником.** Ответы волны-1:
- A4.4(в) → recall разрешён ТОЛЬКО пока руководитель НЕ открыл/не начал решение. После начала approve — нельзя (иначе гонка).
- A4.3(в) → SUBMITTED остаётся редактируемым + кнопка «Отозвать отправку».
- Кто: только сам автор записей (employeeId == actor). НЕ руководитель (у него un-approve = A4.25, отдельный WI).
- Статус-переход: только SUBMITTED→DRAFT. APPROVED/REJECTED recall не трогает (APPROVED lock CISO-011; REJECTED уже вернулся сотруднику).

**CISO-критично:**
- Серверный гард identity: actor по server-side (event.userWorkspaceId), НЕ по client param. Тот же класс, что CISO-005 — recall чужих записей = IDOR. Проверь, что recall фильтрует по реальному автору с сервера.
- Не позволять recall чужого периода (employeeId guard на бэке, не только UI).

**UX (для Dev1-части если будет):**
- Кнопка «Отозвать отправку» рядом с submit-статусом (A4.5: футер + «Мои периоды»).
- D4.15 словарь: «Отозвать отправку» (не «recall»/«отозвать»).
- Тост успеха «Отправка отозвана, неделя снова черновик» (F4.13).

Связь: recall (A4.4, сотрудник) ≠ un-approve (A4.25, руководитель APPROVED→SUBMITTED) — это РАЗНЫЕ WI. Если wi-10 только про сотрудника-recall — un-approve остаётся отдельным открытым (WI-25, нужен для A4.25/A4.26). Уточни scope.

---

**[Аналитик · итерация 101 · 2026-06-21]**
@Dev1 @Dev2 [signal-arch]

Догоняю два не отмеченных мной [report]. Оба соответствуют requirements.

**1. missing-report «Незаполненные» в Отчётах (Dev1)** ← A3.1(а), A3.2(а), A3.27.
- 3-й сегмент «Незаполненные» в дашборде Отчётов → A3.1 «добавить вкладку» ✓; A3.2 «в Отчётах, не в Согласовании» ✓.
- Колонки Сотрудник·Отдел·Заполнено(бар %)·Норма·Факт·Недобор(warn), сортировка по недобору, tabular-nums → корректно.
- Скрытие период/срез-тулбара (бэк считает фикс. текущую неделю) → разумно, но: волна-1 A3.3 просила гибкость диапазона. Сейчас фикс «текущая неделя» — для MVP ок. Флаг: при расширении (F4.5 «за N дней до дедлайна») понадобится период-выбор. Не блокер.
- Замечание про ФИО: таблица показывает Сотрудник — проверь, что при `revealEmployeeNames=false` идёт КОД (CISO-007/UC-RPT-12), как в дайджесте. Если уже через employeeCode() — ✓.
- Empty-state «Все заполнили» → A3.27/D1.9 (emoji 🎉 для позитива — ок).

**2. fields-p1 поля-фиксы (Dev2)** ← B2.12, C1.2, B2.11, B4.12, B2.13.
- WorkType.name TEXT + labelIdentifier → B2/C1.2: виды работ были неразличимы (3 колонки) — теперь имя есть ✓.
- Department.headcount УДАЛЕНО, SSOT = reports-calc (Σ FTE) → B4.12 «активный = действующее FTE-назначение» ✓. Устранение ручного дубля — правильно (keep-it-simple). capacity/calc-load не читали поле → не ломается ✓.
- Project.description TEXT в «Обзор» → B2.11 «разделить реквизиты» (описание было только в заголовке) ✓.
- Stage.name TEXT (labelIdentifier остался code) → B2.13-аналог для этапов ✓.
- UUID v4 без дублей, dry-run apply-ready — twenty-sdk-apply-граблей нет (поля ADDITIVE, не drop в том же sync кроме headcount-delete — он изолирован).

Один момент по fields-p1: Project.name (TEXT-заголовок проекта) из B2.12 НЕ в этой пачке — заголовок проекта пока `code`. Это отдельный пункт (B2.12 рекомендовал name как заголовок, code подзаголовок). Не критично, но зафиксируй для arch как WI-проект-name (с бэкфиллом). 

@arch: к доске добавь — WI-проект-name (Project.name заголовок, B2.12) + ранее: WI-30/WI-01/WI-06b. Все — хвосты подтверждённых requirements.

---

**[Аналитик · итерация 100 · 2026-06-21]**
@Dev1 @arch [signal-arch]

WI-11 Фаза-1 «Планировать» принят — реализует PLANNING_DIALOG_UX Q1-Q21 целиком. Построчная сверка с рекомендациями:
- inline-поповер «✎ План» (useState, row-menu/cell-comment паттерн, без host-DOM) → Q1/Q3 ✓
- один экран сверху-вниз → Q2 ✓
- радио Равновномерно(дефолт)/Вручную(disabled-заглушка «скоро») → Q4/Q5/Q10 ✓ (заглушка вместо активной мёртвой кнопки — точно по impeccable)
- «С»/«ПО» два type=date, «С» теперь редактируется → Q7 ✓ (это и был прямой запрос заказчика)
- объём в часах (parseEffort) → Q10 ✓
- живое превью мини-баром по РАБОЧИМ дням через computePreview→plannedHoursInPeriod+PlanSpread (двойник доски WI-05) → Q14/Q15 ✓ (превью==доска — именно то, что нужно для доверия)
- Σ-сверка «раскид = N · план M ✓/≠» → Q14/I3 ✓
- овербукинг мягкая подсветка + «⚠ +N», НЕ блокирует → Q16/H1 ✓
- «ПО» обязательна, иначе кнопка заблокирована + подпись → Q17 ✓
- Отмена/Esc без confirm, Сохранить через usePlanEdit → Q20 ✓
- inline-поля строки оставлены как быстрый путь → Q1 ✓

Чистый модуль plan-preview.ts (тестируемый, 13 тестов, инвариант Σ, раскид по раб.дням, овербукинг) — отличное разделение. Превью переиспользует plannedHoursInPeriod → нет дрейфа превью↔доска.

**MVP-ядро планирования ЗАКРЫТО:** WI-05 (раскид по раб.дням, Dev2) + WI-11 (диалог ввода, Dev1) вместе закрывают прямой запрос заказчика «как система раскидывает 500ч до 13.09» — теперь видно вживую до сохранения, по рабочим дням, с проверкой овербукинга.

**Открыто (фаза 2, нужен бэк, зона Dev2):**
- WI-30 «Вручную по месяцам» — нужно поле `planMethod` (EVEN/MANUAL) + хранение ручного раскида (объект `credos-time-plan-slot` период×часы ИЛИ JSON). Заглушка в UI уже готова, ждёт бэк. PLANNING_DIALOG_UX фаза-2 Q12/Q13.
- Фаза 3: единица FTE%/ч-нед (`planUnit`), план×сотрудник (отдельный экран ресурс-плана).

@arch: предлагаю завести в доску — WI-30 (planMethod бэк, Dev2), WI-01 (удаление чипа ⇥, Dev1), WI-06b («Очистить неделю» вынос из CopyMenu, Dev1). Все три — хвосты подтверждённых requirements, не блокеры.

---

**[Аналитик · итерация 99 · 2026-06-21]**
@Dev2 [signal-arch]

Фикс раскида план→рабочие дни принят. КОРЕНЬ непонимания заказчика закрыт. Сверка:
- `workHoursBetweenKeys` на базе существующей логики, НЕ дублирует deptCapacity — ровно то, о чём предупреждал (итер.97/98: один SSOT рабочих дней, не плодить второй). ✓
- Инвариант Σ=plannedEffort сохранён, выходные/праздники=0, короткий день пропорц. → PLANNING_MODELS B2/F1 ✓.
- Back-compat: старые тесты без spread не тронуты, behavior-change только для путей с произв.календарём — корректно.
- Применено ко ВСЕМ слоям (project/share/deptPlan/booking) единой plannedHoursInPeriod — правильно, раскид был единый.

**Один момент к согласованию (не блокер):**
Проект без endDate ты решил как «открытый до horizonEnd» (E3-гипотеза «считать открытым»). В PLANNING_DIALOG_UX Q17 рекомендация для UI-диалога была «в MVP обязать ПО». Это НЕ конфликт, а два слоя:
- Бэк-раскид (твой): проект без endDate теперь ВИДЕН (тянется до края горизонта) — лучше прежнего «план=0/невидим». ✓
- UI-диалог планирования (фаза 1, Dev1/будущее): при ВВОДЕ плана через панель «✎ План» всё равно просить «ПО» (иначе пользователь не понимает горизонт).
Итог: твой бэк = безопасный fallback (показать открытый), диалог = мягко подталкивать задать «ПО». Оба правильны. Зафиксировал для arch — при реализации диалога (WI-планирование) учесть оба слоя.

Связь план↔ёмкость теперь единая (рабочие дни). Это разблокирует живое превью в диалоге планирования (PLANNING_DIALOG_UX Q14/Q15) — превью считает тем же раскидом до сохранения.

Остаётся из планирования (фаза 2+, не сейчас): метод «Вручную по месяцам» (WI-30), единица FTE%/ч-нед, план×сотрудник. MVP-ядро (рабочие дни + видимость без endDate) закрыто.

---

**[Аналитик · итерация 98 · 2026-06-21]**
@Dev1 [signal-arch]

wi-02-06-07 принят. Сверка с requirements — соответствует:
- WI-02: DAILY_NORM_HOURS убран из fill-путей, normFor из useDailyNorm SSOT, короткий день/норма-0 учтён → A1.12/A4.8/A1.14 ✓. Оставление константы как fallback внутри useDailyNorm — верно (это не хардкод-fill).
- WI-06: «Заполнить неделю для всех» + «Заполнить будни нормой» → A1.13/D4.6 ✓. Развод двух путей (calcFillRowWeekdays vs fillStandardWeek) корректен.
- WI-07: lockedCells look-up, пустая APPROVED read-only → UC-TS-10/CISO-011 ✓.

**ДВА открытых хвоста (зафиксировал, в бэклог следующей итерации):**
1. **WI-01** — удаление/переименование чипа ⇥ (заливка значения ячейки). Dev1 не трогал по ТЗ (закреплён за WI-01). Ответ-якорь: E1.11 → ячейка = только число/input (C1), чип убрать; D4.6 → «убрать чип». bulkFill(hours) для чипа пока сохранён — корректно до WI-01.
2. **A1.11** — «Очистить неделю» вынос из CopyMenu. Под-scope WI-06, не сделан (P0-ядро было норма+развод). Якорь: A1.11 → вынести деструктив из меню копирования + confirm со счётчиком (D4.11 «Удалить N записей»).

Оба не блокируют — корректно отложены. Прошу arch завести их в доску как WI-01 (чип) и WI-06b («Очистить неделю» вынос).

Напоминание про SSOT рабочих дней: WI-02 теперь использует normFor (произв.календарь). Тот же источник нужен Dev2 для фикса раскида плана (calc-load.ts, calendar→work days). Переиспользовать существующий хелпер, не плодить второй.

---

**[Аналитик · итерация 97 · 2026-06-21]**
@Dev1 [signal-arch]

Подтверждаю кластер wi-02-06-07 — точно ложится на ответы requirements-сессии. Якоря для сверки при реализации:

**WI-02 (норма-SSOT в fill)** ← ответы A1.12, A1.24, B4.3, A4.8.
- `DAILY_NORM_HOURS=8` хардкод в format.ts убрать → `useDailyNorm(date)`.
- ЕДИНЫЙ источник нормы: `normHoursPerDay` (Settings) = `useDailyNorm` = норма для «Заполнить». `fillTemplateHours` сделать алиасом или убрать.
- Короткий предпраздничный день (7ч) и рабочая суббота — из произв.календаря (`hours` дня), не из константы. См. C1.15.

**WI-06 (bulk-fill 2 явных действия)** ← ответы A1.13, D4.6, G2.10.
- Три «заполнить» развести в ДВА однозначных имени (чип убрать): «Заполнить неделю для всех» (тулбар, все строки) / «Заполнить будни этой строки» (RowMenu). Решено в TIMESHEET_CELL_CONTROLS §F5.
- Только пустые будни, без перезаписи (A1.14). Учитывать `isNonWorkingDay` (произв.календарь), не хардкод sat/sun (A1.15).

**WI-07 (copy-week lockedByDay-guard)** ← ответ на QA-флаг #5 (UC-TS-10).
- Явный lockedByDay-guard в `use-timesheet-actions.ts:50-78` для единообразия с bulkFill/8×5. APPROVED-ячейка с пустыми часами теоретически перезаписывалась — закрыть.

Хот-файл `use-timesheet-actions.ts` твой, иду последовательно — ОК, не пересекаюсь (аналитик код не трогает).

Замечание: при WI-02 не забудь — норма дня берётся из произв.календаря как SSOT (C1.14), `isWeekend`→`isNonWorkingDay`. Это та же связка, что нужна для фикса раскида плана (Dev2, calc-load.ts) — общий SSOT рабочих дней. Координируйте с Dev2 чтобы не сделать два разных хелпера рабочих дней.

---

**[Аналитик · итерация 96 · 2026-06-21]**
[signal-arch]

**ВОЛНА-2 ОТВЕЧЕНА ПОЛНОСТЬЮ. `docs/analysis/UI_ANSWERS_2.md` — записан.**

226 ответов: E (59) + F (64) + G (82) + Planning Dialog Q1–Q21 (21, ратифицировано).

**ИТОГО по всем волнам: 651 вопрос отвечён** (359 A·B·C·D + 38 planning-models + 226 волна-2 + 28 cell-controls).

---

**Ключевые решения волны-2:**

E (микро/motion/onboarding/клавиши):
- E1.11 → ячейка = только число/input (C1) — закрывает корневой баг контролов.
- E1.18 → внутренний clipboard виджета (React-стейт), системный в RemDOM нет.
- E2.x → анимации минимальны: только opacity/color/transform, без layout; skeleton без shimmer.
- E4.5 → Shift+Enter ДОДЕЛАТЬ (расхождение cheatsheet↔код, [bug]).
- E4.14 → SSOT-список клавиш → cheatsheet + обработчик из одного массива.

F (плотность/тема/undo/уведомления):
- F1.1 → compact/standard глобально, per-user в `credosTimeSettings`.
- F2.1 → тёмная тема отложена (сначала спрос). F2.7 → SUBMITTED янтарным (синий путается с action).
- F2.12/F2.14 → 4 дискретных ступени heatmap + textFaint до AA (аудит).
- F3.1 → Undo последнего действия + toast «Отменить». F3.9 → НЕ откатывать молча.
- F4.1 → доставка уведомлений = БЛОКЕР (SDK канал не даёт): pull сейчас, email когда появится канал.

G (поиск/массовые/печать/локаль):
- G1.1 → поиск = видимая строка, НЕ Cmd+K (в Worker глобальных слушателей нет).
- G2.x → inline-чекбоксы в «Согласовании», sticky-бар действий, batch через inline-поповер.
- G3.1/G3.9/G3.12 → ВСЯ печать/PDF/XLSX = серверная генерация → URL (подтверждает C4.2).
- G4.1 → дата = date-only локали (не UTC-datetime), убирает риск границы суток.
- G4.5 → запятая в UI, точка в CSV. G4.13 → переключатель Decimal/HH:MM.

Planning Dialog Q1–Q21 — рекомендации аналитика/дизайнера принимаю целиком. Inline-поповер «✎ План», радио Равномерно/Вручную, «С/ПО» через type=date, живое превью по РАБОЧИМ дням, овербукинг показывать не блокировать.

---

**ЗАПРОСЫ К ЗАКАЗЧИКУ (не закрыты решением аналитика — нужен ввод):**
1. **F2.4** — бренд-палитра Credos (hex/гайд) для accent.
2. **G4.18/G4.20** — целевая 1С-конфигурация (десятичный разделитель + формат даты CSV).
3. **F2.1** — нужна ли тёмная тема (определяет объём dual-token).

---

**ПОДТВЕРЖДЕНИЯ (волна-2 совпала с волной-1) — усиливают приоритет:**
- Норма дня SSOT + произв.календарь (G2.11, G4.12) ← A1.12/A1.15.
- Backend-генерация файлов (G3.x) ← C4.2/A3.21.
- Корневой баг контролов C1 (E1.10/E1.11) ← TIMESHEET_CELL_CONTROLS §F4.
- Фикс раскида план→рабочие дни (Q14/Q15) ← PLANNING_MODELS B2.

---

**РАСХОЖДЕНИЯ КОД↔ДОК (в бэклог):**
- Shift+Enter обещан cheatsheet, нет в use-keyboard (E4.5) — [bug].
- Нет SSOT клавиш → дрейф cheatsheet↔код (E4.14).
- Дрейф-цвета вне токенов: validation-toast/reminder-banner (#f3c4ab/#f6da90) (F2.6).
- prefers-reduced-motion в RemDOM недоступен (E2.10).

Все вопросы requirements-сессии закрыты. Жду приоритизацию arch на реализацию (топ: фикс раскида план→рабочие дни + норма SSOT + backend-download).

---

**[Аналитик · итерация 95 · 2026-06-21]**
[signal-arch]

**1. UI_ANSWERS.md — записан**

`docs/analysis/UI_ANSWERS.md` — 359 ответов на блоки A·B·C·D.
Формат: каждый вопрос → рекомендованный вариант + 1–2 строки обоснования + ссылка (Timetta/CISO/RemDOM/NB).

Топ-7 приоритетов, вышедших из ответов:
1. **Backend download** (C4.2 = A3.21) — presigned URL с бэка. Blocker экспорта.
2. **Норма дня SSOT** — `useDailyNorm` везде, `DAILY_NORM_HOURS=8` убрать (A1.12, A4.8, B4.3).
3. **Производственный календарь SSOT** — `isNonWorkingDay` вместо `isWeekend` (A1.15, C1.14).
4. **Отзыв согласования** un-approve (A4.25) + Recall SUBMITTED (A4.4) — нереализованы.
5. **`role="grid"`** на таймшит-сетку (D2.1) — крупнейший a11y-долг.
6. **Unified StateView** (D1.1) — скелетоны/ошибки/empty единообразно.
7. **`revealEmployeeNames` confirm-диалог** при включении (B4.17) — 152-ФЗ.

---

**2. БАГ «раскид по календарным дням» — ЧИНИТЬ НЕМЕДЛЕННО**

Ответ арху на вопрос «чинить сейчас или ждать методов?»: **чинить сейчас, не ждать**.

Обоснование:
- Баг корневой: план и ёмкость считаются по разным правилам (план = calendar days, ёмкость = рабочие часы) → визуально «не бьются» → заказчик не доверяет доске.
- Фикс малый: в `calc-load.ts:plannedHoursInPeriod()` заменить `totalDays = utcDay(endDate) − utcDay(startDate) + 1` на `workHoursBetween(startDate, endDate)` — функция уже есть в `calc-load.ts`.
- Behavior-change для Demand: числа в ячейках изменятся (станут выше в периодах с выходными) — это ПРАВИЛЬНО и ожидаемо.
- Риска data-loss нет: `plannedEffort` в БД не меняется, меняется только отображение раскида.
- Делать вместе с методами — задержит фикс на 2+ волны вопросов (ещё E/F/G + dialog).

**Dev2-зона:** `apps/time/src/front-components/capacity/calc-load.ts` строки 189–205.

---

**3. Проект без `endDate` — план = 0 (НЕВИДИМ на доске)**

Из PLANNING_MODELS.md §1.3 п.2: `if (!plannedEffort || !startDate || !endDate) return 0`.

Решение (E3): **требовать `endDate` при вводе плановых часов** — блокировать PATCH `plannedEffort` если `endDate` не задана; в UI добавить подсказку «Для отображения на доске укажите дату окончания».
Это проще, чем open-ended раскид или front-метод авто-конца.

---

**4. Ответы на 38 вопросов PLANNING_MODELS.md**

**A. Единица плана**
A1 → **часы итого** (привычно для РП). Часы/нед и % — roadmap v2.
A2 → нет FTE/% на MVP. Только часы.
A3 → **да, бюджет-потолок нужен** отдельно от раскида (`budgetRemaining` уже есть, достаточно усилить UI: прогресс-бар план/факт/остаток).
A4 → **только часы** (NB: no-billable-concept).

**B. Метод распределения**
B1 → **равномерно = дефолт**. + ручной по периодам (месяцам) в следующей итерации (C1).
B2 → **рабочие дни** — да, именно (BUG FIX выше).
B3 → **нет** фронт/бэк-загрузки на MVP.
B4 → **нет** «вся ёмкость» — риск автоматического овербукинга.
B5 → **нет** авто-без-овербукинга — конфликт-подсветка достаточна.

**C. Ручной ввод и правка**
C1 → **да, нужна ручная правка** по периодам (месяцам). Ключевое требование заказчика.
C2 → **месяцы** как масштаб ручного ввода. Недели — детально, дни — избыточно.
C3 → **сохранять** введённые значения при сдвиге дат. Как Timetta «ручной метод».
C4 → **нет гибрида** ручное+авто — выбор метода целиком (либо равномерно, либо вручную).
C5 → **нет undo/redo** на MVP для планирования.

**D. Гранулярность**
D1 → **проект × отдел** (как сейчас). Этап — опционально если проект многофазный. Сотрудник — roadmap.
D2 → **нет**, план по сотруднику на MVP. Отдел ÷ headcount достаточно. Per-person — roadmap v2.
D3 → **нет**, этапы без привязки часов к ним на MVP. Этапы = вехи, не единица плана.
D4 → **вручную** — РП задаёт доли по отделам. Авто поровну — дефолт для новых отделов без доли.

**E. Старт и горизонт**
E1 → **от `startDate`** проекта. Прошедшие периоды показывают план (можно скрыть позже через точку прогноза — roadmap).
E2 → **оставлять план** в прошедших периодах (не обнулять) — для ретроспективы откуда взялась нагрузка.
E3 → **требовать `endDate`** — блокировать PATCH без даты окончания (решение выше п.3).
E4 → **настраиваемый горизонт** (8/12/16 недель) из `planningHorizonWeeks` в Settings.

**F. Нерабочее время**
F1 → **да, огибать выходные/праздники** (это и есть BUG FIX в B2).
F2 → **только ёмкость** падает при отпусках/больничных. Plan остаётся, conflict показывается. Timetta-модель.
F3 → **нет**, неполная ставка FTE — только в ёмкости (headcount). В раскид плана не входит.
F4 → **нет** индивидуальных графиков. Производственный календарь = общий SSOT.

**G. План vs Бронь vs Факт**
G1 → **отдельные оси** (как сейчас): план = оценка проекта, бронь = резервирование ресурса, факт = реальность. Timetta: «оценка ≠ резерв ≠ факт».
G2 → **нет авто-перераспределения**. Алерт РП о отставании достаточен, решение — за менеджером.
G3 → **нет прогноза** (факт+остаток) на MVP. Слишком сложно, roadmap v3.
G4 → **те же методы** что у плана (равномерно по рабочим дням). 5 методов брони — roadmap.

**H. Овербукинг, приоритеты, роли**
H1 → **показывать конфликт** (мягко). Не блокировать. Timetta-модель.
H2 → **нет приоритетов** проектов на MVP.
H3 → **РП — SOFT бронь** (пресейл/предварительные); **рук.отдела — подтверждает HARD** (ёмкость потребляется). Timetta-модель.
H4 → **да**, разграничение SOFT (РП создаёт) / HARD (рук.отдела подтверждает) уже в модели. UI-жест «Подтвердить» — A2.13 решено.

**I. Визуализация**
I1 → **тултип-разбивка** по проектам в ячейке (проект А 30ч + проект Б 20ч). `deptProjectLoads` данные уже есть.
I2 → **нет** бейджа метода — шум. Метод виден только при ручном режиме (иначе «равномерно» — дефолт).
I3 → **да**, строка-сверка «Σ = plannedEffort» полезна для доверия. Показывать внизу карточки/тултипе.
I4 → **да**, слои в ячейке: план / SOFT-бронь / HARD-бронь / факт — цветом или штриховкой. Уже частично есть (SOFT пунктир, HARD сплошной).
I5 → **нет** отдельного экрана ресурсного плана. Вкладка «Трудозатраты» в карточке проекта достаточна.

---

**5. Волна-2 (E/F/G) — дождусь файлов, отвечу отдельной итерацией.**
PLANNING_DIALOG_UX.md — жду. Как появится — отвечу.

UI_QUESTIONS_INDEX.md прочитан. Счёт ~650 вопросов, 30 разделов — принято, структура ясна.

---

**[Аналитик · итерация 94 · 2026-06-21]**

**[report] таймшит-ячейки — анализ завершён**

Дополнил архитекторский разбор: `docs/analysis/TIMESHEET_CELL_CONTROLS.md` → добавлен **раздел F** (7 подразделов).

**Референсы Timetta/Kimai/Harvest:**
- Все три: клик = сразу ввод, hover-кнопок в ячейке нет. Подтверждает C1 как индустриальный стандарт.
- Kimai-специфика: preset-панель СНИЗУ при фокусе (не в ячейке) — C3, Remote DOM делает дорогим.
- Timetta: контролы при hover строки (не ячейки), статус pill слева, dot-маркер коммента.

**Синтез 30 ответов заказчика → решения:**
- Клик = сразу ввод (C1 подтверждён)
- Мышь-приоритет → промежуточное «выбрана-с-кнопками» убрать
- Чип нормы: оставить, но показывать при фокусе input (вне ячейки), значение = norma из settings
- Duplicate = со часами (как Kimai, не prefill-только)
- Рабочие субботы: `isWeekend` → `isNonWorkingDay(date, calendar)` — нужен Dev2
- `DAILY_NORM_HOURS` хардкод → `useDailyNorm(date)` везде (SSOT)
- UC-TS-10: явный lockedByDay-guard в copyWeek → Dev1 XS-задача

**Переименование bulk-fill (F5):**
- Тулбар: «Заполнить неделю для всех» (все строки, norma)
- ⋯ строки: «Заполнить будни нормой» (эта строка, norma)
- Чип ⇥ «значение ячейки» → убрать (C1) или явно подписать

**Scope C1:** ~1.5 волны Dev1 + небольшой Dev2 (calendar-source). Нет новых routes/schema.

**@arch:** файл готов к review перед постановкой задач переделки. Требует решения по calendar-source для isNonWorkingDay (сейчас нет).

---

**[Аналитик · итерация 93 · 2026-06-21]**

**Новые сигналы:** QA [10:51] + QA [11:05] (строки 11784-11827) — полный прогон 103 UC по коду + подтверждение bug#3.

**Итог QA-прогона (103 UC): ✅79 · ⚠️18 · ❌1 · 🔲5**

**Статус bug#3 (UC-APR-04 reject comment):**
QA подтвердил: фронт не передаёт comment → rejectComment=null. **НО: в предыдущей сессии уже ИСПРАВЛЕНО Dev1** — RejectButton с inline textarea в approval-bar.tsx + comment протянут через reject()→resolveEntries→callRoute. UC-APR-05 (показ причины) тоже закрыт. Статус: ✅ в uncommitted-дереве Dev1.

**Статус UC-VAL-03 (underfill WARNING):**
QA помечает как ⚠️ «не интегрирован». **Исправлено Dev1 в предыдущей сессии** — SubmitButton с поповером недобора (WARNING, не блок). Статус: ✅ в uncommitted-дереве Dev1.

**Открытые риски из QA (актуальны):**
- ⚠️ **UC-TS-10** — copy-week нет явного lockedByDay-guard для APPROVED пустых ячеек (use-timesheet-actions.ts:50-78). Минор. Dev1.
- ⚠️ **UC-PLN-13** — серверного RBAC на PATCH плана нет, UI-guard есть. Класс CISO-005. Документировать OPEN в security-docs.
- ⚠️ **UC-RPT-10** — UC обещает 40+ полей/6 групп, computeDetail отдаёт 7 колонок. Расхождение описания. Dev2 или обновить UC.
- ⚠️ **UC-TS-07 / CISO-005** — IDOR на delete: identity из params (client-supplied). Известный, on-hold до server-identity.

**Arch-gate по-прежнему ожидает:**
34 uncommitted блока (Dev1:24, Dev2:11). Три ⛔ блокера: CISO-010 (timesheet-t13 isManager guard Dev1+Dev2) + CISO-009 (seed реальные имена Dev2). Dual-patch approval-bar → объединить A(RejectButton)+B(SubmitButton) в финальный файл.

**Команда стоит, arch не ответил на ping итерация 92.**

---

**[Аналитик · итерация 92 · 2026-06-22]**

### [ping-arch ‼️] Gate не открыт — 34 uncommitted блока, риск коллизий

**@arch:** SIGNALS стоит уже много итераций. 34 блока dry-run, команда ждёт.

**Что нужно от arch прямо сейчас:**
1. Применить **diff A** — revealEmployeeNames singleton migration (backfill-project-departments.post-install.ts)
2. Применить **diff B** — canDestroyObjectRecords credosTimeEntries (default-role.ts)
3. Применить **diff C** — booking.company field + constants/universal-identifiers.ts (+2 UUID)
4. **Merge-сессия**: 34 dry-run блока Dev1+Dev2 — app-sync + `yarn twenty dev --once --apply`
5. approval-bar.tsx — **2 патча** (RejectButton + SubmitButton) оба должны войти

**BLOCKER к деплою (не gate, а отдельно):**
- CISO-010 Dev1 ⛔ `timesheet-t13.tsx` — `{isManager && <button withCodes/>}`
- CISO-010 Dev2 ⛔ `tabel-T13` — role-guard на codes=true
- CISO-009 Dev2 ⛔ `seed-company-bookings.mjs` — заменить 3 реальных юрлица на синтетику

Эти 3 пункта Dev1/Dev2 могут закрыть самостоятельно до или параллельно с gate.

---

**[Аналитик · итерация 91 · 2026-06-22]**

### Dev1 [report] UC-APR-05 — ПРИНЯТО ✅ Approval-петля ЗАМКНУТА

Показ `rejectComment` в «Мои периоды»: отклонённые недели → баннер «Причина отклонения: …» (терракот). Цепочка: `ApiEntry.rejectComment` → `fetchEntries` → `summarizeWeeks` → Row. XSS: React `{text}` ✅ (CISO-нота соблюдена). period-status 11/11, dry-run.

Approval loop полон: UC-APR-04 (RejectButton + comment до бэка) + UC-APR-05 (показ причины сотруднику). Паттерн Timetta подтверждён.

**@QA:** ре-верифай UC-APR-04 + UC-APR-05 вместе (один smoke-прогон, можно кодом).

### Картина: 34 uncommitted блока

Dev1 (24): +UC-APR-05 (types.ts · period-status.ts · my-periods.tsx)
            CISO-010 ⛔ timesheet-t13 — всё ещё не закрыт
Dev2 (11): seed-company-bookings ⛔ CISO-009 + 10 apply-ready

BLOCKER к деплою: CISO-010 Dev1⛔ + Dev2 tabel-T13⛔ + seed синтетика⛔
Arch-gate: 34 блока + diff A/B/C

---

**[Аналитик · итерація 90 · 2026-06-22]**

### Dev1 [report] underfill-submit — ПРИНЯТО ✅ UC-VAL-03 ЗАКРЫТ

`SubmitButton` в `approval-bar.tsx` — поповер «Неделя недозаполнена: N из M ч. Всё равно отправить?» при недоборе. WARNING не блок, Remote DOM-safe (useState, без window.confirm). SSOT: `validateWeekUnderfill` (constants/validation.ts). Не дублирует GapsNotice (разные точки: пассивный баннер vs осознанный submit). +10 тестов gaps.test.ts, 1924 passed. dry-run.

### ⚠️ HOT-FILE: approval-bar.tsx — 2 Dev1 агента

`reject-comment-ui` добавил `RejectButton`, `underfill-submit` добавил `SubmitButton` — оба в одном файле `approval-bar.tsx`. Изменения непересекающиеся, но при gate **@arch: применить оба диффа, проверить что RejectButton И SubmitButton присутствуют в финальном файле**. Не автомерж.

### UC-APR-05 — показ rejectComment в my-periods

Хвост UC-APR-05 ещё открыт: `rejectComment` в БД, но нигде не рендерится. Dev1-verify брал этот хвост (iter 89). Ждём [taking]/[report].

### Картина: 33 uncommitted блока

Dev1 (23): +underfill-submit. CISO-010 ⛔ timesheet-t13 всё ещё открыт
Dev2 (11): seed-company-bookings ⛔ CISO-009 + 10 apply-ready

1924 тестов ✅.
BLOCKER: CISO-010 Dev1⛔ + Dev2⛔ + seed синтетика⛔

---

**[Аналитик · итерация 89 · 2026-06-22]**

### Dev1 [verify] bug#3 + [taking] underfill-submit — ПРИНЯТО

**[verify] bug#3:** подтверждение параллельного Dev1 — фикс уже в дереве, коллизии нет. bug#3 ЗАКРЫТ.

**[taking] underfill-submit (UC-VAL-03):** мягкое подтверждение-поповер при Submit с недобором недели. WARNING не блок, GapsNotice не дублирует, Remote DOM-safe (паттерн RejectButton). Принято, зона grid/.

### UC-APR-05 — ОТКРЫТЫЙ хвост, @Dev1-verify берёт

`rejectComment` пишется в БД, но НИГДЕ не рендерится. Нужен показ на отклонённых неделях в `my-time/my-periods`. Dev1-verify уже проверил зону → **берёт отображение**, Dev1-reject-comment-ui свою зону закончил. Антидубль: до [taking] UC-APR-05 другим Dev1 не стартовать.

**XSS-нота (CISO):** при рендере `rejectComment` — только React `{text}`, НЕ `dangerouslySetInnerHTML`. Поле пользовательское.

### Картина: 32 uncommitted блока (+0 кода от [verify])

Dev1 (22+): underfill-submit in-progress (новый блок появится по [report])
Dev2 (11): seed-company-bookings ⛔ CISO-009 + 10 apply-ready

1917 тестов ✅.

BLOCKER: CISO-010 Dev1 timesheet-t13⛔ + Dev2 tabel-T13⛔ + seed синтетика⛔

---

**[Аналитик · итерация 88 · 2026-06-22]**

### Dev1 [report] reject-comment-ui — ПРИНЯТО ✅ [bug]#3 UC-APR-04 ЗАКРЫТ

RejectButton с inline-поповером textarea «Причина отклонения (обязательно)». Remote-DOM safe (useState, без window.prompt). Disabled при пустом тексте. Цепочка: `approval-bar.tsx` → `use-approval.reject(comment)` → `approval-rest.resolveEntries(ids,false,comment)` → `/s/approval {op:'reject',ids,comment}`. Fallback PATCH тоже пишет rejectComment. UC-APR-05 (сотрудник видит причину) теперь достижим. +4 теста approval-rest, 1917 passed, 0 failed. dry-run.

Сверка Timetta ✅: отклонение = обязательный комментарий.

### Картина: 32 uncommitted блока

Dev1 (22): +reject-comment-ui. CISO-010 ⛔ timesheet-t13 всё ещё не закрыт
Dev2 (11): seed-company-bookings ⛔ CISO-009 (реальные юрлица) + 10 apply-ready

1917 тестов ✅ (+3 от iter 87)

BLOCKER к деплою: CISO-010 Dev1⛔ + Dev2 tabel-T13⛔ + seed-company-bookings синтетика⛔
Arch-gate: diff A · diff B · C(booking-company+constants) + 32 блока

---

**[Аналитик · итерация 87 · 2026-06-22]**

### QA [report] uc-runthrough 103 UC — ПРИНЯТО ✅ Итог: ✅79 · ⚠️18 · ❌1 · 🔲5

Полный прогон USE_CASES_LIBRARY.md по коду. Статус-колонка проставлена, сводная матрица обновлена.

**❌ [bug]#3 P2 UC-APR-04 rejectComment — @Dev1 FIX НУЖЕН**
`approval-bar.tsx:138` → `onReject()` без комментария → `use-approval.ts:108` reject() → `approval.logic.ts:153` comment=null всегда. Бэк готов принять. Фикс Dev1:
1. `approval-bar.tsx` — добавить inline textarea при нажатии «Отклонить» (не prompt)
2. `use-approval.ts` reject(comment: string) → протянуть в resolveEntries
3. `approval-rest.ts` resolveEntries — добавить comment в payload `{op:'reject', ids, comment}`
Бэк-контракт не менять. UC-APR-05 (сотрудник видит причину) недостижим без этого фикса.

**⚠️ Открытые риски (новые, для arch):**
- UC-PLN-13: plan PATCH без server isManager guard → обход через DevTools. Тот же класс CISO-005. Документируем как OPEN до CISO-005/RBAC.
- UC-VAL-03: `validateWeekUnderfill` не подключена к UI (Dev1, minor, WARNING не показывается).
- UC-TS-10: copy-week — косвенная защита APPROVED через filled Set, без явного lockedByDay-guard. Риск: согласованная пустая ячейка перезапишется (Dev1, minor).
- UC-RPT-10: UC обещает «40+ полей/6 групп», computeDetail = 7 колонок. Обновить UC-описание OR расширить detail (Dev2). Не баг, расхождение ожиданий.
- role-guard.test.ts: import ошибка (нет src/default-role) — pre-existing с 1ab2956. Dev2/arch после деплоя фазы 1.

**✅ Подтверждены 6 топ-критичных UC:** lock-APPROVED · SoD · ПДн-КОД · add-row при фильтрах · CSV · booking-овербукинг.

**Dev1 [report] missing-report** — canonical missing-view.tsx в reports-dashboard (1879 тестов). Уже учтён через дедуп-сигнал итер. 81.

### Картина: 31 uncommitted + 1 баг к фиксу

Dev1 (21): bug#3 rejectComment ⚠️ (approval-bar/use-approval/approval-rest — fix нужен до gate)
Dev2 (11): seed-company-bookings ⛔ CISO-009 + 10 apply-ready

BLOCKER: CISO-010 Dev1⛔ + Dev2⛔ + seed-company-bookings синтетика⛔ + **bug#3 P2 rejectComment Dev1**

---

**[Аналитик · итерация 86 · 2026-06-22]**

### Dev1 [report] booking-confirm-btn — ПРИНЯТО, кнопка не нужна ✅

Правильное решение: исследование ДО кода, нашёл нативную точку подтверждения.

**Вывод для команды:** SOFT→HARD = нативный SELECT bookingType в карточке брони (ядро Twenty делает PATCH само). Capacity-доска = агрегатные маркеры без per-booking id → кастомная кнопка архитектурно невозможна в worker-компоненте. Совпадает с Timetta §2.2/§40.

**[signal-arch] → @arch:** MVP = нативный SELECT достаточен. Если хотим UX-ускорение (скрытая кнопка «Подтвердить» только для SOFT) → record-action в views/page-layouts (зона Dev2/arch, НЕ front-components). Рекомендую пропустить в MVP.

Кода не добавлено → Dev1 блоков без изменений (21).

### Картина: 31 uncommitted блок, тихо

Dev1 (21): ждёт gate, CISO-010 ⛔ timesheet-t13
Dev2 (11): seed-company-bookings ⛔ CISO-009 + 10 apply-ready

1914 тестов ✅. Arch решает: booking-confirm-btn MVP=native или record-action.

---

**[Аналитик · итерация 85 · 2026-06-22]**

### Dev1 [report] projects-report-ui — ПРИНЯТО ✅ (+1 блок, быстрый консьюм)

Сегмент «Проекты» в reports-dashboard. 4 файла: report-types · projects-plan-fact-rest · use-projects-plan-fact · projects-view. Таблица 7 колонок, overrun-подсветка (терракот тинт + бейдж ▲), pill-фильтр статуса из SSOT, KPI-полоска из totals. Состояния: skeleton/error/empty — полный набор. 1914 тестов, 0 failed. dry-run, lint 0.

Dev1 взял Dev2-контракт из [report] projects-plan-fact и закрыл UI за 15 мин — хорошая координация.

⚠️ Timestamp 2026-06-21 на обоих сигналах (вероятно, опечатка — сигналы на вершине стека, т.е. новейшие). Принято как 2026-06-22.

### Картина: 31 uncommitted блок

Dev1 (21): +projects-report-ui · (ждёт gate, CISO-010 ⛔)
Dev2 (11): seed-company-bookings⛔CISO-009 + 10 apply-ready

1914 тестов ✅. 

Arch-gate: diff A · diff B · C(booking-company+constants)
BLOCKER: CISO-010 Dev1 timesheet-t13 ⛔ · Dev2 tabel-T13 ⛔ · seed-company-bookings синтетика ⛔

---

**[Аналитик · итерация 84 · 2026-06-22]**

### Dev2 [report] projects-plan-fact — ПРИНЯТО ✅

REPORTS_COMPLETENESS P1. `computeProjectsPlanFact` — план/факт/остаток в часах (no-billable ✅). Факт из credosTimeEntry Σ, НЕ из factHours — верно (factHours накопительный, период-фильтр иначе врёт). overrun-флаг строгий (fact>planned). 1914 тестов, 0 failed, lint 0.

Расширение `/s/reports` (groupBy=projects-plan-fact), не новый роут — правильно. dry-run: только `updated logicFunction`. apply-ready.

**Контракт Dev1 для UI:** `POST /s/reports { from?, to?, groupBy:'projects-plan-fact', status?, departmentId? }` → `{ rows:[{projectId, name, code, status, planned, fact, remaining, overrun, pct}], totals }`. Сортировка: overrun↑ → fact↓ → name. Красная подсветка overrun=true.

### seed-company-bookings ⛔ ещё не исправлен

Tail файла без изменений — реальные юрлица всё ещё в коде. @Dev2: fix приоритетен перед следующим [taking].

### Картина: 30 uncommitted блоков

Dev1 (20): ждёт arch-gate, CISO-010 ⛔ timesheet-t13
Dev2 (11): fields-p1-additive · booking-company-field · seed-overload(live) · seed-company-bookings⛔CISO-009 · **projects-plan-fact** (new) + 6 прежних

1914 тестов ✅. Arch-gate: diff A · diff B · C(booking-company+constants)
BLOCKER: CISO-010 Dev1⛔ + Dev2 tabel-T13⛔ + seed-company-bookings синтетика⛔

---

**[Аналитик · итерация 83 · 2026-06-22]**

### CISO [ciso-block] seed-company-bookings — ПРИНЯТО, Dev2 → FIX

⛔ CISO подтвердил: ТехноНиколь/Геберит Рус/Евротэк-Югра = реальные юрлица, CISO-009, НЕ коммитить.

**@Dev2:** замени в `seed-company-bookings.mjs` до commit (аналитик и CISO совпали):
- ТехноНиколь → ООО «РесурсТех»
- Геберит Рус → ООО «СтройЕвро»
- Евротэк-Югра → ООО «ПромЭнерго»

После замены → [report] + тег `ciso-009-fix` → CISO разблокирует автоматически.

### Картина: 29 uncommitted блоков, 1 блок под fix

Dev1 (20): ждёт arch-gate
Dev2 (10): 9 apply-ready + **seed-company-bookings ⛔ CISO-009 (синтетика-fix нужна)**

Arch-gate ожидает: diff A · diff B · booking-company-field+constants
BLOCKER: CISO-010 Dev1 ⛔ + Dev2 ⛔

---

**[Аналитик · итерация 82 · 2026-06-22]**

### Dev2 [report] overload-booking-company — ПРИНЯТО ✅ (+2 блока)

**Задача A (перегруз ОВ):** seed-overload.mjs прогнан live — ОВ util 119/130/121% (красный), ОПР 40% (контраст недогруза). Реалистичный микс достигнут. deptPlans +7, HARD-брони +6. headcount из FTE, не из ручного поля — верно.

**Задача B (booking под компанию):** поле `company` ADDITIVE (RELATION→Company, nullable, onDelete SET_NULL). Обратная коллекция company.credosTimeBookings. 2 новых UUID в constants. dry-run: 1 created fieldMetadata, 0 deleted — apply-ready. seed-company-bookings.mjs корректно делает exit 3 до деплоя поля (arch-gate).

1888 тестов, 0 failed ✅.

### ⚠️ CISO-009 — seed-company-bookings: реальные названия клиентов

`seed-company-bookings.mjs` использует: **ТехноНиколь / Геберит Рус / Евротэк-Югра** — реальные юрлица. CISO-009 = реальные клиенты/заказчики NOT в git. CISO-007 approved был только для SEED_DIVERSE (синтетика). **Этот seed НЕ проверен CISO.** → @CISO: требуется review до apply/commit. Либо Dev2 заменяет на синтетику (ООО «РесурсТех» / ООО «СтройЕвро» / ООО «ПромЭнерго»).

### ⚠️ HOT FILE: constants/universal-identifiers.ts

Dev2 добавил 2 UUID (CREDOS_TIME_BOOKING_COMPANY_FIELD_ID, CREDOS_TIME_COMPANY_BOOKINGS_FIELD_ID). Этот файл = HOT-зона arch (3× collision история). **Dev2 НЕ коммитил** — правильно. @arch: включить в свой коммит вместе с полем booking.company.

### Картина: 29 uncommitted блоков (Dev2 +2)

Dev1 (20): без изменений (ждёт gate)
Dev2 (10): fields-p1-additive · booking-company-field · seed-overload(live) · seed-company-bookings(not-live) + 6 прежних

Arch нужен: A=revealEmployeeNames · B=canDestroyObjectRecords · C=booking-company-field+constants(новое)

BLOCKER: CISO-010 Dev1 timesheet-t13 ⛔ + Dev2 tabel-T13 ⛔

---

**[Аналитик · итерация 81 · 2026-06-22]**

### Dev1 [дедуп ‼️] #4 + [report] #5 «Команда» — ПРИНЯТО ✅

**Дедуп #4:** коллизия swarm разрешена правильно — standalone (7 untracked) удалён, canonical = reports-dashboard версия. ПОПРАВКА к картине: missing-timesheets #4 = dashboard-интеграция, не standalone. Nav-пункт не плодился — хорошо.

**#5 project-team ✅:** клиент-агрегация (limit:500) → `/s/project-team` (PII-redaction + cursor-пагинация). Компонент не тронут, 6/6 тестов, oxlint 0/0. dry-run.

**«Проекты сотрудника» (mode=employee-projects):** Dev1 НЕ берёт — ждёт [taking] перед стартом. Правильная осторожность.

### ⚠️ Dev1 рекомендует ПАУЗУ — поддерживаю

Причины: много dry-run накопилось (T2 · UX-доска · R3 · REQ-0019 · Т-13 · #5), swarm плотный, риск коллизий растёт (#4 уже случился). **Рекомендую arch-gate немедленно.**

### Картина: 28 uncommitted блоков (уточнение)

Dev1 (19→20): R1-токены · REQ-0019-UI · REQ-0016 · REQ-0018 · OLAP-drill · Resource-gap
              ...
              dept-view-head · kpi-scope · timesheet-t13⛔CISO-010 · card-tables
              missing-timesheets(dashboard) · **project-team** (new)

Dev2 (8): без изменений

Дифф у arch (ожидание, ‼️): A=revealEmployeeNames · B=canDestroyObjectRecords

BLOCKER: CISO-010 Dev1 timesheet-t13 ⛔ + Dev2 tabel-T13 ⛔ — до деплоя

**→ @arch: gate нужен, Dev1 сам просит паузу**
...
### Dev2 tabel-T13 (REQ-0006 п.4) ✅ — не закоммичено

Чистая реализация. Отдельный модуль `reports-timesheet-grid.ts` — без конфликтов с reports-calc.ts. Переиспользование CSV_BOM/CSV_DELIMITER/toCsvRow — правильно, без дублирования. 18/18 тестов. ✅

**ПДн / CISO-007:** `revealEmployeeNames` читается из настроек (дефолт false) → при false в первой колонке employeeId, не ФИО. Соответствует политике. ✅

**no-billable-concept:** только часы, без billable/оплаты. ✅

**Коды Т-13 (`codes=true`):** маппинг VACATION→ОТ, SICK→Б, UNPAID→ДО, BUSINESS_TRIP→К, OTHER→НН — правильный для 1С:ЗУП. Нюанс: `Б` (SICK) — мед.ПДн. При выгрузке с кодами действует CISO-010 (будущий CSV-экспорт со SICK). Пока dev — окей; при проде нужен role-guard на `codes=true`.

### ⛔ BLOCKER перед arch gate: 4 failing теста Dev1

```
src/front-components/grid/time-rest.test.ts — 4 fail
```

Dev2 идентифицировал: `time-rest.ts` изменён (CISO-012 L2 — роутинг через `/s/time-entry`), тест не обновлён. **Dev1: это твоя зона, нужен фикс ДО arch gate.** Arch gate с failing тестами = НЕЛЬЗЯ.

### Текущее состояние: 11 uncommitted блоков

```
Dev1 (8): R1-токены · REQ-0019-UI · REQ-0016 · REQ-0018 · OLAP-drill · Resource-gap · W6-2/CISO-012 · CISO-012-L2(time-rest.ts, тест сломан)
Dev2 (3): REQ-0019-consumers · валидация-правила · booking-REQ0004 · tabel-T13
```

*(tabel-T13 = 4-й блок Dev2)*

**Порядок коммитов без изменений** (1→10), tabel-T13 добавляется после #9 как #11 (независим от booking, зависит только от базовых logic-functions).

**arch: 11 блоков готовы, 1 blocker (time-rest.test.ts). Dev1 фиксит тест → можно gate.**

— Аналитик

**[Аналитик · итерация 59 · 2026-06-21]**

### Dev2 ciso012-server — критическое SDK-ограничение ✅

**SDK ВЕТО: НЕТ.** Twenty database-event = after-mutation only. `throw` в хендлере не откатывает мутацию — она уже состоялась. Подтверждено: (1) офиц.доки (types: httpRoute/cron/databaseEvent/serverWebhook, все кроме httpRoute реактивные), (2) `wrapEvent` в project-fact-rollup-events.ts глотает ошибки — потому что throw как вето бесполезен.

**Revert-к-снапшоту отвергнут правильно** (keep-it-simple): окно гонки с 1С, хрупкий delete-revert, ложные срабатывания на approval-флоу, дублирование RLS. ✅

**Архитектурный факт для ADR:** database-event нельзя использовать как enforcement-слой. Только как реактивный after-хук. Фиксируем.

**Три слоя CISO-012 (обновлено в 04-security.md):**
- L1 ✅ UI-lock Dev1 (isCellLocked/notifyLocked) — уже реализован
- L2 📋 **NEW action Dev1**: роутить `grid/time-rest.ts` через `/s/time-entry` → существующий `cannot_modify_approved` гард срабатывает автоматически. Дёшево.
- L3 🔮 RLS/fieldPermissions — только после CISO-005 (server-actor)

→ **Dev1: L2 — небольшая задача. Стоит взять как CISO-012-L2 до arch gate.**

### SDK-ограничение → предупреждение команде

> ⚠️ `database-event` ≠ before-hook. Нельзя строить валидацию/enforcement через throw в event-хендлерах. Любой серверный enforcement идёт только через `/s/...` httpRoute-обработчики (logic functions).

Рекомендую зафиксировать в `docs/developer/03-adding-feature.md` или ADR как SDK-pitfall.

— Аналитик

**[Аналитик · итерация 58 · 2026-06-21]**

### W6-2 + CISO-012 (Dev 1) ✅ — не закоммичено

**Ключевое:** данные уже были (`ApiEntry.status`, `lockedByDay`), не хватало UI commit-гарда + a11y. Сейчас закрыто на всех слоях кроме DB-event (остаток = RBAC-волна, корректно отложен).

**`isCellLocked()` как SSOT** — правильный паттерн: один предикат → одно место. ✅

**Баг project-view** — `lockedByDay` не передавался в GridRow в режиме «Проект». Теперь пробросили. Это был реальный функциональный баг, хорошо что поймали. ✅

**Dev2-хвост** (Dev1 сделал): `maxHoursPerDay` добавлен в `GlobalSettings` тип + маппинг в `fetchGlobalSettings`. `use-validation` берёт лимит из настроек. Это нормальная синхронизация после uncommitted Dev2-работы по валидации.

**CISO-012 новая запись** — добавлю в `docs/developer/04-security.md` после arch gate:

```
| CISO-012 | P2 | MITIGATING | APPROVED-ячейки редактируемы через UI (Core REST мимо server-guard) |
|          |    |            | Dev1: isCellLocked() + notifyLocked() в grid. DB-RLS — RBAC-волна. |
```

**1720 passed, 0 failed** (+22). Lint 0/0. Dry-run чисто. ✅

### ⚠️ 10 uncommitted блоков — новый максимум

```
Dev1 (7): R1-токены · REQ-0019-UI · REQ-0016 · REQ-0018 · OLAP-drill · Resource-gap · W6-2/CISO-012
Dev2 (3): REQ-0019-consumers · валидация-правила · booking-REQ0004
```

`settings/types.ts` и `settings-rest.ts` затронуты как Dev1 (W6-2), так и REQ-0019-UI (Dev1). Но это ОДИН разработчик (Dev1), нет коллизии.

**Порядок коммитов (финальный, 10 блоков):**
```
1. [Dev1] R1-токены
2. [Dev1] REQ-0019-UI  ← GlobalSettings type (12 полей включая overtimeWarnHours)
3. [Dev2] REQ-0019-consumers
4. [Dev2] валидация-правила  ← после 2: +3 поля в settings, maxHoursPerDay тип
5. [Dev1] W6-2/CISO-012  ← settings-rest.ts читает maxHoursPerDay (зависит от 4 для полноты, но технически работает с fallback)
6. [Dev1] REQ-0016
7. [Dev1] REQ-0018-карточка
8. [Dev1] OLAP-drill
9. [Dev1] Resource-gap
10. [Dev2] booking-REQ0004  ← независим от всего кроме базовых объектов
```

**arch: 10 блоков, 1698–1720 тестов, всё чисто. Когда gate?**

— Аналитик

**[Аналитик · итерация 57 · 2026-06-21]**

### booking-REQ0004 (Dev 2) ✅ — не закоммичено

**Архитектура правильная:**
- `credosTimeBooking` отдельная сущность, не наследует от planAllocation. Инвариант «бронь ≠ оценка ≠ факт» соблюдён — три оси без FK. Соответствует Timetta booking-concept. ✅
- `bookingType` вместо `type` (зарезервировано ядром) — хороший catch.
- SOFT = пресейл, не потребляет ёмкость. HARD = твёрдый Demand. Ovebooking не блокируется — флаг конфликта. ✅
- `tentativeBookingEnabled` из Dev1 (REQ-0019-UI) = toggle SOFT-брони. Связка задокументирована.

**1698 passed, 0 failed.** +39 от 1659. Dry-run: 15 created, чисто. ✅

**Контракт Dev1 (принят, детали в REQ-0004 Часть C):**
- `hours` раскидываются равномерно по workdays [start..end] — та же функция что план
- HARD → твёрдый Demand в `LoadCell.load`, SOFT → полупрозрачный слой
- Режим-переключатель доски: «план vs hard-брони»
- Конфликт = HARD > capacity → подсветка (не блок)

**Follow-up Dev1 (capacity board):** интеграция HARD-Demand, вкладки «Брони» в карточках employee/project, soft-фильтр.

### ⚠️⚠️⚠️ `universal-identifiers.ts` — 4 параллельных набора UUID

| PR | Dev | UUID |
|----|-----|------|
| REQ-0016 (uncommitted) | Dev1 | +21 |
| REQ-0018-карточка (uncommitted) | Dev1 | +7 |
| валидация-правила (uncommitted) | Dev2 | +3 |
| booking-REQ0004 (uncommitted) | Dev2 | +14 |

Итого: **+45 UUID в одном файле**, ни один не закоммичен. При последовательном merge — 4 ручных правки одного файла. Плюс `select-options.ts` параллельно правили Dev1 (REQ-0019-UI) и Dev2 (booking).

**arch: это должно остановиться.** Предлагаю merge-сессию вместо обычного gate:
1. Запустить dev с текущим деревом: `yarn twenty dev --once --dry-run` от базы
2. Применить блоки в порядке из ит.55, разрешая конфликты в `universal-identifiers.ts` / `select-options.ts` руками
3. После commit #2 (REQ-0019-UI) — Dev2 делает rebase своих uncommitted поверх

### 9 uncommitted блоков — итоговая таблица

| Блок | Dev | Статус | Касается hot-файлов |
|------|-----|--------|---------------------|
| R1-токены | Dev1 | ✅ uncommitted | shared/tokens.ts |
| REQ-0019-UI | Dev1 | ✅ uncommitted | credosTimeSettings, universal-ids, select-options |
| REQ-0019-consumers | Dev2 | ✅ uncommitted | shared/, grid/, capacity/ |
| REQ-0016 связанность | Dev1 | ✅ uncommitted | universal-ids (+21) |
| REQ-0018-карточка | Dev1 | ✅ uncommitted | universal-ids (+7) |
| OLAP drill-down | Dev1 | ✅ uncommitted | reports/, shared/use-drill |
| Resource-gap | Dev1 | ✅ uncommitted | capacity/ |
| валидация-правила | Dev2 | ✅ uncommitted | universal-ids (+3), credosTimeSettings |
| booking-REQ0004 | Dev2 | ✅ uncommitted | universal-ids (+14), select-options |

### gap-аудит v3 #1 — всё ещё неизвестен

Dev2, это последний неизвестный пункт. Поделись — аналитик занесёт в картину и проверит пересечения с накопленными изменениями.

— Аналитик

**[Аналитик · итерация 56 · 2026-06-21]**

### Dev 2 [taking] booking-REQ-0004 — gap-аудит v3 #2

**Архитектурно верно:** `credosTimeBooking` ≠ `credosTimePlanAllocation`. Timetta строго разделяет оценку и резерв (research/timetta/docs/resources-concepts-booking-concept.md: «бронирование — отдельная сущность, не план»). Это правильный инвариант.

**Карта gap-аудита v3 (теперь видна):**
- **#1** = ? — по-прежнему неизвестен. Dev2: открой список, аналитик занесёт в картину.
- **#2** = booking/REQ-0004 (Dev2, 🔄 in progress)
- **#3** = resource-gap (Dev1, ✅ uncommitted)
- **#4** = validation-rules (Dev2, ✅ uncommitted)

**Контракт Dev1 (зафиксировать сейчас, чтобы не переделывать):**

Capacity board сейчас: `Demand = load = credosTimePlanAllocation`. После booking: `Demand = bookings (soft+hard)`. Два разных источника. Dev2, при проектировании `credosTimeBooking` — предусмотри endpoint или поле для агрегации `bookingHours(employeeId, deptId, month)` аналогично `plannedEffort`. Это нужно Dev1 для `use-capacity.ts` → `LoadCell.load`.

Пока capacity board НЕ переключать — это отдельная задача после commit booking. Документировать как `DP-booking-demand-integration`.

**⚠️ `universal-identifiers.ts` — третье параллельное добавление:**
- REQ-0016 (Dev1, uncommitted): UUID карточек
- валидация-правила (Dev2, uncommitted): UUID полей настроек
- booking-REQ-0004 (Dev2, in-progress): UUID нового объекта

При merge три разных блока UUID в одном файле. **arch: merge-стратегия нужна.** Предлагаю: каждый Dev добавляет свои UUID строго в отдельную секцию с комментарием-пометкой реквизита → меньше конфликтов.

**Накопление uncommitted — критический уровень:**
```
Dev1 uncommitted (6): R1-токены · REQ-0019-UI · REQ-0016 · REQ-0018 · OLAP · Resource-gap
Dev2 uncommitted (2): REQ-0019-consumers · валидация-правила
Dev2 in-progress (1): booking-REQ-0004
```

Пока arch не даст gate на первые 8 — merge-нагрузка на `universal-identifiers.ts` и `credosTimeSettings` будет только расти. **Стоп-рекомендация: Dev2, после commit booking НЕ брать #1 до gate от arch.**

— Аналитик

**[Аналитик · итерация 55 · 2026-06-21]**

### валидация-правила (Dev 2) ✅ — не закоммичено

**Архитектура правильная:**
- `constants/validation.ts` — SSOT уровней, кодов, чистые функции. Тестируемо независимо.
- ERROR блокирует, WARNING пропускает с флагом `warnings:[]` в ответе. Соответствует Timetta (ит.46).
- `readValidationThresholds()` с fallback на `VALIDATION_DEFAULTS` — защита от пустой/недоступной БД.
- Back-compat: `error:'hours out of range'` сохранён.

**Коллизия полей `credosTimeSettings` — анализ:**

| Поле | Кто | Тип | Дефолт |
|------|-----|-----|--------|
| `overtimeWarnHours` | Dev1 (REQ-0019-UI) | NUMBER | 12 |
| `warnOnScheduleDeviation` | Dev2 (валидация) | BOOLEAN | true |
| `maxHoursPerDay` | Dev2 (валидация) | NUMBER | 24 |
| `minHoursPerWeek` | Dev2 (валидация) | NUMBER | 0 |

Имена НЕ пересекаются. Но **смысловая граница размыта**: `overtimeWarnHours` (порог переработки, Dev1) + `warnOnScheduleDeviation` (включить/выключить предупреждение, Dev2) — оба про одно. **Нужно решение arch:**
- Оставить два раздельных поля (включатель + порог) — чище для UI
- Или: если `warnOnScheduleDeviation=false` → Dev2 ставит порог в 0 (уже реализовано так); `overtimeWarnHours` тогда = источник порога

Предлагаю: `warnOnScheduleDeviation` = toggle в UI (видим), `overtimeWarnHours` = порог (виден рядом, enabled только если toggle=true). **Dev2:** подтверди, что `readValidationThresholds()` читает `overtimeWarnHours` из singleton (Dev1-поле), а не использует внутренний дефолт 12 независимо.

**`universal-identifiers.ts` — двойное добавление:**
Dev1 (REQ-0016) + Dev2 (валидация) оба добавили UUID. При merge конфликт строк гарантирован. Решение: при коммите второго — ручная правка константного файла (добавить UUID второго в уже обновлённый файл первого). **arch:** назначить порядок.

**1659 passed, 0 failed** (+55 от 1604). QA-баг исправлен (тест «лимит 24» скорректирован). ✅

**Follow-up → Dev 1:** ответ `time-entry-api` теперь может содержать `warnings:[{level:'warning',code,message}]`. В `grid/` — показывать? Предлагаю: иконка ⚠️ в ячейке таймшита при Warning, без блока ввода. UX: пользователь вводит 14ч → желтая ⚠️ + тултип «Переработка: более 12 ч за день». Это часть REQ-0015 UX ввода.

**`validateWeekUnderfill` follow-up:** функция готова, в API не вызвана (нужен недельный агрегат при submit периода). Занести в BACKLOG как `VAL-week-submit`.

### Итоговый uncommitted — 8 блоков ⚠️⚠️⚠️

```
Dev1: R1-токены · REQ-0019-UI · REQ-0016 · REQ-0018-карточка · OLAP-drill · Resource-gap
Dev2: REQ-0019-consumers · валидация-правила
```

Порядок коммитов (финальный, учёт пересечений):
```
1. [Dev1] R1-токены           ← shared/tokens.ts SSOT первым
2. [Dev1] REQ-0019-UI         ← credosTimeSettings (12 полей) + universal-identifiers
3. [Dev2] REQ-0019-consumers  ← зависит от 2
4. [Dev2] валидация-правила   ← добавляет +3 поля к уже задеплоенному объекту;
                                  merge universal-identifiers после 2
5. [Dev1] REQ-0016
...
### 2026-06-21 — [taking] warnings-UI
Показ валидации (warn/error) при вводе + порог переработки из настроек (hour-cell) + баннер напоминаний «Мои часы». Зона grid/ + my-time/. dry-run, не коммит.

### 2026-06-21 — [report] warnings-UI — валидация при вводе + порог переработки + баннер напоминаний

**КЛЮЧЕВОЕ НАБЛЮДЕНИЕ (важно для Dev2):** фронт-CRUD на dev идёт НАПРЯМУЮ по Core REST (`grid/time-rest.ts` → `c.post/patch /rest/credosTimeEntries`), а НЕ через `/s/time-entry`. Поэтому структурный ответ upsert (`validation`/`warnings`) до фронта НЕ доходит. Решение: валидирую КЛИЕНТСКИ той же чистой `validateEntry` (SSOT — `constants/validation`), пороги из `useGlobalSettings`. Семантика 1:1 с бэком. Если в проде CRUD переведут на `/s/time-entry` — `useValidation` легко переключить на чтение `resp.validation/resp.warnings`.

**1. Валидация при сохранении (ERROR блок / WARNING не блок):**
- `grid/use-validation.ts` (new, 79) — хук: `checkAndNotify(hours) → {blocked}`. ERROR (лимит часов/день, дефолт 24 — поля `maxHoursPerDay` нет в клиентских GlobalSettings, оно бэкенд-only) блокирует запись; WARNING (переработка > `overtimeWarnHours` из настроек) не блокирует. Стек уведомлений с авто-гашением WARNING (6с), ERROR держится до закрытия.
- `grid/validation-toast.tsx` (new, 112) — плашки внизу-справа виджета. ERROR красный (`T.over`/`overSoft`), WARNING янтарь (`T.warnSolid`/`warnTint`). aria-live=assertive/polite, ведущая иконка, полная рамка + фон-тинт (без side-stripe), закрываемые. Не модалка, не блокирует вид.
- `grid/weekly-grid.tsx` — обёртки `commitCell`/`bulkFill`: проверка → блок при ERROR, иначе пишем. Подключено во все 3 режима (Неделя/День/Проект). Контейнер `position:relative` под якорь плашек.

**2. hour-cell порог переработки из настроек (хвост Dev2 закрыт):**
- `grid/hour-cell.tsx`: `value > 12` → `isOvertime(value, overtimeThreshold)` (helper из `grid/format.ts`, fallback `OVERTIME_WARN_HOURS_DEFAULT`). Порог тянется из `useGlobalSettings().overtimeWarnHours` через `weekly-grid → week/day/project-view → grid-row/day-row → hour-cell`. day-row тоже подсвечивает переработку (`T.warn`) + title-подсказка.

**3. Баннер напоминаний «Мои часы» (поверх /s/reminders):**
- `my-time/reminders-rest.ts` (new, 50) — POST `/s/reminders` `{mode:'missing-timesheets'}` (как `/s/reports`). `/s/`-роуты с фронта вызываются (см. `reports-rest.ts`), доступны.
- `my-time/use-reminders.ts` (new, 52) — `mine` (моя строка недобора по employeeId) + `team` (все недозаполнившие). `enabled=false` → баннер скрыт. Ошибка → мягкая деградация (баннер не появляется).
- `my-time/reminder-banner.tsx` (new, 142) — личный баннер «Заполните неделю DD.MM–DD.MM: N из M ч (не хватает K ч)» + CTA «Внесите на странице Записи». Руководителю (`isManager`) — сворачиваемый дайджест команды; ФИО показываю ТОЛЬКО при reveal (бэк сам пустит `name=''` при `revealEmployeeNames=false`, тогда «Сотрудник · КОД-отдела» — CISO-007 соблюдён). Янтарь, мягкий, не алёрт.
- `my-time/my-hours.tsx` — баннер над KPI, всегда про ТЕКУЩУЮ неделю (не зависит от выбранного периода KPI — Timetta-семантика периода).

**СВЕРКА (правило 8):** Timetta timesheet-validation-rules = 2 уровня (Ошибка блокирует отправку, Предупреждение уведомляет/позволяет) — зеркалю точно. Напоминания = недобор норма−факт за неделю, система напоминает заполнить — баннер + дайджест.

**ВАЛИДАЦИЯ:** `yarn lint` = **0/0** (321 файл). `yarn test:unit` = **1698 passed, 0 failed** (15 todo). `yarn twenty dev --once --dry-run` = **чисто, typecheck OK** (4 updated frontComponent — пересборка моих бандлов; credosTimeBooking/«Брони ресурсов» — не моё, Dev2-доска уже в дереве). НЕ dev --once. НЕ коммитил.

**ВОПРОС arch/Dev2:** клиентский ERROR-лимит использует дефолт 24 (поля `maxHoursPerDay` нет в клиентском `GlobalSettings`/settings-rest). Если хотите настраиваемый лимит на фронте — Dev2 добавить `maxHoursPerDay` в `settings/types.ts:GlobalSettings` + `fetchGlobalSettings`, тогда подхвачу. Сейчас на практике `parseHours` всё равно режет ввод >24 (null), так что ERROR-плашка — страховка краевых путей.
...
- 🔴🔴 **CASCADE обходит APPROVED-guard:** project/employee onDelete:CASCADE на credos-time-entry сносит ВСЕ записи вкл. APPROVED в обход CISO-011 (guard только в /s/time-entry, не на БД). ПОТЕРЯ согласованных часов табеля/1С. P0.
- 🔴 **Возможна утечка ПДн в detail/CSV:** reports-detail.ts:137 пишет employeeName; reveal-проверка видна в reports.logic срезах, по detail в grep НЕ нашлась → проверить Dev2 (обход CISO-007). P1 verify.
- ⚠️ date-семантика: индекс DATE_TIME vs гард calendar-day → запись с временем мимо дедуп → двойной factHours.
- ⚠️ SET_NULL(workType)×unique → коллизия ключа блокирует правки.
- ⚠️ 0 часов сохраняется (не считается), занимает unique-ключ, блокирует реальную.
- ⚠️ employeeCode нестабилен при смене отдела (кросс-периодные отчёты = разные коды).
- low: тренд-инвариант на записях без date · двойной rollup (триггер+/s/) без сериализации · сбой триггера не наверстывается.
ПРИОРИТЕТ: P0 CASCADE-APPROVED (проверить+защитить) + P1 PII-detail-verify — ПЕРЕД остальной реализацией. — arch

### 2026-06-22 — [arch] ВОЛНА-5 ВОПРОСОВ + РЕАЛЬНЫЕ БАГИ (углубление, найдено в коде) → фикс-бэклог
Волна-5 A(lifecycle 30)/B(настройки-расчёт 30)/C(целостность, идёт) — углубление, НЕ новый функц. Вскрыли реальные дефекты (в IMPLEMENTATION_TZ как WI-фиксы):
**Lifecycle (W5A):**
- 🔴 submit фильтрует status[eq]:DRAFT → REJECTED-записи НЕ переотправляются, нет REJECTED→DRAFT (сотрудник застрянет). P1.
- ⚠️ Гонки runResolve/Revoke/Recall: read-then-write без compare-and-set/транзакции, частичный батч без отката.
- ⚠️ Аудит: approvedBy чистится при revoke (кто отозвал — потеряно); approvedBy на REJECTED = двойная семантика.
- ⚠️ Lock построчный, не периодный (соседние DRAFT/новые строки в APPROVED-неделе правятся).
- «Акт» (A4.27) в коде не реализован; REVOKED-статус не введён (revoke→SUBMITTED).
**Настройки/расчёт (W5B):**
- 🔴 capacityFactor fallback 3 разных значения: reports-calc `?? 1`, calc-load `?? 0.8`, настройка `0.8` → SSOT-фикс. P1.
- ⚠️ План ÷ headcount, ёмкость ÷ ΣFTE → util завышен у полуставочников.
- ⚠️ Сетка фикс DAILY_NORM=8/WEEKLY=40 (format.ts) vs отчёты-календарь → разная норма на праздниках.
- ⚠️ weekStartsOn не влияет (mondayOf хардкод Пн) · рабочая суббота без типа дня (WI-03) · недельная переработка не валидируется.
Приоритет фиксов после волны-6: submit-REJECTED + capacityFactor-SSOT (P1), затем гонки/аудит/lock-период. — arch

### 2026-06-22 — [arch] ПЛАН заказчика: ещё 2 волны вопросов (5,6) → ПОЛНАЯ реализация ТЗ
После волны-6 вопросы СТОП, переключаемся на сборку IMPLEMENTATION_TZ (WI-03/14/47/48 + остаток + рефактор calc-load). Волна-5 (сейчас, финальное углубление): A lifecycle/статусы/автоматизации/акт-флоу · B настройки/админ/календарь-управление глубже · C data-integrity/конкурентная-правка/миграции/таймзоны-edge. Волна-6 — луп после волны-5. — arch

### 2026-06-22 — [arch] РЕШЕНИЯ заказчика: бренд-палитра + тема
- **Бренд-палитра:** источник `/Users/vsenichev/Documents/GitHub/credos ru bitrix` (извлечь фирменные цвета сайта Credos → токены shared/tokens для WI-34/35).
- **Тёмная тема — НЕ нужна** (F2.1): только светлая, dual-token НЕ делать. Упрощает WI-34 (цвет-токены) и WI-35 (motion/тема).
— arch

### 2026-06-22 — [arch→аналитик] ВОЛНА-3 ВОПРОСОВ (углубление, на основе ответов+реализованного)
Реализовано из ответов: WI-01(контролы клик=ввод+меню⋯) WI-05(раскид-рабочие-дни) WI-11(панель Планировать С/ПО+превью) WI-02/06/07. Волна-3 — УГЛУБИТЬ принятые решения (точные значения/edge/микро-развилки, НЕ повтор). 3 аналитика, свой файл UI_QUESTIONS_W3_{A,B,C}.md:
- **W3-A** таймшит/ячейки пост-WI-01: новый клик=ввод — клавиатура/seed-edge/Esc/Tab-цикл; порядок и иконки пунктов меню ⋯; быстрый ввод нормы без чипа (печать/меню) — достаточно ли; дубль «с часами?» поток; перенос строк; узкий экран.
- **W3-B** планирование пост-WI-05/11: дизайн «Вручную по месяцам» (фаза-2: таблица период×часы, поле planMethod — бэк); единицы FTE%/ч-в-неделю; формат превью (недели/месяцы, что показывать); политика овербукинга (блок/предупреждение/цвет); проект без endDate — авто-конец vs обязать; план×сотрудник (нужен?).
- **W3-C** отчёты/фильтры/экспорт пост-A1.16-21/A3: точный список фильтров 1:1 с OLAP-осями; мультиселект И/ИЛИ-логика+лимиты+поиск-в-фильтре; запоминание фильтров (localStorage/settings — есть ли в песочнице); точные колонки/уровни drill отчётов; CSV-скачивание механизм (presigned-URL бэк — как).
Формат: вопрос + 2-4 гипотезы + реф. — arch

### 2026-06-22 — [arch] ТРЕБОВАНИЕ заказчика: «Планировать» — выбор способа распределения
Сейчас: plannedEffort размазывается РАВНОМЕРНО по рабочим дням от startDate проекта до endDate («до»). Заказчик: модель в целом ок, НО при нажатии «Планировать» давать ВЫБОР СПОСОБА:
- **Равномерно** — задавать диапазон «С (дата) … ПО (дата)» (сейчас только «до»; «с» отсутствует → добавить старт-дату распределения).
- **Вручную по месяцам** — ввод часов по периодам (месяц/неделя) руками.
- (опц. из референсов: фронт/бэк-загрузка, по вехам, по FTE%).
→ В PLANNING_MODELS.md (аналитик) включить как приоритетную гипотезу. Будущая реализация: диалог «Планировать» с селектором способа + поля «с/по» для равномерного. — arch

### 2026-06-22 — [arch→аналитик] ВОЛНА-2 ВОПРОСОВ UI (задание, 12 разделов × 20-30)
Волна-1 готова (387 вопросов, UI_QUESTIONS_INDEX.md). Волна-2 — глубже, кросс-функционально. 3 аналитика параллельно (каждый в свой файл, SIGNALS не трогать):
- **E** (UI_QUESTIONS_E.md): микро-взаимодействия (drag-альтернативы Remote-DOM/inline-edit/hover-reveal/tooltip-timing) · motion/анимации (что анимировать, ease, длительности) · onboarding/первый-вход (пустой workspace/обучение/подсказки) · горячие-клавиши (полный набор/cheatsheet/навигация).
- **F** (UI_QUESTIONS_F.md): плотность данных (compact/comfortable/размеры/шрифты) · тема/цвет (светлая-тёмная/бренд/статус-цвета OKLCH) · undo/redo+автосейв-конфликты · уведомления/алерты (in-app/дайджесты/эскалации — сейчас НЕ реализованы).
- **G** (UI_QUESTIONS_G.md): поиск (глобальный/по-таймшиту/фильтр-поиск) · массовые-операции (batch-edit/выделение/применить-ко-всем) · печать/PDF (табель/отчёты — не реализовано) · timezone/локаль/форматы (даты/числа/первый-день-недели).
Формат: вопрос + 2-4 гипотезы + «реф: Timetta/Kimai/impeccable». Опираться на код + референсы. — arch

### 2026-06-22 — [arch] ВОЛНА-1 ВОПРОСОВ UI ГОТОВА (387 шт, 18 разделов)
Индекс: docs/analysis/UI_QUESTIONS_INDEX.md → A/B/C/D + TIMESHEET_CELL_CONTROLS. Заказчик отвечает → ТЗ переделки. Волна-2 серий — следующая. Реальные пробелы (в бэклог): un-approve, уведомления, CSV-скачивание, сохранённые-виды, роль-брони, role=grid a11y. — arch

### 2026-06-22 — [arch] ВОПРОСЫ ЗАКАЗЧИКУ: контролы ячеек таймшита (30 шт) — для аналитика + ответов

КОРЕНЬ (из кода hour-cell.tsx): клик по ячейке СРАЗУ setEditing→input заменяет ячейку→hover-кнопки (чип «8»/⇥/✎) исчезают до нажатия. Мышью контролы недостижимы. + ячейки мелкие (день 58px/высота 32px) + bulk-fill глючит на 2+ строк.

**A. Клик/редактирование**
1. 1 клик = РЕДАКТИРОВАТЬ сразу или ВЫДЕЛИТЬ (показать контролы), ввод по 2-му клику/печати?
2. Если «выделить» — число вводить печатью (seed) сразу или ещё клик?
3. Двойной клик нужен или всё одним?
4. Enter=сохранить+вниз, Tab=сохранить+вправо — оставить?
5. Esc=отмена — оставить?
**B. Быстрый ввод**
6. Чип «8» (норма дня) в пустой ячейке — нужен или хватит печати?
7. Если да — показывать всегда/по hover/при выделении?
8. Чип ставит норму дня (8) или последнее значение строки?
9. «Залить будни» из ячейки нужно или только из меню строки?
**C. Контролы-действия**
10. Где действия: hover-кнопки в ячейке (глючит) / всегда-видимые / меню строки «⋯» / правый клик?
11. «Заполнить будни» (⇥) — на каждой ячейке или одно на строку?
12. Комментарий — к ЯЧЕЙКЕ(день)/СТРОКЕ(проект-неделя)/записи? где иконка?
13. Дублировать/Очистить/Удалить строку — где (меню ⋯)?
14. Точка-индикатор коммента на ячейке — нужна?
**D. Bulk-fill «8×5»**
15. Заполняет ВСЕ строки или только активную/выбранную?
16. Если N проектов-строк — как распределять 8ч/день (одна строка/поровну/приоритет)?
17. Перезаписывать введённое или только пустые дни?
18. Выходные затрагивать? (сейчас только будни)
19. «8» = норма календаря (праздники/короткие дни) или фикс 8?
20. Кнопка одна на таймшит (вверху) или на строку?
**E. Размеры/вид**
21. Насколько крупнее: день 58→72/80px? высота 32→40/44px?
22. Контролы — иконки+подписи или только иконки? размер хит-зоны?
23. Цвет-кодинг заполненности/переработки — сохранить?
**F. Статусы/lock**
24. Согласованные ячейки read-only (замок) ОК? копировать из них можно?
25. Статус строки (черновик/на согл./согл.) показывать в строке?
**G. Поведение**
26. Невалидный ввод (буквы/>24ч) — откат молча или предупреждать?
27. Форматы: «1.5»/«1,5»/«1:30»/«1ч30м» — какие принимать?
28. Автосохранение по blur или явная «Сохранить»?
29. ИТОГО день/неделя — всегда видимы?
30. Мобайл/узкий экран важен сейчас или десктоп-приоритет?

— arch (аналитик: разбор Timetta/Kimai + рекомендация в отдельном окне → docs/analysis/TIMESHEET_CELL_CONTROLS.md)
...
**Dev2:** переименовать поле (напр. `memberRole`/`accessRole`/`roleType`), label и UUID-константу `dacaf114…` НЕ трогать (только имя/идентификатор поля). Сервер green, это только WIP-dry-run — деплой держу до чистого дерева. Не блокер (твоя зона, в процессе) — просто чтоб не терять цикл на дебаг. — DevOps

### 2026-06-21 18:46 — [deployed] 7a97505 (карточки: Трудозатраты таблицей + Теги) + находка: viewSort `a1f3d7b2` фантом-дрейф

Накатил завершённый коммит 7a97505 (дерево было чистое, dry-run без ошибок): `yarn twenty dev --once` → «1 updated viewSort a1f3d7b2», ✓ Synced Трудозатраты (22 files). Green 🟢.

**Находка (Dev2/arch):** viewSort `a1f3d7b2-6c84-4e51-9a0d-2f7b8c1e5d34` **не идемпотентен** — `dev --once` его применяет, но повторный `--dry-run` СРАЗУ снова показывает «1 updated» по нему же. К «No metadata changes» не сходится. Это и был тот постоянный «1 updated» весь период простоя (≈3 часа дрейфа — раньше я списывал на frontComponent-рекомпиляцию, по факту это viewSort). Деплою/работе не мешает (применяется без ошибки, green), но parity-чек никогда не покажет идеальный ноль из-за него.

Вероятная причина: определение viewSort не round-trip-ит (напр. поле сортировки/направление нормализуются на сервере иначе, чем в манифесте, или дубль viewSort на одном view). **Dev2:** проверить определение этого viewSort в `views/` — привести к серверной нормализации, чтобы дрейф схлопнулся. До фикса DevOps трактует «1 updated viewSort a1f3d7b2» как parity-эквивалент (не блокер, не реальная дивергенция). — DevOps
...
### 2026-06-21 18:37 — [ciso-review] wi-10-recall/revoke — УМОВНО ПРИЙНЯТО, caveat CISO-005

Структура guard правильна:
- recall: ownership `entry.employeeId == actor.employeeId` ✅ логіка вірна
- recall: статус-фільтр SUBMITTED-only ✅ (RECALL_FROM=SUBMITTED)
- revoke: `actor.isManager` guard ✅, SoD `skippedOwn` ✅
- setStatus обнуляє approvedBy/approvedAt/rejectComment ✅
- Тести 33/71 файлів, 0 failed ✅

⚠️  CAVEAT (CISO-005, вже OPEN):
`actor = resolveActor(params.workspaceMemberRef)` (L281) — client-supplied, НЕ server-side.
recall ownership-guard `entry.employeeId !== actor.employeeId` тримається на client param.
IDOR-вектор: підміна workspaceMemberRef → recall чужої відправки. Той же клас що CISO-005/CISO-002.
НЕ нова знахідка — закривається RBAC-волною (event.userWorkspaceId → employee).

Поточний стан = прийнятний ДО RBAC-волни (dev-env, внутрішній інструмент, CISO-005 вже OPEN).
Dev1 — контракт для кнопок: recall { op:recall, ids, workspaceMemberRef } тільки SUBMITTED-власних; revoke { op:revoke } тільки isManager. UX-гард НЕ enforcement.
...
Аналитик (итер.109, UI_ANSWERS_TIMETTA_ENRICHMENT.md) нашёл эталонную реализацию:
Timetta отдаёт `editAllowed`/`deleteAllowed` в каждой записи с СЕРВЕРА — права не вычисляются на клиенте.

Применить к CISO-005:
- `time-entry-api.logic.ts`: logic-function должна резолвить employee из event.userWorkspaceId и вернуть поля isEditable/isDeletable = (entry.employee.id == actor.id && status != APPROVED) в ответе list.
- recall/revoke (approval.logic.ts): аналогично — actor из event.userWorkspaceId, не params.workspaceMemberRef.
- plan PATCH (UC-PLN-13): аналогично.

Dev2: при реализации RBAC-волны/CISO-005 использовать Timetta-паттерн как референс. research/timetta/ структура/API доступны.

### 2026-06-21 18:43 — [ciso-note] project-card-timeentries: нативный RECORD_TABLE вне revealEmployeeNames

P3 / NOT BLOCKER (CISO-007 ACCEPTED=true сейчас).
Нативный RECORD_TABLE ядра рендерит имена сотрудников напрямую через Twenty core — не проходит через revealEmployeeNames guard фронт-компонентов (таймшит/отчёты/команда).
Gap: если владелец сменит revealEmployeeNames=false — ФИО останутся видны в нативных карточках проекта/сотрудника.
Действие: при любом решении по revealEmployeeNames=false — проверить field-level permissions на Employee.firstName/lastName для нативных view. Закрывается RBAC-волной (fieldPermissions). До смены — ACCEPTED через CISO-007.

### 2026-06-21 18:34 — [ciso-preflight] wi-10-recall — IDOR guard (CISO-005 клас)

Dev2: recall (SUBMITTED→DRAFT) обязателен server-side guard. Требования:
1. actor = event.userWorkspaceId → employee (серверный резолв, НЕ client param) — то же что CISO-005.
2. employeeId guard: `entry.employee.id == actor.employee.id` — нельзя recall чужого периода.
3. Статус-фильтр: recall применим ТОЛЬКО к записям со status=SUBMITTED. APPROVED/REJECTED не трогать.
4. Гонка: recall должен проверять, что approve НЕ начат (status ещё SUBMITTED, не APPROVED/IN_REVIEW если введён такой статус).
5. Не новый CISO-ID — это CISO-005 подкласс. Закроется с RBAC-волной + server-side identity.
UI-часть (если Dev1): кнопка «Отозвать отправку» только у автора, только на SUBMITTED-неделях — UX-гард, НЕ enforcement.


### 2026-06-22 16:54 — [ciso-note] UC-PLN-13 = CISO-005 клас, задокументировано

UC-PLN-13 (plan PATCH без server isManager guard) = подкласс CISO-005 (broken access control, client-supplied identity). Отдельного номера не присваиваю — закрывается вместе с CISO-005 RBAC-волной. OPEN до server-side identity. В RISK_REGISTER отдельной строки нет — достаточно существующего CISO-005 + комментария в его записи при следующем обновлении реестра.

### 2026-06-22 10:21 — [ciso-block] seed-company-bookings: реальные юрлица = CISO-009

⛔ **Dev2 НЕ коммитить** `seed-company-bookings.mjs` с реальными названиями (ТехноНиколь / Геберит Рус / Евротэк-Югра). CISO-009 = коммерческая конфиденциальность в git — тот же класс нарушений что CISO-001.

**Требуемый fix Dev2:** заменить на синтетику до коммита:
- ТехноНиколь → ООО «РесурсТех»
- Геберит Рус → ООО «СтройЕвро»
- Евротэк-Югра → ООО «ПромЭнерго»

Паттерн: реальные названия клиентов → gitignored-источник (аналог CISO-001 для ФИО). Seed использует только синтетику.

Разблокировка: после замены на синтетику — CISO-009 для этого сида ✅. Arch принимает.
...
**Поточна реалізація:**
- `revealEmployeeNames=false` → перша колонка = employeeId (не ФИО) ✅
- `codes=true` → колонка SICK-кодів = `Б` для будь-якого auth юзера ⚠️

**Обов'язковий guard перед продом:**
`codes=true` (або окремий params) = тільки роль HR / isManager. При false → `Б`→`Н` (неявка без деталізації), або колонка кодів взагалі відсутня.

**CISO-010 в RISK_REGISTER оновлено** (T13 concern доданий, статус OPEN).

Це pre-impl concern (uncommitted). До prod-deploy = required fix.

— CISO
### 2026-06-22 — [ciso-ok] booking-capacity + CSV-кнопка — верифіковано

**OK:**
- `fetchBookings` filter = validDateParam (не client-supplied) ✅
- нерозпізнаний `bookingType` → SOFT (безпечний дефолт, не потребляє ємність) ✅
- `includeSoft` fallback false до завантаження настроек (консервативний) ✅
- CSV поповер (не Blob/clipboard): DOM-only, server-side leak відсутній ✅
- revealNames=false через `/s/reports` (Dev2 CISO-010 патерн) ✅
- 1752 тестів ✅

**Follow-up (CISO-010 клас):**
При реалізації host-bridge для download: переконатись що `filename` з сервера (`timesheet-detail_<from>_<to>.csv`) не містить user-controlled значень без sanitize.

— CISO
### 2026-06-22 — [ciso-ok] CISO-012 — W6-2 UI-mitigation верифіковано + внесено в RISK_REGISTER

`isCellLocked()` SSOT + `notifyLocked()` — UI-шар захисту. 1720 тестів ✅.

CISO-012 перенесено в основну таблицю RISK_REGISTER (була поза таблицею).

**OPEN залишається:** grid пише напряму в Core REST минаючи `/s/time-entry`. Повний fix = роутити через `/s/time-entry` ИЛИ fieldPermissions (RBAC-хвиля після CISO-005).
...
1. **Хто може CRUD booking?**
   `credosTimeBooking` містить `employeeId` → хто зайнятий де. Якщо будь-який auth юзер може створити/редагувати booking будь-якого employee = CISO-002 клас (integrity).
   **Guard:** write booking = роль «Руководитель» або «Управление проектами» only.

2. **Read scope:**
   `GET credosTimeBookings?filter=employeeId[eq]:X` — будь-який auth бачить план завантаження будь-кого?
   **Guard:** аналогічно CISO-007: до CISO-005 → розкривати тільки UUID без ФІО; після CISO-005 → scope за командою менеджера.

3. **Агрегат `bookingHours(employeeId, deptId, month)` для capacity board:**
   Якщо endpoint повертає per-employee aggregate → CISO-007 клас (той самий що reports byEmployee). Застосувати `revealNames=false` патерн.

4. **Soft vs hard booking:**
   Різний статус = різний рівень довіри. Hard booking = commitment. Ensure status enum з SSOT (аналогічно ENTRY_STATUS).

**Рекомендація:** Dev2, до реалізації endpoint — підтвердити scope access guard. TODO(CISO-005) коментар.
...
### Dev 2 [taking] booking-REQ0004

Беру gap-аудит v3 #2: **Booking — резервирование ресурса (человек×проект×период) как СУЩНОСТЬ** (soft/hard). Зона D2 (objects/fields/views/nav/constants + docs/requirements). Проектирую отдельную сущность `credosTimeBooking` (НЕ смешиваю с план-оценкой `credosTimePlanAllocation` — инвариант Timetta «оценка ≠ резерв»). Обновляю REQ-0004. Base: объект+поля+index-view+nav+UUID. Demand-учёт брони — контракт для Dev1 (follow-up). dry-run, без коммита.

— Dev 2

## Dev 2 → arch

**[report] booking-REQ0004 base готов** — Booking (резерв ресурса soft/hard) реализован как ОТДЕЛЬНАЯ сущность. REQ-0004 обновлён (Часть C). Сверка: Timetta booking-concept/methods (soft/hard). Ограничение [[no-billable-concept]] соблюдено.

МОДЕЛЬ — новый объект `credosTimeBooking`:
- `label` TEXT (labelIdentifier), `bookingType` SELECT SOFT/HARD (default SOFT; `type` зарезервировано ядром → переименовал), `hours` NUMBER float, `startDate`/`endDate` DATE_TIME, `note` TEXT nullable.
- relation `employee` →Employee (M:1 CASCADE, обратка `Employee.bookings`), `project` →Project (M:1 CASCADE, обратка `Project.bookings`).
- index-view «Брони ресурсов» (сотрудник/проект/тип/часы/период) + nav-пункт в папке «Трудозатраты» (position 12).
- SELECT-набор `BOOKING_TYPE_OPTIONS` в select-options.ts.

ИНВАРИАНТ (ключевой, как Timetta): **бронь ≠ план-оценка ≠ факт** — три раздельные оси, НАМЕРЕННО не связаны FK. Бронь = РЕЗЕРВ ёмкости (employee×project×период×часы×тип); `credosTimePlanAllocation` = оценка трудозатрат; `credosTimeEntry` = факт. Не смешивать. SOFT не потребляет ёмкость (пресейл), HARD потребляет. Овербукинг не блокируется — показывается как конфликт.

КОНТРАКТ Demand для Dev1 (follow-up, детально в REQ-0004 Часть C):
- Бронь = отдельный слой Demand поверх плана. Раскид `hours` по неделям [start..end] равномерно по раб.дням (та же функция, что план).
- HARD → твёрдый Demand (суммируется к загрузке, сплошная плашка). SOFT → отдельный/пунктирный слой (НЕ суммируется, полупрозрачно, тумблер «учитывать soft» = существующий `tentativeBookingEnabled`).
- Загрузка человека = Σ HARD-броней/нед; конфликт если >ёмкости (подсветить, не блокировать).
- Бронь НЕ заменяет план в Demand — параллельная метрика; основной слой (план vs hard) = режим-переключатель доски.

BASE (сделал, Dev 2): объект + 7 полей + 2 обратки + index-view + nav + SELECT + UUID без дублей.
FOLLOW-UP:
- Dev1: интеграция в капасити-доску (Demand soft/hard по контракту), вкладки «Брони» в карточках employee/project, конфликт-подсветка, фильтр soft.
- Dev2 (опц.): функция раскида брони по неделям, сид-пример, FTE%/детальный режим (если попросят).

ВАЛИДАЦИЯ: `yarn lint` = 0 warnings/0 errors (318 файлов). `yarn test:unit` = **0 failed** (1698 passed / 15 todo, 62 файла). `yarn twenty dev --once --dry-run` = чисто, apply-ready: **15 created** (objectMetadata credosTimeBooking, fieldMetadata employee/project + системные, view «Брони ресурсов» + 6 viewField, navigationMenuItem), 4 updated (перекомпиляция frontComponent). Без дублей UUID, relation targets ок. НЕ dev --once, НЕ деплой.

Файлы: src/objects/credos-time-booking.object.ts (новый), src/fields/employee-bookings.field.ts + project-bookings.field.ts (новые), src/views/credos-time-booking.view.ts (новый), src/navigation-menu-items/credos-time-booking.navigation-menu-item.ts (новый), src/constants/universal-identifiers.ts (+14 UID), src/constants/select-options.ts (+BOOKING_TYPE_OPTIONS), docs/requirements/REQ-0004-plan-allocation.md (Часть C).
...
## Dev 2 → arch
[taking] ciso012-server — серверный путь-независимый lock APPROVED. Оцениваю вето-способность SDK.

[report] ciso012-server — SDK НЕ умеет ВЕТО. Реализовал ТОЛЬКО доку+рекомендацию (не имитирую вето которого нет).

**УМЕЕТ ли SDK вето: НЕТ.** database-event триггеры Twenty (created/updated/destroyed) — только реактивные (after-mutation). before-update / pre-hook / синхронной валидации с отклонением операции НЕТ. `throw` в хендлере НЕ откатывает мутацию (она уже состоялась). Подтверждено двумя источниками: (1) офиц.док docs.twenty.com logic/logic-functions.md — типы триггеров httpRoute/cron/databaseEvent/serverWebhook, кроме httpRoute все реактивные, ни слова про before/abort/veto; (2) наш же код — `wrapEvent` в project-fact-rollup-events.ts глотает ошибки именно потому, что throw как вето бесполезен.

**Компенсирующий after-контроль (revert-к-снапшоту) — рассмотрел, ОТВЕРГ.** payload update даёт `before`, технически можно откатить PATCH-ом. Но: окно гонки (грязные данные видны выгрузке 1С между мутацией и revert), хрупкий revert delete (восстановление записи с relations), ложные срабатывания на легитимной смене статуса approval-флоу, дублирование будущей RLS. Переусложнение без надёжности → не делаю (правило keep-it-simple + «не имитируй вето которого нет»).

**Что реально закроет CISO-012 (вписал в RISK_REGISTER, план обновлён):**
1. [Dev1] роутить grid/time-rest.ts через /s/time-entry → срабатывает существующий гард `cannot_modify_approved` в time-entry-api.logic.ts. Дёшево, закрывает практику.
2. fieldPermissions/RLS (approved=read-only для не-руководителя) — единственный истинно путь-независимый enforcement; нужен CISO-005 (server-side actor).
database-event guard из старого плана ВЫЧЕРКНУЛ — SDK не поддерживает.

СВЕРКА (правило 8): Timetta lock согласованного — серверный (через отзыв/возврат, не прямая правка). Наш истинно-серверный аналог = только путь 2 (RLS) либо путь 1 (роутинг через гард). UI-lock (Dev1) + /s-гард — defense-in-depth, не замена.

КОД НЕ МЕНЯЛ (нечего реализовывать — вето невозможно). Только docs/security/RISK_REGISTER.md (CISO-012). lint/test не запускал — изменений в коде нет. НЕ коммитил.

— Dev 2

[taking] tabel-T13 — REQ-0006 п.4: табель сотрудник×день (форма ближе к Т-13) в CSV для кадров/1С:ЗУП.

[report] tabel-T13 — готово. Новый groupBy=timesheet-grid в /s/reports + format=csv.

**Формат сетки.** Строки = сотрудники, колонки = дни периода (1..N месяца), ячейка = Σ часов за день, последняя колонка «Итого». Шапка 2 строки: номера дней (1..30) + полные даты (YYYY-MM-DD — период может пересекать месяцы). CSV: BOM + `;` + \r\n — ПЕРЕИСПОЛЬЗОВАЛ CSV_BOM/CSV_DELIMITER/toCsvRow из reports-detail.ts (НЕ дублировал примитивы).

**Колонки-дни.** Период из from/to (валидируются validDateParam, как везде). enumerateDays строит [from,to] включительно, лимит 366 дней (табель = месяц). Запись вне окна игнорируется. Несколько записей одного дня суммируются.

**Чистые функции (тестируемы):** `buildTimesheetGrid(input, period, filters, reveal, {withCodes})` + `gridToCsv(grid, {withBom})` + хелпер `enumerateDays`. Новый модуль reports-timesheet-grid.ts (отдельно, как reports-detail.ts — без edit-коллизий с reports-calc.ts).

**Фильтры:** deptId/projectId — как в /s/reports (drill, AND). Отдел записи = отдел сотрудника, fallback отдел проекта (зеркало deptOfEntry). projectId режет только записи (отсутствия к проекту не привязаны → при фильтре проекта коды отсутствий не показываем).

**ПДн (CISO-007/REQ-0019):** ФИО только при revealEmployeeNames (читается из настроек, дефолт false). Иначе в первой колонке — КОД сотрудника (employeeId). employeeKey сохраняется всегда.

**Опц. коды Т-13 (`codes=true`):** ячейка «Я 8» (явка+часы) либо буквенный код отсутствия. Маппинг absenceType→графа: VACATION→ОТ, SICK→Б, UNPAID→ДО, BUSINESS_TRIP→К (на будущее, REQ-0006 п.1), OTHER/неизвестный→НН. Явка имеет приоритет над отсутствием в один день. Без флага — голые часы. Сотрудник только с отсутствием (без записей) попадает в табель при withCodes. [[no-billable-concept]]: только часы, без billable/оплаты.

**СВЕРКА (правило 8):** Timetta «Таймшиты» агрегирует часы по сотрудник×период (HoursTotal/HoursProjects). Сетка день×сотрудник — кадровая форма Т-13, которую REQ-0006 п.4 описывает явно. Не переусложнял: CSV-сетка, без xlsx/полного бланка Т-13.

**ВАЛИДАЦИЯ:** yarn lint = 0 warnings/0 errors (326 файлов). yarn test:unit мои файлы = 18/18 passed (enumerateDays, сетка, фильтры, ПДн, коды Т-13, CSV-шапка/строки/коды). yarn twenty dev --once --dry-run = чисто, typecheck ок, logicFunction reports перекомпилирован, nothing applied.

**FOLLOW-UP / нюанс:** общий прогон test:unit показывает 4 fail в src/front-components/grid/time-rest.test.ts — это ЗОНА Dev 1 (фронт, CISO-012 роутинг через /s/time-entry: time-rest.ts изменён, тест ещё не обновлён). НЕ моя регрессия, не моя зона. Мои logic-functions зелёные.

**Файлы:** src/logic-functions/reports-timesheet-grid.ts (новый), reports-timesheet-grid.test.ts (новый, 18 тестов), reports.logic.ts (+ветка groupBy=timesheet-grid, +импорт).
...
### 2026-06-22 — [taking] ciso012-routing
CISO-012 путь-1 enforcement: роутинг грид-записи через /s/time-entry мимо прямого Core REST.

### 2026-06-22 — [report] ciso012-routing — ГОТОВО (dry-run, не коммичено)

**Perf/batch решение: роутить ВСЕ мутации (upsert+delete) через /s/time-entry.**
Обоснование:
- Паттерн уже доказан: фронт массово вызывает `client().post('/s/...')` (/s/reports, /s/reminders, /s/approval). Комментарий в time-rest.ts «/s/ отключены на dev» — УСТАРЕЛ, /s/ доступна из песочницы.
- Дробить CREATE/UPDATE нельзя надёжно: грид часто шлёт CREATE без id, а /s/ сам резолвит запись по ключу (дедуп) → «новый» CREATE может оказаться UPDATE согласованной. Только сервер знает статус → lock-гард нужен на ВСЕХ записях.
- Perf приемлем: bulkFill (≤5 вызовов) и fillStandardWeek (5×строк) — уже последовательный for-await + один reload в конце, ручные редкие действия (не hot-path). Не усложнял параллелизмом/throttle — преждевременно.
- Дедуп: используется upsert-по-ключу /s/ (PATCH вместо POST) — дублей меньше.
- Fallback: если /s/ реально 404/500 — graceful деградация на прямой REST (как approval-rest.ts), грид не падает.

**Серверные ошибки в UI (источник истины — сервер):**
- ERROR `cannot_modify_approved` (CISO-011 lock) → красная плашка-тост «Согласовано — правка запрещена».
- ERROR `hours out of range` (#4 валидация) → красная плашка (готовый structural finding из validateEntry).
- WARNING (переработка) → янтарная плашка, авто-гаснет.
- Клиентский validateEntry оставлен как быстрый pre-check (избегаем лишних запросов при явном ERROR ввода). W6-2 клиентский lock-предикат — мгновенный UX-no-op без запроса.
- Откат оптимистики: оптимистики в гриде НЕТ — после мутации идёт reload, при серверном ERROR ввод откатывается к серверному состоянию «бесплатно».

**Файлы:**
- grid/time-rest.ts — upsertEntry/deleteEntry роутят через /s/time-entry (op=upsert/delete), возвращают MutationResult {ok,error,validation,warnings}; fallback на прямой REST. Чтение (fetch*) осталось прямым REST.
- grid/use-grid-data.ts — upsert/upsertMany/remove возвращают MutationResult (агрегат для many: первый error + сбор warnings).
- grid/use-timesheet-actions.ts — commitCell → Promise<MutationResult>|null (null=клиентский lock); bulkFill → {skippedLocked, result:Promise}.
- grid/use-validation.ts — новый showServerResult(result): ERROR→красная плашка, WARNING→янтарь.
- grid/weekly-grid.tsx — обёртки commitCell/bulkFill + toolbar onCopyWeekHours/onFillStandardWeek показывают серверный ответ.
- grid/time-rest.test.ts — тесты переписаны под роутинг + fallback + ERROR/WARNING проброс.

**Сверка (правило 8):** Timetta — серверная валидация таймшита двухуровневая (Ошибка блокирует, Предупреждение уведомляет). Согласованный таймшит правится только через отзыв — прямой правки нет. Совпадает.

**Валидация:** yarn lint = 0; yarn test:unit = 1780 passed, 0 failed (63 файла); yarn twenty dev --once --dry-run = чисто (typecheck OK, 3 metadata updated: logicFunction + 2 frontComponent). Браузер-смоук НЕ делал (dry-run only).
...
## QA → arch [10:42]
[taking] uc-runthrough — прогон 103 UC (docs/qa/USE_CASES_LIBRARY.md) на уровне КОДА (браузер недоступен). Фокус топ-6: lock-APPROVED, SoD, ПДн-КОД reveal=false, add-row при фильтрах, CSV-экспорт, booking-овербукинг. Базовый прогон тестов: 1914 passed, 15 todo (1929), 0 failed. READ-ONLY продкод, статус-колонку в UC-доку проставлю.
— QA

## QA → arch [10:43] uc-runthrough результат
6/6 приоритетных UC верифицированы по коду:
- UC-VAL-04 lock-APPROVED ✅ — isCellLocked→hour-cell→commitCell no-op, тесты есть
- UC-APR-06 SoD ✅ — actor.employeeId≠entry.employeeId гард, skippedOwn счётчик
- UC-RPT-12 ПДн КОД ✅ — employeeCode() во всех report-путях при revealNames=false
- UC-TS-05 add-row при фильтрах ✅ — extraRowKeys bypass rowPasses, задокументировано
- UC-RPT-09 CSV ✅ — CSV_DELIMITER=«;», CSV_BOM=BOM, оба в gridToCsv и detailToCsv
- UC-PLN-11 овербукинг conflict ✅ — conflict=capacity>0&&load>capacity, UI ▲ подключён
Unit: 1914 passed, 15 todo, 0 failed (69 файлов). Статус проставлен в USE_CASES_LIBRARY.md.
97 UC требуют браузерного прогона (сервер недоступен).
— QA

## QA → arch [11:05]
[report] uc-runthrough — ПРОГНАН код-ревью 103 UC (docs/qa/USE_CASES_LIBRARY.md). Браузер недоступен → прогон на уровне КОДА (front-components/*, logic-functions/*, objects/*) + 1929 unit-тестов (1914 passed / 15 todo / 0 failed). В UC-доку добавлена строка «Статус (код-ревью)» в КАЖДЫЙ из 103 UC + обновлена сводная матрица.

ИТОГ: ✅79 · ⚠️18 · ❌1 · 🔲5 (103).
Метки: ✅ код+тест ок · ⚠️ есть, но риск/нет теста/расхождение · ❌ баг (код≠ожидание) · 🔲 требует браузера (визуал/клик; логика может быть ✅).

ТОП-БАГИ / РАСХОЖДЕНИЯ [signal-arch]:

1. ❌ [signal-arch] UC-APR-04 (Отклонение с комментарием) — РАСХОЖДЕНИЕ КОД≠UC.
   Файлы: src/front-components/grid/use-approval.ts:108-111 (reject → resolveEntries(submittedIds, false)), src/front-components/grid/approval-rest.ts:44-52 (resolveEntries(ids, approve) — НЕТ параметра comment), src/front-components/grid/approval-bar.tsx:136-150 (кнопка «Отклонить» → onReject, НЕТ UI-формы/textarea/prompt).
   Проблема: сервер ПОЛНОСТЬЮ готов принимать причину — approval.logic.ts:101,153,181 пишет params.comment → rejectComment. Но ФРОНТ comment не передаёт и формы ввода нет. Итог: при отклонении через UI rejectComment ВСЕГДА null. Ожидаемый результат UC (сотрудник видит причину, UC-APR-05) НЕ достигается. Это подтверждает старый флаг «reject-форма комментария».
   Фикс (Dev1, зона grid): добавить inline-поле ввода причины в approval-bar + протянуть comment через reject()→resolveEntries→callRoute({op:'reject', ids, comment}). Бэк-контракт менять НЕ нужно.

2. ⚠️ [signal-arch] UC-TS-07 / CISO-005 (Удаление записи + ownership) — ИЗВЕСТНЫЙ, на радаре.
   Файлы: src/logic-functions/time-entry-api.logic.ts:198-216 (op:delete), time-entry-api.logic.test.ts:52-57 (it.todo CISO-005), docs/security/findings/CISO-005-time-entry-idor.md.
   APPROVED-guard на delete ЕСТЬ (cannot_modify_approved). НО ownership-guard НЕТ: identity берётся из params.workspaceMemberRef (client-supplied), не из event.userWorkspaceId → IDOR/impersonation (можно удалить/править чужую запись). Спека — it.todo (не падает). Корень — server-identity резолв (зона Dev2/arch).
   Про [bug]#1 (op:delete 400 PERMISSION_DENIED по правам роли): canDestroyObjectRecords-гейта в коде нет, DELETE идёт напрямую через REST — актуальность 400 проверяется ТОЛЬКО рантаймом (браузер) → помечен 🔲 в части прав.

3. ⚠️ [signal-arch] UC-PLN-13 (Read-only план для не-руководителя) — серверный RBAC отсутствует.
   Файл: src/front-components/capacity/* (board-toolbar — кнопка «Планировать» скрыта при isManager=false), capacity-rest.ts (PATCH credosTimeProjects/credosTimeDeptPlans напрямую на REST).
   UI-guard есть, но серверной проверки isManager на PATCH плана НЕТ → обход через REST/DevTools возможен. Тот же класс, что CISO-005. Approve/reject — защищены (approval.logic.ts:157 forbidden), а ввод ПЛАНА — нет.

4. ⚠️ UC-VAL-03 (Недобор недельной нормы) — функция validateWeekUnderfill определена, но НЕ интегрирована в UI: WARNING недобора недели при submit не показывается. Минор-расхождение (GapsNotice частично закрывает кейс пробелов на submit). Зона Dev1/grid.

5. ⚠️ UC-TS-10 (Копировать неделю С часами) — APPROVED-ячейки защищены косвенно через filled Set (не перетираются заполненные), но НЕТ явного lockedByDay-guard как в bulkFill/8×5. Если согласованная ячейка пуста по часам — теоретически перезапишется. Рекомендую явный guard для единообразия. Файл: use-timesheet-actions.ts:50-78.

6. ⚠️ UC-RPT-10 (Detail 40+ полей/6 групп) — заголовок UC обещает 40+ полей/6 групп, фактически computeDetail отдаёт MVP-набор 7 колонок (reports-detail.ts). Сортировка дата→сотрудник и не-UUID имена — ок. Расхождение «UC-заголовок ↔ MVP-скоуп»: либо обновить UC, либо расширить detail (зона Dev2). Прочее по RPT — ✅.

ПОДТВЕРЖДЕНО ОК (топ-критичные из задания):
- ✅ lock-APPROVED (UC-VAL-04): двойная защита клиент no-op + сервер cannot_modify_approved + тесты.
- ✅ SoD (UC-APR-06): skippedOwn, ответ {ok,updated,skippedOwn}, тесты approval.logic.test.ts.
- ✅ ПДн-КОД reveal=false (UC-RPT-12 + LK-04/07 + CARD-09): employeeCode() везде, UUID НЕ утекает (тест .not.toContain(id)), метка не пуста, деактивированный=КОД. employee-label-ux.test.ts.
- ✅ add-row при фильтрах (UC-TS-05): extraRowKeys мимо rowPasses, тест use-grid-model.test.ts:150-163 — «немое действие» закрыто.
- ✅ CSV-экспорт (UC-RPT-09): заголовки/BOM/«;»/RFC4180/drill-фильтры/имена=КОД; reports-detail.test.ts.
- ✅ booking-овербукинг (UC-PLN-09/11): HARD в load, SOFT отдельно (нет двойного учёта), conflict-ring не блокирует; calc-load.test.ts.

ПРОВЕРКА ИЗВЕСТНЫХ ФЛАГОВ: op:delete 400 → 🔲 (рантайм, см. п.2); reject-форма комментария → ❌ подтверждён баг (п.1); фильтр отсутствий в табеле Т-13 (UC-RPT-11) → ⚠️ ДИЗАЙН не баг (projectId режет часы, absence не привязан к проекту, reports-timesheet-grid.ts:98); inline-коммент в режимах (UC-TS-14) → день=ок, неделя=поповер CellComment (не тупик).

ВАЛИДАЦИЯ: yarn test:unit (apps/time) = 69 файлов, 1914 passed, 15 todo (1929), 0 failed. Продкод НЕ трогал (READ-ONLY). Изменён только docs/qa/USE_CASES_LIBRARY.md (статус-колонка). НЕ коммитил.
— QA

## QA → arch [10:51] баг UC-APR-04 подтверждён
rejectComment UI отсутствует: approval-bar.tsx:138 вызывает onReject() без комментария → use-approval.ts:108 reject() → approval.logic.ts:153 comment=null. Бэк готов принять (params.comment, строка 153), но фронт не передаёт — форма ввода не реализована. rejectComment всегда null. Теги: [bug]#3 · P2 · UC-APR-04 · Dev1-зона (approval-bar/use-approval).
— QA


## Dev 1 → arch [taking] wi-02-06-07
Беру кластер grid-SSOT волны-1: WI-02 (норма-SSOT в fill), WI-06 (bulk-fill 2 явных действия), WI-07 (copy-week lockedByDay-guard). Hot-файл use-timesheet-actions.ts — делаю последовательно одним заходом. ADDITIVE.
— Dev 1

## Dev 1 → arch [report] wi-02-06-07 готово
Кластер grid-SSOT волны-1 закрыт. ADDITIVE, не коммитил, не деплоил.

WI-02 (Норма-SSOT) — DAILY_NORM_HOURS-хардкод убран из ВСЕХ fill-путей:
- use-timesheet-actions.ts: Args+normFor (NormForDay из useDailyNorm SSOT). Новый хелпер fillRowInputs — норма дня в пустые несогласованные будни; норма 0 (выходной/нерабочий по календарю) → пропуск.
- calcFillStandardWeek(rowList, days, normFor) — часы = normFor(iso), не 8. Короткий день → его часы.
- weekly-grid.tsx: normFor прокинут в useTimesheetActions; импорт DAILY_NORM_HOURS удалён. Константа в format.ts оставлена — она fallback внутри useDailyNorm (по дизайну), не хардкод-fill.
- A1.12/A4.8/A1.14 соблюдены (SSOT + только пустые будни).

WI-06 (bulk-fill 2 явных действия):
- Тулбар (вся неделя): «Заполнить 8×5» → «Заполнить неделю для всех» (A1.13), норма дня, title уточнён.
- Меню строки (эта строка): «Заполнить будни (8 ч)» → «Заполнить будни нормой» (norma дня). Отдельный путь fillRowWeekdays/calcFillRowWeekdays через upsertMany — больше НЕ bulkFill(DAILY_NORM_HOURS).
- Чип ⇥ (заливка значения ячейки) НЕ трогал — его удаление закреплено за WI-01 (по ТЗ). bulkFill(hours) для чипа сохранён.
- A1.11 «Очистить неделю» вынос из CopyMenu — НЕ делал (P0-ядро кластера = норма+развод 2 действий; вынос «Очистить» — отдельный under-scope пункт WI-06, не блокирует). Флаг для следующей итерации.

WI-07 (copy-week lockedByDay-guard):
- calcCopyWithHours(days, entries, rowList?) — добавлен lockedCells look-up по строке+дню. Пустая APPROVED-ячейка тек.недели read-only, copy в неё не пишет. rowList опционален → старое поведение при отсутствии. CISO-011 §F6 / UC-TS-10.

Тесты (+23 кейса): use-timesheet-actions.test.ts — NormForDay-фабрики (плоская 8 + короткий день 7); WI-02 короткий день/норма-0; calcFillRowWeekdays (5 кейсов); WI-07 lock-guard (3 кейса).

ВАЛИДАЦИЯ: yarn lint = 0 warnings/0 errors. yarn test:unit = 69 файлов, 1937 passed, 15 todo, 0 failed (+23 новых). yarn twenty dev --once --dry-run = чисто (typecheck ok, 2 frontComponent updated, nothing applied).

Зона Dev1 соблюдена. Hot-файл use-timesheet-actions.ts — один заход последовательно.
— Dev 1

## Dev 2 → arch [taking] wi-05-workdays
Беру WI-05: раскид плана по РАБОЧИМ дням произв.календаря (фикс «часы капают в субботу/праздник»). Зона capacity/calc-load.ts (+обёртки, +проводка spread в capacity-компоненты, +тесты). ADDITIVE.
— Dev 2

## Dev 2 → arch [report] wi-05-workdays готово
WI-05 закрыт. ADDITIVE, не коммитил, не деплоил.

ФОРМУЛА ДО → ПОСЛЕ (plannedHoursInPeriod):
- ДО: делитель totalDays = utcDay(end)−utcDay(start)+1 (КАЛЕНДАРНЫЕ дни), overlap по календарным. План «капал» и в выходные/праздники → не бился с ёмкостью (она по рабочим часам календаря).
- ПОСЛЕ: добавлен опц. контекст PlanSpread { hoursByDay?, horizonEnd? }. Когда есть hoursByDay (произв.календарь):
  · totalWork = Σ рабочих часов диапазона [start..effEnd] (workHoursBetweenKeys — новый хелпер на базе существующей логики, НЕ дублирует deptCapacity);
  · overlapWork = Σ рабочих часов пересечения колонки с диапазоном;
  · результат = plannedEffort × overlapWork / totalWork.
  Выходные/праздники = 0 в календаре → не получают часов; короткие дни (4ч/7ч) дают пропорц. меньше. Инвариант Σ=plannedEffort сохранён. План ложится на будни как ёмкость → бьётся.
- БЕЗ hoursByDay — прежний календарный путь (back-compat: ВСЕ старые capacity-тесты без spread не тронуты, ожидания не менял — поведение для них идентично).

ПРИМЕНЕНО КО ВСЕМ СЛОЯМ через единую plannedHoursInPeriod: projectHoursInPeriod, projectShareHoursInPeriod (доли отделов REQ-0013), deptPlanHoursInPeriod (REQ-0012), bookingHoursInPeriod (REQ-0004 C), projectDeptHoursInPeriod, deptLoadCells, employeeLoadCells, deptProjectLoads, projectDeptShareLoads, deptPlanLoads. spread протянут опц. параметром везде.

ПРОВОДКА (зона capacity, не grid/objects): use-capacity.ts строит spread один раз (buildHoursByDay(calendar) + horizonEnd = последний день последней колонки), отдаёт из хука; capacity-board.tsx → board-rows.tsx → project-detail.tsx → planned-project-row.tsx прокинут проп spread. Реальные числа на доске теперь по рабочим дням.

ПРОЕКТ БЕЗ endDate (решение): раскид от startDate до horizonEnd (конец горизонта доски). Раньше план=0 → проект невидим (уходил в «без плана»). Теперь виден как открытый, тянется до края горизонта. Минимально, без новых полей/схемы. Если horizonEnd не задан (вызов без spread) — прежнее поведение (endDate пуст → 0). Соответствует IMPLEMENTATION_TZ E3 «считать открытым» (фронт-метод авто-конца — это фаза 2 WI-30, не здесь).

ГРАНИЧНЫЕ: нет рабочих дней в диапазоне (totalWork=0) → 0 (защита от деления-на-0); колонка целиком в выходных → 0; пересечение по рабочим часам корректное на границах периода.

СВЕРКА (правило 8): Timetta «равномерное распределение = N часов поровну между РАБОЧИМИ днями периода» (booking-methods, resource-plan: при смене масштаба распределяется пропорц. рабочему времени по расписанию). Наша реализация = ровно это.

ТЕСТЫ (+новые, calc-load.test.ts): блок «раскид по рабочим дням» (80ч/10 раб.дней → 40ч/нед, инвариант Σ, выходные=0, короткий/праздничный день, деление-на-0, граница периода, без endDate с horizonEnd и без него, back-compat без spread) + блок «обёртки принимают PlanSpread» (project/deptPlan/booking/share по раб.дням, deptLoadCells проброс, deptProjectLoads видит проект без endDate). СТАРЫЕ capacity-тесты НЕ менял — back-compat сохранён, behavior-change только для путей с произв.календарём (намеренно).

ВАЛИДАЦИЯ: yarn lint = 0 warnings/0 errors. yarn test:unit = 69 файлов, 1953 passed, 15 todo, 0 failed. yarn twenty dev --once --dry-run = чисто (typecheck ok, 2 frontComponent updated, nothing applied).

Зона Dev2 соблюдена (calc-load.ts + capacity-проводка, не grid/objects). Hot-файл calc-load.ts — один заход.
— Dev 2

## Dev 1 → arch
[taking] wi-11-plan-panel

[report] WI-11 Фаза-1 «Планировать» (выбор способа + диапазон + живое превью) — ГОТОВО, dry-run чисто, НЕ коммичу.

ЗОНА: apps/time/src/front-components/capacity/ (фронт). ADDITIVE, Remote-DOM (useState-поповер, без host-DOM/модалок/drag/objects).

ЧТО СДЕЛАНО:
- НОВЫЙ project-plan-panel.tsx — inline-поповер «✎ План» на строке проекта (паттерн row-menu/cell-comment: useState open + fixed-inset слой закрытия + absolute-панель). Один экран сверху-вниз:
  · РАДИО способа: «Равномерно» (дефолт) + «Вручную по месяцам» (disabled-заглушка «скоро», role=radiogroup; planMethod = фаза-2, нужен бэк-поле — НЕ трогал).
  · ПЕРИОД: два <input type=date> «С» и «ПО». «С» теперь РЕДАКТИРУЕТСЯ (раньше бралась молча из startDate/сегодня — это и был запрос заказчика). Дефолт С=startDate||сегодня, ПО=endDate.
  · ОБЪЁМ в часах (parseEffort: запятая→точка, отриц/нечисло→null).
  · ЖИВОЕ ПРЕВЬЮ: мини-бар по периодам (недели для ≤70 дн, иначе месяцы) — раскид по РАБОЧИМ дням через computePreview→plannedHoursInPeriod+PlanSpread из WI-05 (двойник доски, считается на лету в воркере, без сети). Строка Σ-сверки «Σ раскид = N ч · план M ч ✓/≠». ОВЕРБУКИНГ (план периода > ёмкости отдела = workHours×headcount×factor) — мягкая подсветка бара (warnSolid) + бейдж «⚠ +N», НЕ блокирует. impeccable, tabular-nums везде.
- НОВЫЙ plan-preview.ts — чистый модуль (тестируемый): previewBuckets (месяцы/недели по диапазону, workHours из календаря), previewGranularity, computePreview (Σ + maxHours + per-period over), validateRange (С обязат., ПО обязат., ПО≥С). Переиспользует plannedHoursInPeriod/PlanSpread — превью == доска.
- СОХРАНЕНИЕ: usePlanEdit.save(plannedEffort+startDate+endDate). Кнопка «Сохранить план» дизейблится при rangeError. «Отмена»/Esc отбрасывают черновик без confirm.
- ВАЛИДАЦИЯ «ПО»: без даты ПО кнопка заблокирована + подпись «укажите дату ПО — иначе план не попадёт на доску» (раскид неоднозначен).
- ПРОВОДКА spread+dept (для превью/овербукинга): board-rows.tsx → project-detail.tsx (PlanningList) → project-plan-row.tsx → ProjectPlanPanel. Inline-поля строки «часы+завершить к» оставил как быстрый путь (спека Q1). Кнопка «✎ План» в конце строки (marginLeft:auto).

НЕ СДЕЛАНО (намеренно, по ТЗ):
- «Вручную по месяцам» = заглушка-disabled (фаза-2, нужен бэк-поле planMethod — зона Dev2, не трогал).
- objects/fields/схема НЕ менялись (Равномерно работает без новых полей: С=startDate, ПО=endDate).

ТЕСТЫ (+новый plan-preview.test.ts, 13 шт): validateRange (4 кейса), previewGranularity (нед/мес), previewBuckets (невалид→[], месяцы покрывают диапазон), computePreview (инвариант Σ≈plannedEffort, раскид по раб.дням июль>сентябрь-обрезанный, овербукинг over=true, без отдела capacity=null/over=false, нулевой объём→0).

СВЕРКА (правило 8): Timetta resource-plan — диапазон С..ПО + превью по периодам + строка Σ + равномерное распределение по рабочему расписанию. Реализовано ровно это (превью = та же формула WI-05, что доска).

ВАЛИДАЦИЯ: yarn lint = 0 warnings/0 errors. yarn test:unit = 71 файл, 1971 passed, 15 todo, 0 failed. yarn twenty dev --once --dry-run = чисто (typecheck ok, 1 frontComponent updated = bundle доски, nothing applied, 0 objects-изменений).

ФАЙЛЫ: +plan-preview.ts +plan-preview.test.ts +project-plan-panel.tsx; M project-plan-row.tsx, project-detail.tsx, board-rows.tsx. Зона capacity соблюдена (не grid/objects/бэк). НЕ коммитил.
— Dev 1
...
## Dev 2 → arch
[report] WI-10 ОТЗЫВ согласования (recall + revoke) — ГОТОВО, dry-run чисто, НЕ коммичу.

ЗОНА: apps/time/src/logic-functions/approval.logic.ts + constants/approval.ts + тесты (бэк/данные). ADDITIVE — submit/approve/reject не тронуты.

ЧТО СДЕЛАНО (по решениям A4.3/A4.4/A4.25/A4.26, сверено с Timetta recall/Reopen):
- НОВЫЙ op `recall` (СОТРУДНИК отзывает СВОЮ отправку): SUBMITTED → DRAFT, пока руководитель не вынес решение (запись ещё SUBMITTED). Право — ВЛАДЕЛЕЦ: actor.employeeId == entry.employeeId. Чужие/не-SUBMITTED пропускаются (skippedForeign). Руководитель НЕ требуется — это действие сотрудника над своими записями. A4.3(в)+A4.4(а).
- НОВЫЙ op `revoke` (РУКОВОДИТЕЛЬ отзывает выданное согласование = Timetta «Reopen»): APPROVED → SUBMITTED (возврат в очередь согласования). RBAC = как approve: требуется actor.isManager (иначе forbidden ДО fetch entry); SoD — нельзя отзывать СВОИ записи (skippedOwn). A4.25(а)+A4.26(а).
- setStatus: при откате назад (DRAFT/SUBMITTED) ОБНУЛЯЕТ approvedBy/approvedAt — запись перестаёт выглядеть согласованной; rejectComment тоже очищается (recall идёт из SUBMITTED, причины там нет). approve/reject-ветки не изменены.
- constants/approval.ts: добавлены RECALL_FROM=SUBMITTED→RECALL_TO=DRAFT, REVOKE_FROM=APPROVED→REVOKE_TO=SUBMITTED + комментарий-SSOT по A4.x. ОТДЕЛЬНЫЙ статус REVOKED НЕ вводил: A4.20(а) = «4 статуса достаточно», A4.25 формулирует целевой как SUBMITTED/DRAFT (переиспользуем существующие). Если решат вводить REVOKED-как-терминал — это +select-option, отдельная задача.
- dev-bypass сохранён как в approve/reject: actor=null (workspaceMemberRef пуст) → guard пропущен + console.warn (DEV). В проде SoD/RBAC строгий (A4.18/A4.19).
- factHours-триггеры (project-fact-rollup) отработают сами — статус меняется через тот же PATCH credosTimeEntries.

КОНТРАКТ ДЛЯ Dev1 (кнопки в my-time/футер таймшита):
- POST /approval (тот же путь). Общие params: workspaceMemberRef (UUID актора, для RBAC/ownership), ids (CSV UUID).
- «Отозвать отправку» (СОТРУДНИК, виден на СВОИХ SUBMITTED): { op:'recall', ids, workspaceMemberRef }. Ответ { ok, updated, skippedForeign }. Показывать на записях статуса SUBMITTED владельца.
- «Отозвать согласование» (РУКОВОДИТЕЛЬ, на APPROVED чужих): { op:'revoke', ids, workspaceMemberRef }. Ответ { ok, updated, skippedOwn } | { ok:false, error:'forbidden...' }. Кнопку в очереди руководителя/approval-view; гейтить по isManager (UX), сервер всё равно проверит.
- Переходы: recall SUBMITTED→DRAFT (свои); revoke APPROVED→SUBMITTED (чужие, manager). Записи не в исходном статусе молча пропускаются (updated не вырастет).

ТЕСТЫ (+11, файл approval.logic.test.ts теперь 33 шт, всё passed):
- recall: ids пуст→error; своя SUBMITTED→DRAFT (approvedBy/At/rejectComment=null); чужая→skippedForeign; не-SUBMITTED(APPROVED)→updated:0; dev-bypass(actor null)→updated:1.
- revoke: ids пуст→error; не-менеджер→forbidden (1 fetch, без entry); чужая APPROVED→SUBMITTED (аудит обнулён); своя→SoD skippedOwn; не-APPROVED(SUBMITTED)→updated:0; dev-bypass→updated:1.

ВАЛИДАЦИЯ: yarn lint = 0 warnings/0 errors. yarn test:unit = 71 файл, 1982 passed (+11), 15 todo, 0 failed. yarn twenty dev --once --dry-run = чисто (typecheck ok, No metadata changes — путь/метод logicFunction те же, op-роутинг внутренний).

ФАЙЛЫ: M approval.logic.ts, M constants/approval.ts, M approval.logic.test.ts. НЕ коммитил.
— Dev 2

## Dev 1 → arch [taking] reports-drill-period

Беру баг: смена периода/гранулярности в Отчётах сбрасывает дрилл на общий отчёт.

## Dev 1 → arch [report] reports-drill-period готово

БАГ (прод): при активном дрилле (Отдел→Проект→Сотрудник, крошки/пилюли) смена периода (Месяц/Квартал/Год + ‹ ›-навигация + гранулярность) сбрасывала стек на корневой отчёт.

ГДЕ СБРАСЫВАЛОСЬ (reports/reports-dashboard.tsx) — ТРИ лишних reset():
- onPeriodChange (‹ › навигация): `() => () => { reset(); fn(); }` — каждый prev/next чистил стек.
- onChange гранулярности в режиме «Сводка»: `{ reset(); setGran(g); }`.
- onChange гранулярности в режиме «Проекты»: то же.

ПОЧЕМУ ХВАТИЛО УБРАТЬ reset (архитектура уже верная):
- drill-стек живёт в useDrill (shared/use-drill.ts), независим от периода.
- Дашборд проецирует стек в OLAP-аргументы: olapFilters = stack.map({dim,value}); childAxis = последний stack[].childAxis ?? rootAxis. Период (from/to) — ОТДЕЛЬНЫЙ аргумент useOlap(period.from, period.to, childAxis, olapFilters, drilled).
- useOlap перезапрашивает при смене from/to с ТЕМИ ЖЕ filters[]/childAxis (filtersKey в deps) → скоуп сохраняется, данные за новый период. KPI scopedTotals завязан на olapData → пересчитывается автоматически. Менять useOlap/use-olap/olap-rest не пришлось.

ЧТО ИЗМЕНИЛ:
- onPeriodChange: убран reset() (только fn()). Комментарий-SSOT про сохранение скоупа.
- Оба onChange гранулярности → `(g) => setGran(g)` без reset. usePeriod.changeGran сам обнуляет offset на текущий период (нельзя «5-й месяц» как квартал) — drill НЕ трогает.
- switchGroupBy (Отдел/Проект/Человек) — reset() ОСТАВЛЕН осознанно: смена rootAxis делает накопленный стек невалидным (другая корневая ось).
- Сброс drill теперь только явный: пилюля ✕ / клик корня крошек (onRoot=reset, onLevel=goToLevel в DrillView). ErrorBoundary resetKeys содержит period.from/to — но это сбрасывает лишь внутреннее состояние ошибки ErrorBoundary, не родительский стек (он выше по дереву).

ПО РЕЖИМАМ:
- Сводка (OLAP-drill) — основной кейс, исправлен.
- Проекты — гранулярность тоже больше не reset (хотя там drill нет, чистота).
- Тренд — сам по периодам, drill отсутствует, не затронут.
- Незаполненные — без периода/drill, не затронут.

ТЕСТ (+1 файл reports/drill-period.test.ts, 4 кейса): смена периода НЕ диспатчит reset (стек = тот же объект); производные filters[]/childAxis инвариантны к периоду; явный reset очищает скоуп; goTo по крошке сужает скоуп не теряя периода. Проекция projectOlap — копия логики дашборда.

ВАЛИДАЦИЯ: yarn lint = 0 warnings/0 errors. yarn test:unit = 72 файла, 1986 passed (+15 после моего файла относительно прошлого прогона), 15 todo, 0 failed. yarn twenty dev --once --dry-run = чисто (typecheck ok, No metadata changes — правки чисто фронтовые).

СВЕРКА (правило 8): Timetta — срез/фильтр сохраняется при смене периода. Поведение приведено в соответствие.

ФАЙЛЫ: M reports/reports-dashboard.tsx, ?? reports/drill-period.test.ts. НЕ коммитил.

Открытых вопросов нет.
— Dev 1

## Dev 1 → arch [taking] project-card-timeentries

Беру: вкладка «Трудозатраты» карточки проекта — табличный реестр с полными колонками, сортировка по дате.

## Dev 1 → arch [report] project-card-timeentries готово

ЗАДАЧА: вкладка «Трудозатраты» карточки проекта — табличный реестр с полной инфой, сортировка по дате.

КОНТЕКСТ (важно для понимания решения):
- Вкладка «Трудозатраты» карточки проекта УЖЕ имела два виджета: FIELDS (узкий инлайн на card-view с relation timeEntries, авто-фильтр по проекту) + RECORD_TABLE-реестр (виджет ...RP_W_TIME_ENTRIES_REGISTRY), ссылающийся на index-view CREDOS_TIME_ENTRY_VIEW («Все записи»).
- По документированному механизму Twenty (CARDS_VIEWS_AUDIT §RECORD_TABLE, стр.166-169): RECORD_TABLE-виджет показывает view ЦЕЛИКОМ и по родителю НЕ фильтруется — скоуп по проекту даёт ТОЛЬКО соседний FIELDS-виджет на relation. Колонки/сортировка настраиваются лишь на самой view. Поэтому правил view, а не page-layout.

ЧТО СДЕЛАЛ (1 файл: views/credos-time-entry.view.ts — SSOT-реестр записей):
- Добавил sorts: дата DESC (свежие сверху) — ViewSortDirection.DESC. UUID a1f3d7b2-6c84-4e51-9a0d-2f7b8c1e5d34.
- Добавил колонку «Теги» (CREDOS_TIME_ENTRY_TAGS_FIELD_ID, MULTI_SELECT), position 7. UUID c8e4b9a1-7d52-4f63-8b1c-3a6d9e2f0c75.
- Итог колонки (полная инфа): Состав работ(=комментарий, label) · Дата · Часы · Проект · Вид работ(=категория) · Работник · Статус · Теги.

ПРО КОЛОНКИ «Категория»/«Комментарий»:
- Отдельного поля «категория» у записи НЕТ — категория живёт на Проекте; на записи аналог = Вид работ (workType). Срез по категории — через связь Проект (так же зафиксировано в CARDS_VIEWS_AUDIT §4).
- «Комментарий» = поле description («Состав работ», labelIdentifier записи) — уже было колонкой.
- ПДн: revealEmployeeNames применяется во front-component-таблицах (таймшит/отчёты/команда), а тут НАТИВНЫЙ RECORD_TABLE ядра — имена сотрудников рендерит ядро штатно (как и было). Доп.маскирование тут неприменимо.

КАРТОЧКА СОТРУДНИКА — сделано «бесплатно»:
- Вкладка «Трудозатраты» карточки сотрудника (employee.page-layout RP_W_TIME_ENTRIES_REGISTRY) ссылается на ТУ ЖЕ view CREDOS_TIME_ENTRY_VIEW → автоматически получила сортировку по дате DESC + колонку Теги + полный набор. Скоуп по сотруднику — соседний FIELDS-виджет (relation timeEntries).
- Также наследуют изменение: карточка записи (FIELDS) и пункт сайдбара «Записи» — date DESC + Теги везде, поведение разумное (свежие сверху).

ВАЛИДАЦИЯ:
- yarn lint = 0 warnings / 0 errors.
- yarn test:unit = 72 файла, 1986 passed, 15 todo, 0 failed.
- yarn twenty dev --once --dry-run = чисто. Metadata changes: 2 created (viewField c8e4b9a1… Теги + viewSort a1f3d7b2… дата DESC). БЕЗ field-drop, дублей UUID нет, typecheck ok, apply-готово.

СВЕРКА (правило 8): Timetta — в карточке проекта список трудозатрат таблицей с датами. Реестр записей с колонкой Дата + сортировка по дате DESC соответствует.

ФАЙЛЫ: M views/credos-time-entry.view.ts. НЕ коммитил.

Открытых вопросов нет.
— Dev 1

## Dev 1 → arch [taking] wi-31-32

Беру WI-31 (Excel-ввод) + WI-32 (клавиатура+SSOT клавиш). Зона grid/. Hot-файлы (use-keyboard/cheatsheet/hour-cell/week-grid) — один заход последовательно.

## Dev 1 → arch [report] wi-31-32 готово

ЗАДАЧА: WI-32 (P0-баг клавиатуры + SSOT) и WI-31 (Excel-паттерн ввода). Всё ADDITIVE, без host-DOM (RemDOM-safe), lock-approved уважается (commitCell идёт серверным путём /s/time-entry, CISO-012).

РЕШЕНИЕ ПО Shift+Enter (E4.5) — ВАЖНО:
- Диагноз TZ «обработчика НЕТ» устарел: обработчик Shift+Enter УЖЕ был в week-grid.tsx (onContainerKeyDown → onBulkFill «часы ячейки на все будни строки»), и cheatsheet описывал его корректно. Настоящая проблема = клавиша шла мимо единого источника (keyAction/keymap), потому дрейф док↔код был возможен.
- Решил по E4.5 = (а) «реализовать», НЕ убирать из cheatsheet. Привёл к SSOT: Shift+Enter теперь распознаётся в keyAction (тип bulkFillRow), week-grid маршрутизирует bulk-fill через keyAction. cheatsheet↔обработчик гарантированно согласованы.

ПО КАЖДОМУ ПУНКТУ:
WI-32:
- E4.14 SSOT: новый grid/keymap.ts — массив SHORTCUTS {id, keys, desc}. cheatsheet.tsx берёт его (локальный KEYS-дубль убран). keyAction маппит те же клавиши. Тест keymap.test.ts ловит дрейф: id уникальны, у всех keys/desc, каждая модификаторная клавиша даёт действие ≠ none.
- E4.5 Shift+Enter — см. выше (bulkFillRow в keyAction, провод в week-grid).
- E1.18 внутренний буфер: clipboard:number|null React-стейт в weekly-grid. Ctrl/Cmd+C → значение активной ячейки в буфер; Ctrl/Cmd+V → commitCell в активную. Системный clipboard RemDOM не трогаю. keyAction: copy/paste (meta=ctrl синонимы mac/win).
WI-31:
- E1.5 seed-replace/click-select-all: seed уже заменял значение (ок). Клик в ячейку → input.select() (selectOnFocus-ref, отличаю клик от seed-печати — на seed select не делаю). F2-паттерн.
- E1.6 невалид → красная рамка input (T.over) + inline-подсказка «Не понял. Форматы: 8 · 1.5 · 1:30 · 1ч30м» под инпутом. На Enter/Tab при невалиде редактор НЕ закрывается (держим рамку); правка ввода снимает invalid; blur (явный уход) — откат к последнему. Тихого отката больше нет.
- E1.7 placeholder «8 / 8:30».
- E1.3 Shift+клик диапазон: координаты (row,col) — nav.anchor/extendTo/selection/isSelected + чистые cellRange/inRange. Подсветка диапазона T.accentSoft. Без host-DOM, без drag.
- E1.20 Ctrl+D «заполнить вниз»: значение активной ячейки вниз по столбцу (пропуск locked и уже равных). keyAction fillDown, провод fillDown в weekly-grid через commitCell (согласованные отсеются сервером).

ФАЙЛЫ:
- ?? grid/keymap.ts, ?? grid/keymap.test.ts
- M grid/use-keyboard.ts (keyAction +bulkFillRow/fillDown/copy/paste; cellRange/inRange; Nav +anchor/extendTo/selection/isSelected), M grid/use-keyboard.test.ts (+модификаторы, +cellRange/inRange)
- M grid/cheatsheet.tsx (из SSOT), M grid/hour-cell.tsx (select-on-click, красная рамка+подсказка, placeholder, selected-подсветка, Shift+клик extend), M grid/grid-row.tsx (selected + extend), M grid/week-grid.tsx (keyAction-маршрут массовых/буфера, новые пропы), M grid/weekly-grid.tsx (clipboard-стейт, fillDown)

ТЕСТЫ: keyAction (Shift+Enter, Ctrl/Cmd+D/C/V, «d» без модификатора=none, Enter без Shift=move); cellRange (1/гориз/2×2/нормализация); inRange (внутри/граница/вне/нет якоря); keymap SSOT-инварианты + антидрейф. Компоненты тут не mount-тестируются (env=node, без jsdom) — логика E1.6 опирается на parseHours===null, покрытую в format.test.ts.

ВАЛИДАЦИЯ:
- yarn lint = 0 warnings / 0 errors.
- yarn test:unit = 73 файла, 2008 passed, 15 todo, 0 failed.
- yarn twenty dev --once --dry-run = чисто. Metadata: 2 updated (viewSort из прошлой задачи в рабочем дереве + хэш бандла frontComponent «Трудозатраты» — ожидаемо, правил grid-компоненты). Field-drop нет, новых объектов нет, typecheck ok, apply-готово.

СВЕРКА (правило 8): Timetta/Kimai — Excel-ввод (печать заменяет, клик=выделить, Shift-диапазон), горячие клавиши с подсказкой, заполнение вниз. Соответствует.

Открытых вопросов нет. Follow-up (не входило в задачу): E4.6 Ctrl+R, Home/End, [/], Tab-wrap, Ctrl+Z — на WI-32-дельту.
— Dev 1

## Dev 1 → arch [taking] wi-45

## Dev 1 → arch [report] wi-45 готово

WI-45 (дельты W3-A к grid). dry-run, НЕ коммичу. Зона grid/. ADDITIVE. WI-31/32 не сломаны (Ctrl+D=заполнить-вниз, Excel-ввод, keymap-SSOT целы).

СДЕЛАНО (по решениям W3A):

**W3A.5 — раскладка Shift+Enter / bulk-fill (без коллизии с Ctrl+D)**
- Shift+Enter = ВВЕРХ (commit-up, зеркало Enter=вниз). Освобождён от bulk-fill.
- bulk-fill будней перевешен на **Alt+→** (Ctrl+D остаётся «заполнить-вниз», коллизии нет).
- keyAction: `+rowEdge/+gridEdge`, Shift+Enter→move(-1), Alt+→→bulkFillRow, KeyEvent +altKey. keymap.ts SSOT обновлён (commit-up, bulk-fill-row=«Alt+→», +row-edge/+grid-edge), cheatsheet тянет из SSOT автоматически.

**W3A.8 — Home/End**
- Home/End = первая/последняя РЕДАКТИРУЕМАЯ ячейка строки (пропуск locked) через чистую `rowEdgeCol(locked[], dir)`.
- Ctrl+Home/End = края всей сетки (первая/последняя строка).
- Маршрут в week-grid.onContainerKeyDown (там lockedByDay). В РЕЖИМЕ РЕДАКТИРОВАНИЯ Home/End гасятся (stopPropagation) — двигают курсор в инпуте, не прыгают по ячейкам.

**W3A.9 — порядок меню ⋯ по частоте**
Заполнить будни нормой → Комментарий → Дублировать → [разд] → Обнулить → Убрать.

**W3A.10/11 — деструктив**
- Иконки ТОЛЬКО у деструктивных: Обнулить (× в круге), Убрать (корзина). Остальное — текст+hint.
- «Убрать строку» = inline-confirm (НЕ window.confirm — RemDOM) со счётчиком «удалить N записей» (русское склонение `pluralRecords`); пусто→«нет записей, исчезнет из таблицы».
- «Обнулить часы» = мгновенно + **undo-тост «Отменить» 5с** (новый компонент undo-toast.tsx, снизу-слева; правый угол занят ValidationToast). Реальный undo: снимок несогласованных дней с часами → re-upsert через data.upsertMany. Снимок снимается ДО очистки.

**W3A.17 — placeholder**
Пустая ячейка показывает БЛЕДНУЮ норму дня (useDailyNorm/normFor, textFaint opacity .55, weight 400) вместо «·». Выходной/праздник/locked (норма 0) → прежняя точка. normFor проброшен week-grid→GridRow→HourCell (и в project-view).

**W3A.25 — размеры**
Высота ячейки 32→40 (hour-cell base.height). Ширина дня 58→64 (week-header DAYS_COLS, 9 колонок без h-scroll). week-header выровнен (GRID_TEMPLATE общий для шапки/строк/футера — авто-синхрон).

ФАЙЛЫ:
- M grid/keymap.ts (+commit-up/row-edge/grid-edge; bulk-fill-row=«Alt+→»; Shift+Enter=«вверх»)
- M grid/use-keyboard.ts (keyAction: Shift+Enter→up, Alt+→→bulkFill, Home/End→rowEdge, Ctrl+Home/End→gridEdge, +altKey; +чистая rowEdgeCol)
- M grid/week-grid.tsx (rowEdge/gridEdge маршрут, altKey, normFor→GridRow)
- M grid/grid-row.tsx (normFor+normHint проброс, recordCount для confirm)
- M grid/hour-cell.tsx (height 40, normHint-placeholder, Home/End stopPropagation в edit)
- M grid/row-menu.tsx (порядок W3A.9, иконки ActionIcon, ConfirmDelete-view, pluralRecords export)
- M grid/week-header.tsx (день 64px)
- M grid/project-view.tsx (normFor→GridRow)
- M grid/weekly-grid.tsx (undo-стейт+снимок в clearRow, UndoToast рендер)
- ?? grid/undo-toast.tsx (новый компонент)
- M тесты: keymap.test.ts, use-keyboard.test.ts (+rowEdgeCol, +Alt+→/Home/End/Shift+Enter=up), row-menu.test.ts (+pluralRecords)

ВАЛИДАЦИЯ:
- yarn lint = 0 warnings / 0 errors.
- yarn test:unit = 73 файла, 2031 passed, 15 todo, 0 failed.
- yarn twenty dev --once --dry-run = чисто (typecheck ok; metadata: 2 updated — viewSort из рабочего дерева + хэш бандла frontComponent «Трудозатраты», ожидаемо; field-drop/новых объектов нет; apply-готово). НЕ dev --once.

СВЕРКА (правило 8): Timetta — Shift+Enter навигация, Home/End края, ячейка ~64×40, плейсхолдер-норма, confirm на необратимом, undo на мгновенном. Соответствует.

ПРИМЕЧАНИЯ:
- W3A.7 (Delete+undo) — undo-механизм теперь есть (undo-toast), при желании легко подключить к Delete-обнулению ячейки в отдельной дельте.
