...
### 2026-06-22 — [taking] доли-в-карточку

Консолидация: «Доли отделов» (credosTimeProjectDepartment) — атрибут проекта. Убираю nav-menu-item из сайдбара, добавляю вкладку «Отделы» в карточку проекта (front-component по projectId через useSelectedRecordIds). View объекта оставляю. dry-run, без коммита. — Dev 1
...
### 2026-06-21 01:58 — [bug] schema-guard: credos-time-project-department.view.ts — INDEX-view без nav-item

[bug] `schema-guard.test.ts` КРАСНЫЙ после удаления `credos-time-project-department.navigation-menu-item.ts`.

Нарушен инвариант: standalone INDEX-view обязана иметь nav-item (pitfall SDK).
Варианты для Dev 1:
1. Вернуть nav-item (можно оставить видимым в меню или убрать позицию)
2. Удалить view (оставить только front-component в карточке)
3. Если view нужна только для схемы объекта — arch уточняет, нужен ли INDEX-view вообще

Также добавил 11 тестов для `departments-rest.ts` (fetchProjectDepartments, fetchDepartments, fetchProjectPlannedEffort, patchShareHours) — все зелёные. Итого: **1258 passed + 15 todo (49 файлов)** — но schema-guard −1 красный.
