...
| CISO-005 | P1 | **OPEN (TOFU-INTERIM)** | IDOR: личность из client-ref. **Механизм есть:** `resolveActor` (server-side `event.userWorkspaceId`→employee, TOFU-мост) + ownership/on-behalf guard (`canWriteFor`) на time-entry CRUD + plan-write + approval. **НЕ закрыт полностью:** DoD/pre-seed userWorkspaceRef не выполнен + DEV-fallback «первый активный» (untrusted-ветка) жив = остаточный IDOR при пустых ref. Закрытие: pre-seed + enforce userMapPending + убрать DEV-fallback; строгий admin (вместо isManager-деградации) — RBAC-волна. Статус по RISK_REGISTER. |
...
| CISO-007 | P1 | **ACCEPTED** | `byEmployee` + detail-режим в `/s/reports` раскрывали ФИО без role-guard | **Решение владельца (2026-06-22): `revealEmployeeNames` defaultValue=TRUE** (законный интерес 152-ФЗ — внутренний учёт). При reveal=false — везде стабильный КОД `Сотрудник·<deptCode>·<4hex>` (не ФИО/UUID): detail/byEmployee/OLAP/CSV/grid/project-team/missing. Тоггл — администратор. Per-роль scope (HR-видит-ФИО) — RBAC-волна. Статус по RISK_REGISTER. |
...
| CISO-012 | P2 | MITIGATING | APPROVED-ячейки редактируемы через UI: Core REST доступен в обход server-guard из фронта | L1 ✅ UI: `isCellLocked()` + `notifyLocked()` (Dev1 W6-2). L2 ✅ Роутинг: все мутации грида через `/s/time-entry` → `cannot_modify_approved` гард (Dev1 ciso012-routing). L3 RLS: нужен CISO-005 (server-actor). DB-event veto НЕВОЗМОЖЕН в SDK |
