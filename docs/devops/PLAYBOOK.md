...
| `settingsCustomTab` — вкладка «Настройки» в карточке app = «Nothing to configure», в ЛЕВОМ сайдбаре Settings НЕТ | на Twenty 2.14 settingsCustomTab front-component рендерится под вкладкой **«На заказ» (#custom)** карточки app (Settings→Приложения→app→На заказ), НЕ в левом сайдбаре Settings и НЕ под вкладкой «Настройки» | (1) доступ — через «На заказ»; (2) для удобства продублировать nav-пунктом + page-layout в сайдбаре app (как Отчёты). Глобальная вкладка в левом сайдбаре Settings — только патчем ядра useSettingsNavigationItems (как Credos CRM), при своём образе |
